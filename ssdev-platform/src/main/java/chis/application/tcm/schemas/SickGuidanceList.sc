...
<entry entityName="chis.application.tcm.schemas.SickGuidanceList" tableName="chis.application.tcm.schemas.TCM_SickGuidance" sort="SGID desc" alias="中医指导">
	...
	<!--	
		<item id="intendedPopulation" alias="适应人群" type="string" length="20">
			<dic id="chis.dictionary.TCMIntendedPopulation"/>
		</item>
		<item id="ageGroupType" alias="年龄段分类" type="string" length="1">
			<dic id="chis.dictionary.TCMAgeGroupType"/>
		</item>
		<item id="tumourType" alias="肿瘤类别" type="string" length="20">
			<dic id="chis.dictionary.tumourHighRiskType"/>
		</item>
		<item id="contentType" alias="内容分类" type="string" length="20">
			<dic id="chis.dictionary.TCMContextType"/>
		</item>
		<item id="habitusType" alias="体质分类" type="string" length="20">
			<dic id="chis.dictionary.TCMHabitusType"/>
		</item>
		<item id="content" alias="内容" type="string" xtype="textarea" length="4000" height="300"/>
		-->
	<item id="guidanceDate" alias="指导日期" type="date" update="false" defaultValue="%server.date.date" maxValue="%server.date.date"/>
	<item id="guidanceDoctor" alias="指导医生" type="string" length="20" update="false"  defaultValue="%user.userId">
		<dic id="chis.dictionary.user" render="Tree" onlySelectLeaf="true" keyNotUniquely="true" parentKey="%user.manageUnit.id"/>
	</item>
	<item id="guidanceUnitId" alias="指导医生机构" type="string" length="20" update="false" width="320" defaultValue="%user.manageUnit.id" fixed="true">
