...
$import(
		...
		"chis.script.PrintWin2",
		"util.widgets.DateTimeField"
		)
chis.script.ChisSimpleListView = function(cfg){
	this.width = 620;
	this.height = cfg.height || 350
	this.selectFirst = true
	this.enableCnd = true
	this.autoLoadData = true
	this.listServiceId = cfg.listServiceId || "simpleQuery"
	this.removeServiceId = cfg.removeServiceId || "simpleRemove"
	this.listMethod = "execute"
	...
	chis.script.ChisSimpleListView.superclass.constructor.apply(this,[cfg])
}
Ext.extend(chis.script.ChisSimpleListView, app.desktop.Module,{
	init:function(){
		this.addEvents({
			"gridInit":true,
			"beforeLoadData":true,
		...
		this.requestData = {serviceId:this.listServiceId,method:this.listMethod,schema:this.entryName,cnd:this.initCnd,pageSize:this.pageSize || 25,pageNo:1}
...
//			f.rendered = false
	...
	doAfterCreateQueryField:function(f, it){
	},
...
//						f.type = "date"
	...
	getCM:function(items){
		...
		if (this.showRowNumber) {
			cm.push(new Ext.grid.RowNumberer({header:'序号'}))
		}
		for(var i = 0; i <items.length; i ++){
			var it = items[i]
	...
	onDestory:function(m){
		if(this.mainApp && m.fullId){
			var tm = this.mainApp.taskManager,id = m.fullId;
			var module = tm.tasks.item(id)
			if(module && module.instance){
				module.instance = null;
				tm.tasks.removeKey(id)
			}
		}
	},
	...
	processRemove:function(){
		...
		util.rmi.jsonRequest(removeCfg,
