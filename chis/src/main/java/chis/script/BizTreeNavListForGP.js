...
chis.script.BizTreeNavListForGP = function(cfg) {
    this.mutiSelect = true
    this.westWidth = cfg.westWidth || 150
    this.gridDDGroup = 'gridDDGroup'
    this.saveServiceId = "simpleSave"
    this.showNav = true
    chis.script.BizTreeNavListForGP.superclass.constructor.apply(this, [cfg])
    this.width = 950;
    this.height = 450
}
Ext.extend(chis.script.BizTreeNavListForGP, chis.script.BizSimpleListView, {
            ...
            switch (it.type) {
                ...
                case 'timestamp' :
                    if (it.xtype == "datefield") {
                        ...
                        cnd[1] = ['$',"str(" + refAlias + "." + it.id + ",'yyyy-MM-dd HH:mm:ss')"]
                        cnd.push(['s', v])
                    }
                    break;
            }
    ...
    saveToServer : function(saveData, r) {
        ...
        this.tree.el.mask("正在保存数据...", "x-mask-loading")
