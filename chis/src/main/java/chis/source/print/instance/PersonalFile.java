...
public class PersonalFile extends BSCHISPrint implements IHandler {
	...
	private Map<String, Object> getSampleInfoMap(Map<String, Object> map,
		...
		if (parameters.containsKey("educationCode")) { //文化程度字典值，字典路径 platform/chis/dictionary/educationjbxx.dic
			String educationCode = (String) parameters.get("educationCode");
			System.out.println("educationCode字典值------->"+educationCode);
			if (educationCode != null) {
				if (educationCode.equals("1")) {  //字典值  1表示 文盲或半文盲
					parameters.put("educationCode", "9"); //打印值  9表示 文盲或半文盲
				} else if (educationCode.equals("2")) { //字典值  2表示 小学
					parameters.put("educationCode", "8"); //打印值  8表示 小学
				} else if (educationCode.equals("3")) { //字典值  3表示 初中
					parameters.put("educationCode", "7"); //打印值  7表示 初中
				} else if (educationCode.equals("4")) { //字典值  4表示 高中
					parameters.put("educationCode", "6"); //打印值  6表示 高中
				} else if (educationCode.equals("5")) { //字典值  5表示 大学专科和专科学校
					parameters.put("educationCode", "3"); //打印值  3表示 大学专科和专科学校
				}else if (educationCode.equals("6")) { //字典值  6表示 不详
					parameters.put("educationCode", "10"); //打印值  10表示 不详
				}else if (educationCode.equals("7")) { //字典值  7表示 研究生
					parameters.put("educationCode", "1"); //打印值  1表示 研究生
				}else if (educationCode.equals("8")) { //字典值  8表示 大学本科
					parameters.put("educationCode", "2"); //打印值  2表示 大学本科
				}else if (educationCode.equals("9")) { //字典值  9表示 中等专业学校
					parameters.put("educationCode", "4"); //打印值  4表示 中等专业学校
				}else {								   //字典值  10表示 技工学校
					parameters.put("educationCode", "5"); //打印值 5表示 技工学校
				}
			}
		}
