...
public class DefaultActionExecutor implements ActionExecutor {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see chis.source.ActionExecutor#execute(java.util.Map,
	 * java.util.Map, ctd.util.context.Context,
	 * chis.source.AbstractActionSupportableService)
	 */
	...
	private void invoke(String action, List<Object> parameters,
			...
			throws InvocationTargetException, PersistentDataOperationException,
	...
	private void invokeNoDBSerivceMethod(AbstractActionService service,
		...
		for (Iterator<Object> it = parameters.iterator(); it.hasNext();) {
			Object obj = it.next();
			if (obj instanceof Map && !obj.getClass().getName().equals(Context.class.getName())){
				clses.add(Map.class);
			} else {
