...
chis.application.psy.script.visit.PsychosisVisitPlanListView = function(cfg){
	cfg.initCnd = cfg.cnds || ['eq', ['$', 'a.businessType'], ['s', '10']];
	this.needOwnerBar = true;
	chis.application.psy.script.visit.PsychosisVisitPlanListView.superclass.constructor.apply(this,[cfg]);
	this.businessType = "10";
}

Ext.extend(chis.application.psy.script.visit.PsychosisVisitPlanListView,chis.script.BizSimpleListView,{
	...
	doModify : function() {
		...
		this.planId = r.get("planId");
		this.showEhrViewWin(r);
	},

	showEhrViewWin : function(r) {
		var m = this.midiModules["psy_ehrView"];
		var visitModule = ['P_01', 'P_02', 'P_03', 'P_04'];
		if (!m) {
			...
			this.midiModules["psy_ehrView"] = m;
			m.exContext.ids["empiId"] = this.empiId;
			m.exContext.ids["visitId"] = this.visitId;
			m.exContext.args["selectedPlanId"] = this.planId;
			m.exContext.args.planDate=r.planDate;
