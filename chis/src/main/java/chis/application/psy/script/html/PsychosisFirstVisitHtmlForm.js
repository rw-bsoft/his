...
$import("chis.script.BizHtmlFormView","chis.application.psy.script.html.PsychosisFirstVisitHtmlTemplate","chis.script.util.widgets.MyMessageTip");
$styleSheet("chis.css.PsychosisHtml")

chis.application.psy.script.html.PsychosisFirstVisitHtmlForm = function(cfg){
	chis.application.psy.script.html.PsychosisFirstVisitHtmlForm.superclass.constructor.apply(this,[cfg]);
	Ext.apply(this,chis.application.psy.script.html.PsychosisFirstVisitHtmlTemplate);
	...
	"visitType_1","visitType_2","visitType_3","visitType_0","this.lastHospitalizationTime"];
	this.otherDisable = [
...
Ext.extend(chis.application.psy.script.html.PsychosisFirstVisitHtmlForm,chis.script.BizHtmlFormView,{
	...
	onLoadData:function(entryName,body){
		this.resetControlOtherFld();
		this.addFieldDataValidateFun(this.schema);
		if(this.initDataId){//修改时，随访日期不可修改
	...
	initFormData : function(data) {
		...
		if(fvData.riskFactor){
			this.frv = fvData.riskFactor.key;
		}
	...
	checkFldValidate : function(){
		...
		if (eflen > 0) {
			...
			if (isReturn) {
				//Ext.Msg.alert("提示", "精神病首次随访 页面上有验证未通过的字段，请修改录入值");
				MyMessageTip.msg("提示", "精神病首次随访 页面上有验证未通过的字段，请修改录入值",true);
				return false;
			}else{
				return true;
			}
	...
	getSaveData : function(){
		var body = {};
		body.firstVisitData = this.getFormData();
		var visitId = body.firstVisitData.visitId;
		if(!visitId){
			this.op = "create";
		}else{
			this.op = "update";
		...
		body.firstVisitData.visitEffect = '1';
		body.firstVisitData.deleteMedicine = false;
		var curDate = Date.parseDate(this.mainApp.serverDate, "Y-m-d");
		var curUid = this.mainApp.uid;
		var curUnitId = this.mainApp.deptId;
		if(body.firstVisitData.visitId != ''){
			body.firstVisitData.lastModifyUser = curUid;
			body.firstVisitData.lastModifyUnit = curUnitId;
			body.firstVisitData.lastModifyDate = curDate;
		}else{
			body.firstVisitData.createUser = curUid;
			body.firstVisitData.createUnit = curUnitId;
			body.firstVisitData.createDate = curDate;
			body.firstVisitData.lastModifyUser = curUid;
			body.firstVisitData.lastModifyUnit = curUnitId;
			body.firstVisitData.lastModifyDate = curDate;
		}
	...
	onReadyAffter : function(){
		...
		this.frv = this.getHtmlFldValue("riskFactor");
	...
	onRiskFactorRadioClick : function(rfFld,me){
		var rfv = rfFld.value;
		var dgv = rfv;
		if(rfFld.checked && !me.FRType && rfv == me.frv){
			rfFld.checked = false;
			dgv = "0";
			me.FRType = true;
		}else{
