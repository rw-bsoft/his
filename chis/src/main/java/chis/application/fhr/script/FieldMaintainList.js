...
chis.application.fhr.script.FieldMaintainList = function(cfg) {
	this.schema = cfg.entryName;
	chis.application.fhr.script.FieldMaintainList.superclass.constructor.apply(this, [cfg]);
	this.removeServiceId = "chis.templateService";
	this.removeAction = "removeFieldMaintain";
}
Ext.extend(chis.application.fhr.script.FieldMaintainList, chis.script.BizSimpleListView, {
			...
			getFieldMaintainForm : function(r) {
				var m = this.midiModules["fieldMaintainModule"];
				if (!m) {
					...
					this.midiModules["fieldMaintainModule"] = m;
				}
				m.initDataId = r.id;
				return m;
			},
