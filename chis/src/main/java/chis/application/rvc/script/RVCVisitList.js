...
chis.application.rvc.script.RVCVisitList = function(cfg) {
	cfg.bbar = this.getBbar();
	cfg.autoLoadData = false;
	cfg.disablePagingTbr = true;
	chis.application.rvc.script.RVCVisitList.superclass.constructor.apply(this,
			[ cfg ])
}

Ext.extend(chis.application.rvc.script.RVCVisitList,
		chis.script.BizSimpleListView, {
	
	init : function(){
		this.serverParams = {"queryDate" : Date.parseDate(
				this.mainApp.serverDate, "Y-m-d")};
		chis.application.rvc.script.RVCVisitList.superclass.init.call(this);
	},
