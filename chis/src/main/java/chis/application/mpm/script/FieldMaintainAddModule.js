...
chis.application.mpm.script.FieldMaintainAddModule = function(cfg) {
	chis.application.mpm.script.FieldMaintainAddModule.superclass.constructor
			.apply(this, [cfg]);
...
Ext.extend(chis.application.mpm.script.FieldMaintainAddModule, chis.script.BizCombinedModule2, {
			initPanel : function() {
				var panel = chis.application.mpm.script.FieldMaintainAddModule.superclass.initPanel
						.call(this);
				this.panel = panel;
			...
			onWinShow : function() {
				...
				this.form.doCreate();
				var useTypeFld = this.form.form.getForm().findField("useType");
				var cnd = [];
				if(useTypeFld){
					if(this.useType){
						useTypeFld.setValue(this.useType);
						useTypeFld.disable();
						cnd = ['eq',['$','useType'],['s',this.useType.key]];
					}else{
						useTypeFld.setValue();
						useTypeFld.enable();
					}
