...
chis.application.dr.script.DRReferralsReferralsAuditList = function(cfg) {
	chis.application.dr.script.DRReferralsReferralsAuditList.superclass.constructor.apply(
			this, [cfg]);
}
/*******************************************************************************
 * 下转转入审核列表(大数据接口)页面内嵌HIS zhaojian 2019-08-28
 */
Ext.extend(chis.application.dr.script.DRReferralsReferralsAuditList,
		chis.script.BizModule, {
			initPanel : function() {
				...
				var result = util.rmi.miniJsonRequestSync({
					...
					body : {
						pdata : pdata,
						serviceurl : "sendExchangeAudit"//服务地址
					}
				});
