...
chis.application.dbs.script.risk.DiabetesRiskHTForm = function(cfg) {
	cfg.colCount  = 3
	cfg.autoLoadSchema = false
	chis.application.dbs.script.risk.DiabetesRiskHTForm.superclass.constructor.apply(
			this, [cfg])
	this.on("winShow",this.onWinShow,this)
}
Ext.extend(chis.application.dbs.script.risk.DiabetesRiskHTForm,
		chis.script.BizTableFormView, {
			saveToServer:function(saveData){
				saveData.riskAssessmentId = this.exContext.args.record.data.recordId
				chis.application.dbs.script.risk.DiabetesRiskHTForm.superclass.saveToServer.call(this,saveData)
			}
			,
			onWinShow:function(){
				this.loadData()
