...
chis.application.dbs.script.visit.DiabetesVisitForm = function(cfg) {
	...
	chis.application.dbs.script.visit.DiabetesVisitForm.superclass.constructor
			.apply(this, [cfg])
...
Ext.extend(chis.application.dbs.script.visit.DiabetesVisitForm,
		chis.script.BizTableFormView, {
			...
			doNew : function() {
				chis.application.dbs.script.visit.DiabetesVisitForm.superclass.doNew
						.call(this)
			},
			...
			initFormData : function(data) {
				...
				chis.application.dbs.script.visit.DiabetesVisitForm.superclass.initFormData
						.call(this, data)
			...
			onReady : function() {
				chis.application.dbs.script.visit.DiabetesVisitForm.superclass.onReady
						.call(this)
				...
				form.findField("medicineNot").on("select",
						this.onMedicineNotChange, this);
				form.findField("medicineNot").on("change",
						this.onMedicineNotChange, this);
				...
				var healthProposal = form.findField("healthProposal");
				if (healthProposal) {
					healthProposal.on("select", this.onHealthProposalSelect,
							this);
				}
			...
			onDiabetesTypeSelect : function(combo, record, index) {
				var value = combo.getValue();
				var form = this.form.getForm()
				var diabetesChange = form.findField("diabetesChange");
				if (!value || value == "") {
					diabetesChange.setValue({
								key : "n",
								text : "否"
							});
				}
				if (this.diabetesType && this.diabetesType == value) {
					diabetesChange.setValue({
								key : "n",
								text : "否"
							});
			...
			onMedicineSelect : function() {
				...
				var medicineNot = this.form.getForm().findField("medicineNot");
				if (this.medicineField.getValue() == 1
						|| this.medicineField.getValue() == "") {
					medicineNot.allowBlank = true;
					medicineNot.setValue();
					medicineNot.disable();
					Ext.getCmp(medicineNot.id).getEl().up('.x-form-item')
							.child('.x-form-item-label').update("不规律服药原因:");
				} else {
					medicineNot.allowBlank = false;
					medicineNot.enable();
					Ext.getCmp(medicineNot.id).getEl().up('.x-form-item')
							.child('.x-form-item-label')
//							.update("<span style='color:red'>" + "不规律服药原因:"
//									+ "</span>");
							.update("<span>" + "不规律服药原因:"
									+ "</span>");
				}
				this.onMedicineNotChange();
				this.validate();
			},
			onMedicineNotChange : function() {
				var medicineNot = this.form.getForm().findField("medicineNot");
				var medicineOtherNot = this.form.getForm()
						.findField("medicineOtherNot");
				var newValue = medicineNot.getValue();
				if (newValue == 99) {
					medicineOtherNot.allowBlank = false;
					medicineOtherNot.enable();
					Ext.getCmp(medicineOtherNot.id).getEl().up('.x-form-item')
							.child('.x-form-item-label')
							.update("<span style='color:red'>" + "其他原因:"
									+ "</span>");
				} else {
					medicineOtherNot.allowBlank = true;
					medicineOtherNot.disable();
					medicineOtherNot.setValue();
					Ext.getCmp(medicineOtherNot.id).getEl().up('.x-form-item')
							.child('.x-form-item-label').update("其他原因:");
				}
				this.validate();
			},
			onVisitDateSelect : function() {
				if (this.planMode == 1) {
					...
					var endDate = this.exContext.args.r.get("endDate")
					if (nowDate > endDate) {
			...
			onWeightCheck : function(f, e) {
				...
				if (this.weight.getValue() > 500 || this.weight.getValue() < 20) {
					// alert("身高输入非法")
					this.weight.markInvalid("体重必须在20-500之间")
					this.weight.focus()
					return
				}
				this.onCalculateBMI()
...
//												+ items[i].alias + ":</span>");
			...
			onHealthProposalSelect : function(combo, node) {
				var value = combo.getValue();
				var form = this.form.getForm()
				var otherHealthProposal = form.findField("otherHealthProposal")
				if (value.indexOf("8") > -1) {
					otherHealthProposal.enable();
				} else {
					otherHealthProposal.setValue();
					otherHealthProposal.disable();
				}
			},
