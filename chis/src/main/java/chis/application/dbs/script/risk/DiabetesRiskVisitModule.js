...
chis.application.dbs.script.risk.DiabetesRiskVisitModule = function(cfg) {
	cfg.width = 880;
	cfg.autoLoadData = false
	chis.application.dbs.script.risk.DiabetesRiskVisitModule.superclass.constructor.apply(this,
			[cfg])
	this.itemWidth = 215
	this.on("winShow", this.onWinShow, this)
	this.nowDate = this.mainApp.serverDate
}
Ext.extend(chis.application.dbs.script.risk.DiabetesRiskVisitModule, chis.script.BizCombinedModule2, {

			initPanel : function() {
				var panel = chis.application.dbs.script.risk.DiabetesRiskVisitModule.superclass.initPanel
						.call(this)
				this.panel = panel;
...
//				// ** 获取检查日期的最大值和最小值
				...
				if (now > endDate) {
					this.form.form.getForm().findField("visitDate").setMaxValue(Date.parseDate(this.nowDate, "Y-m-d"));
					this.form.form.getForm().findField("visitDate").setMinValue(new Date(r.get("beginDate")));
				} else {
					this.form.form.getForm().findField("visitDate").setMaxValue(Date.parseDate(this.nowDate, "Y-m-d"));
					this.form.form.getForm().findField("visitDate").setMinValue(new Date(r.get("beginDate")));
				}
