...
chis.application.dbs.script.visit.DiabetesVisitMedicineForm = function(cfg) {
	var jobId = cfg.mainApp.jobId;
	if (jobId == "chis.01" || jobId == "chis.05" || jobId == "gp.100"
			|| jobId == "gp.101" || jobId == "chis.system") {
		cfg.actions = [{
					id : "save",
					name : "确定"
				}, {
	...
	chis.application.dbs.script.visit.DiabetesVisitMedicineForm.superclass.constructor
			.apply(this, [cfg]);
	...
	this.saveAction = "saveMedicine";
	// this.remoteDicService = "";
	// this.remoteDicAction = "";
};

Ext.extend(chis.application.dbs.script.visit.DiabetesVisitMedicineForm,
		chis.script.BizTableFormView, {
			onWinShow : function() {
				if (this.initDataId) {
					var data = this
							.castListDataToForm(this.r.data, this.schema)
			...
			doCreateMedicine : function() {
				...
				this.data["visitId"] = this.exContext.args.r.get("visitId");
			},
			saveToServer : function(saveData) {
				saveData.phrId = this.exContext.ids.phrId
				saveData.visitId = this.exContext.args.r.get("visitId")
				chis.application.dbs.script.visit.DiabetesVisitMedicineForm.superclass.saveToServer
						.call(this, saveData)
			},
			...
			onReady : function() {
				chis.application.dbs.script.visit.DiabetesVisitMedicineForm.superclass.onReady
						.call(this)
