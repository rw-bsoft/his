...
chis.application.cvd.script.DiseaseOmissionList = function(cfg) {
	cfg.selectFirst = false;
	this.initCnd = cfg.cnds || ['and',["eq", ["$", "c.manaUnitId"], ["s",cfg.mainApp.deptId]],["eq", ["$", "c.manaDoctorId"], ["s", cfg.mainApp.uid]]];
	chis.application.cvd.script.DiseaseOmissionList.superclass.constructor
			.apply(this, [cfg]);
}
Ext.extend(chis.application.cvd.script.DiseaseOmissionList,
		chis.script.BizSimpleListView, {
		loadData : function() {
			chis.application.cvd.script.DiseaseOmissionList.superclass.loadData
					.call(this);
		...
		getPagingToolbar : function(store) {
				...
				var comb = util.dictionary.SimpleDicFactory.createDic({
							id : "chis.dictionary.reportStatu",
							forceSelection : true,
							defaultValue : {
								key : "0",
								text : "全部"
			...
			radioChanged : function(r) {
				...
				if(status!=0){
					statusCnd=['and',["eq", ["$", "a.sfbk"], ["i",status]],["eq", ["$", "c.manaUnitId"], ["s",this.mainApp.deptId]],["eq", ["$", "c.manaDoctorId"], ["s", this.mainApp.uid]]];
				}
...
//				}
			...
			showModule : function(empiId, recordId) {
				debugger;
			...
			doModify : function(item, e) {
				...
				this.showModule(r.get("empiId"), r.get("recordId"));
			},
