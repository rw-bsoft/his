...
$import("app.modules.common", "chis.script.BizModule",
		"util.dictionary.TreeDicFactory")

chis.application.pub.script.ConfigChineseSymptomModule = function(cfg) {
	...
	chis.application.pub.script.ConfigChineseSymptomModule.superclass.constructor
			.apply(this, [cfg])
}

Ext.extend(chis.application.pub.script.ConfigChineseSymptomModule,
		chis.script.BizModule, {
			...
			getList : function() {
				this.list = this.createCombinedModule("list", this.refList);
				this.list.oper = this;
				this.list.grid = this.list.initPanel();
				this.list.on("select", this.recordsSelect, this)
				return this.list.grid;
			},
			recordsSelect : function(records) {
				this.fireEvent("select", records, "3", this)
			},
			loadData : function() {
				this.list.loadData();
			},
