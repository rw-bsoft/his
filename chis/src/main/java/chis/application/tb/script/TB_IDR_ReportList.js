...
chis.application.tb.script.TB_IDR_ReportList = function(cfg) {
	this.initCnd = ["eq", ["$", "a.status"], ["s", "0"]]
	chis.application.tb.script.TB_IDR_ReportList.superclass.constructor.apply(
			this, [cfg]);
}

Ext.extend(chis.application.tb.script.TB_IDR_ReportList,
		chis.script.BizSimpleListView, {
			...
			showEHRViewMule : function() {
				...
				var module = this.midiModules["TB_IDR_ReportListView_EHRView"];
				if (!module) {
					$import('chis.script.EHRView');
					module = new chis.script.EHRView(cfg);
					this.midiModules["TB_IDR_ReportListView_EHRView"] = module
				...
				module.exContext.args.recordId = this.initDataId;
