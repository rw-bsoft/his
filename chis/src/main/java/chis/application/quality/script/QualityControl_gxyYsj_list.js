...
chis.application.quality.script.QualityControl_gxyYsj_list = function(cfg) {
	this.autoLoadData = true
	this.xmxhId="";
	cfg.initCnd="";
	chis.application.quality.script.QualityControl_gxyYsj_list.superclass.constructor.apply(
			this, [cfg]);
	this.height = 270;
	// this.disablePagingTbr = true
}
Ext.extend(chis.application.quality.script.QualityControl_gxyYsj_list,
		chis.script.BizSimpleListView, {
         onReady: function(){
 			chis.application.quality.script.QualityControl_gxyYsj_list.superclass.onReady.call(this);
...
//				this.doNewSet();
		  ...
		  getCndBar : function(items){
			...
			doCndQuery : function(button, e, addNavCnd) {
				...
				if(ZKZB && ZKZB !='09'){
					hcnd = ['eq',['$','a.hypertensionGroup'],['s',ZKZB || '']]
					if(cnd.length > 0){
						cnd = ['and', cnd, hcnd];
					}else{
						cnd = hcnd;
