...
chis.application.hy.script.first.HypertensionFirstForm = function(cfg) {
	...
	chis.application.hy.script.first.HypertensionFirstForm.superclass.constructor.apply(this,
			[cfg]);
};
Ext.extend(chis.application.hy.script.first.HypertensionFirstForm, chis.script.BizModule, {
	...
	checkSave : function() {
		...
		var con = form.findField("__con").getValue();
		...
		var dia = form.findField("__dia").getValue();
	...
	doSave : function() {
		...
		requestData["diagnosisType"] = group2.getValue();
		requestData["postHypertension"] = group1.getValue();
