...
chis.application.hy.script.visit.HypertensionVisitBaseInfoFormHtml_ly = function(
		cfg) {
	chis.application.hy.script.visit.HypertensionVisitBaseInfoFormHtml_ly.superclass.constructor
			.apply(this, [cfg]);
	...
	if(this.area=="ly"){
	Ext.apply(this,chis.application.hy.script.visit.HypertensionVisitBaseInfoHtmlTemplate_ly);
	}else {
	...
	this.createFields = ["visitDate", "nextDate", "visitDoctor", "drugNames1",
...
Ext.extend(chis.application.hy.script.visit.HypertensionVisitBaseInfoFormHtml_ly,
...
//				}
				...
				if (result.json.body) {
					...
					if (groupAlarm == 2 && this.planStatus != "9"
							&& !nonArrivalDate) {
					...
					if (data) {
						...
						this.setCheckBoxValues("complication",
								data.complication || '');
					} else {
					...
					if (data) {
						...
						this.setValueById("targetDrinkCount", targetDrinkCount
										|| '');
						this.setValueById("targetTrainMinute",
								targetTrainMinute);
				...
				this.setNextDate();
				this.onLoadData();
				var noVisitReasonDom = document.getElementById("noVisitReason"
						+ this.idPostfix);
				onVisitEffectChange(1, noVisitReasonDom);
				var otherSymptomsDom = document.getElementById("otherSymptoms"
						+ this.idPostfix);
				...
				util.rmi.jsonRequest({
							...
							if (body) {
								...
								if (visitInfo) {
									...
									var noVisitReasonDom = document
											.getElementById("noVisitReason"
													+ this.idPostfix);
									var visitWayDom = thishyPanel
											.getElementById("div_visitWay"
													+ this.idPostfix);
									var medicineDom = thishyPanel
											.getElementById("div_medicine"
													+ this.idPostfix);
									onVisitEffectChange(visitEffect.key,
											noVisitReasonDom, visitWayDom,
											medicineDom,
											visitInfo.noVisitReason);
									...
									if (medicineBadEffect) {
										var medicineBadEffectTextDom = document
												.getElementById("medicineBadEffectText"
														+ this.idPostfix);
										onMedicineBadEffectChange(
												medicineBadEffect.key,
												medicineBadEffectTextDom);
									}
									currentSymptoms = visitInfo.currentSymptoms;
									if (currentSymptoms) {
										var otherSymptomsDom = document
...
//					}
			...
			setMedicineDisabled : function(flag) {
				...
				for (var i = 1; i < 5; i++) {
					if (flag) {
						document.getElementById("everyDayTime" + i
								+ this.idPostfix).value = "";
						document.getElementById("oneDosage" + i
								+ this.idPostfix).value = "";
						document.getElementById("medicineUnit" + i
								+ this.idPostfix).value = "";
...
function onCurrentSymptomsClick(v, f, value) {
	if (!value) {
		...
		if (v == "9") {
			thishyPanel.clearCheckBoxValues(id);
			thishyPanel.setCheckBoxValues(id, "9")
			f.value = "";
			f.disabled = true;
			return;
