...
chis.application.mzf.script.MZFList = function (cfg) {
	this.needOwnerBar = true;
	chis.application.mzf.script.MZFList.superclass.constructor
        .apply(this, [cfg]);
    this.sex = {"1": "男", "2": "女", "9": "未说明的性别", "0": "未知的性别"}
}

Ext.extend(chis.application.mzf.script.MZFList,
    ...
    doModify: function() {   	
		...
		util.rmi.jsonRequest({
			serviceId : "chis.mZFVisitService",
			serviceAction : "getVisitMZFByPkey",
			method:"execute",
			body:{
				"visitId":r.id,
			}
	...
	doQuchong: function() {
		...
		var cnd = this.getOwnerCnd([]);
		this.requestData.cnd = cnd;
		this.listAction = "listMZFVisitPlanQC";
		this.listServiceId = "chis.mZFVisitService";
