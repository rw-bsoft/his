...
chis.application.def.script.brain.BrainDeformityRecordListView = function(cfg) {
	this.initCnd = cfg.cnds || ["eq", ["$", "a.status"], ["s", "0"]]
	this.entryName = "chis.application.def.schemas.DEF_BrainDeformityRecord"
	chis.application.def.script.brain.BrainDeformityRecordListView.superclass.constructor
			.apply(this, [cfg]);
}

Ext.extend(chis.application.def.script.brain.BrainDeformityRecordListView,
		chis.script.BizSimpleListView, {
			doCreateByEmpi : function(item, e) {
				var m = this.midiModules["DefBrain_EMPIInfoModule"];
				if (!m) {
					...
					this.midiModules["DefBrain_EMPIInfoModule"] = m;
				}
				var win = m.getWin();
			...
			onEmpiReturn : function(data) {
				...
				if (!this.checkAge(birthDay)) {
					Ext.Msg.alert("消息", "满14周岁不允许创建脑瘫儿童康复训练档案")
					return
				}
			...
			doRemove : function() {
				...
				var cfg = this.loadModuleCfg("chis.application.def.DEF/DEF/DEF02_1_1_7")
				cfg.title = "脑瘫残疾训练登记注销"
				cfg.id = r.get("id")
				...
				var module = this.midiModules["BrainDeformityRecordLogoutFormView"]
				if (!module) {
					$import("chis.application.def.script.brain.BrainDeformityRecordLogoutFormView")
					module = new chis.application.def.script.brain.BrainDeformityRecordLogoutFormView(cfg)
					module.on("writeOff", this.onWriteOff, this)
					this.midiModules["BrainDeformityRecordLogoutFormView"] = module
				} else {
			...
			showModule : function(empiId) {
				var module = this.midiModules["DefBrain_EHRView"]
				var cfg = {}
				cfg.initModules = ['DEF_02']
				cfg.empiId = empiId
				cfg.closeNav = true
				cfg.mainApp = this.mainApp
				cfg.exContext = this.exContext
				...
				if (!module) {
					...
					this.midiModules["DefBrain_EHRView"] = module
			...
			onStoreLoadData : function(store, records, ops) {
				chis.application.def.script.brain.BrainDeformityRecordListView.superclass.onStoreLoadData
