...
chis.application.hc.script.WProgestationCheckList = function(cfg) {
	cfg.autoLoadData = false;
	cfg.enableCnd = false;
	cfg.disablePagingTbr = true;// 分页工具栏 
	chis.application.hc.script.WProgestationCheckList.superclass.constructor
			.apply(this, [cfg]);
};

Ext.extend(chis.application.hc.script.WProgestationCheckList,
		chis.script.BizSimpleListView, {
			loadData : function() {
				this.initCnd = ['eq', ['$', 'a.empiId'],
						['s', this.exContext.ids.empiId]];
				this.requestData.cnd = this.initCnd;
				chis.application.hc.script.WProgestationCheckList.superclass.loadData
						.call(this);
			},
			doAdd : function(data) {
				if (!data) {
