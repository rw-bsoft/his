...
chis.application.def.script.intellect.IntellectTerminalEvaluateForm = function(cfg) {
	this.entryName = 'chis.application.def.schemas.DEF_IntellectTerminalEvaluate'
	chis.application.def.script.intellect.IntellectTerminalEvaluateForm.superclass.constructor.apply(this,
			[cfg])
	...
	this.saveAction = "saveIntellectTerminalEvaluate"
}
Ext.extend(chis.application.def.script.intellect.IntellectTerminalEvaluateForm,
		chis.script.BizTableFormView, {
			onReady : function() {
				chis.application.def.script.intellect.IntellectTerminalEvaluateForm.superclass.onReady
						.call(this)
			...
			doSave:function(){
				Ext.Msg.show({
					...
					fn : function(btn, text) {
						if(btn == 'yes'){
							chis.application.def.script.intellect.IntellectTerminalEvaluateForm.superclass.doSave.call(this)
						}
					},
					scope : this
				})
			...
			saveToServer : function(saveData) {
				...
				chis.application.def.script.intellect.IntellectTerminalEvaluateForm.superclass.saveToServer.call(this,saveData)
				if(this.win){
					this.win.hide()
				}
			},
			...
			loadData : function() {
				...
				if(this.result){
					...
					var result = util.rmi.miniJsonRequestSync({
							...
							serviceAction:"queryIntellectTerminalEvaluate",
							method:"execute",
							schema : "chis.application.def.schemas.DEF_IntellectTerminalEvaluate",
							cnd : ["eq", ["$", "defId"],
									["s", this.defId]]
						})
				if (result.code > 300) {
				...
				this.form.el.unmask()
//				chis.application.def.script.intellect.IntellectTerminalEvaluateForm.superclass.loadData.call(this)
			}
			,
