...
chis.application.quality.script.QualityControl_gxyZk_list = function(cfg) {
	chis.application.quality.script.QualityControl_gxyZk_list.superclass.constructor.apply(
			this, [cfg]);
}
Ext.extend(chis.application.quality.script.QualityControl_gxyZk_list,
		chis.script.BizSimpleListView, {
     onReady: function(){
 	 chis.application.quality.script.QualityControl_gxyZk_list.superclass.onReady.call(this);
         this.actionAdd=[
		  ...
		  var result = util.rmi.miniJsonRequestSync({
			...
			if (result.code > 300) {
				...
				if(back["CODERNO"]){
					...
					var name=r.get("personName");
				  MyMessageTip.msg("提示", "请先录入 "+name+" 质控数据！",true);
				}
			}
		}
	},doLrzksj:function(){
		...
		if(r){
			  var values={};
		         ...
		         var result = util.rmi.miniJsonRequestSync({
						...
						serviceAction : "creatGxyZkPf",//getQualityModel  loadHtmlData
						method : "execute",
						body : {
							"values" :values
						  }
