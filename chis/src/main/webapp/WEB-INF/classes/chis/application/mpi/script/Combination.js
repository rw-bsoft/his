...
chis.application.mpi.script.Combination = function(cfg) {
	...
	cfg.height = 450;
	cfg.enableCnd = false;
	cfg.initCnd = ['and', ['eq', ['$', 'personName'], ['s', '王守义']],
			['eq', ['$', 'sexCode'], ['s', '2']]];
	...
	chis.application.mpi.script.Combination.superclass.constructor.apply(this, [cfg]);
}

Ext.extend(chis.application.mpi.script.Combination, app.modules.list.SelectListView, {
	...
	selectBaseRecord : function() {
		...
		var swin = this.selectWin;
		if (!swin) {
			swin = new chis.application.mpi.script.CombinationSelect({
			...
			swin.on("onSelect", function(record) {
					}, this);
			this.selectWin = swin;
		}
		swin.setRecords(records);
		swin.getWin().show()
	}
})
