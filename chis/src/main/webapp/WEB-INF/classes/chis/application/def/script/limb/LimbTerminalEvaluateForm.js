...
chis.application.def.script.limb.LimbTerminalEvaluateForm = function(cfg) {
	this.entryName = 'chis.application.def.schemas.DEF_LimbTerminalEvaluate'
	chis.application.def.script.limb.LimbTerminalEvaluateForm.superclass.constructor.apply(this,
			[cfg])
	...
	this.saveAction = "saveLimbTerminalEvaluate"
}
Ext.extend(chis.application.def.script.limb.LimbTerminalEvaluateForm,
		chis.script.BizTableFormView, {
			onReady : function() {
				chis.application.def.script.limb.LimbTerminalEvaluateForm.superclass.onReady
						.call(this)
			...
			doSave:function(){
				Ext.Msg.show({
					...
					fn : function(btn, text) {
						if(btn == 'yes'){
							chis.application.def.script.limb.LimbTerminalEvaluateForm.superclass.doSave.call(this)
						}
					},
					scope : this
				})
			...
			saveToServer : function(saveData) {
				...
				chis.application.def.script.limb.LimbTerminalEvaluateForm.superclass.saveToServer.call(this,saveData)
				if(this.win){
					this.win.hide()
				}
			},
			...
			loadData : function() {
				...
				if(this.result){
					...
					var result = util.rmi.miniJsonRequestSync({
							...
							serviceAction:"queryLimbTerminalEvaluate",
							method:"execute",
							schema : "chis.application.def.schemas.DEF_LimbTerminalEvaluate",
							cnd : ["eq", ["$", "defId"],
									["s", this.defId]]
						})
				if (result.code > 300) {
				...
				this.form.el.unmask()
//				chis.application.def.script.limb.LimbTerminalEvaluateForm.superclass.loadData.call(this)
			}
			,
