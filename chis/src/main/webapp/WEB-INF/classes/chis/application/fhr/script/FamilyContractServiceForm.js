...
chis.application.fhr.script.FamilyContractServiceForm = function(cfg) {
	...
	chis.application.fhr.script.FamilyContractServiceForm.superclass.constructor
			.apply(this, [cfg]);
}

Ext.extend(chis.application.fhr.script.FamilyContractServiceForm,
...
//				var otherKind = this.FS_Kind.substring(this.FS_Kind
//						.indexOf(":")
//						+ 1);
//				var unitId = null;
//				if (this.mainApp.deptId.length >= 8) {
//					unitId = this.mainApp.deptId.substring(0, 8)
//				}
//				var otherServiceSelect = this.form.getForm()
//						.findField("otherServiceSelect");
//				otherServiceSelect.store.on("load", function() {
//							otherServiceSelect.setValue(otherKind);
//						});
//				otherServiceSelect.store.removeAll();
//				otherServiceSelect.store.proxy = new Ext.data.HttpProxy({
//					method : "GET",
//					url : util.dictionary.SimpleDicFactory.getUrl({
//						id : "chis.dictionary.otherServiceSelect",
//						filter : "['eq',['$','item.properties.createUnit'],['s','"
//								+ this.mainApp.deptId + "']]"
//					})
//				})
//				otherServiceSelect.store.load();
				// alert(this.record.get("FS_PersonGroup"))
				// alert(this.record.get("FS_Kind"))
				// this.queryFRNote();
			},
			...
			setNotKeyPersonStatus : function(checkbox, checked) {
				if (checked == true) {
					if (checkbox.getName() == "oldPerson") {
						...
						this.formFields["notKeyPerson"].setValue(false);
						// if (checkbox.getName() == "oldPerson") {
						// this.disease = this.olddisease;
						// }
					} else if (checkbox.getName() == "otherService") {
						this.formFields["otherServiceSelect"].enable()
					} else if (checkbox.getName() == "hypertensionPerson" 
							|| checkbox.getName() == "diabetesPerson"
							|| checkbox.getName() == "phthisicPerson"
							|| checkbox.getName() == "mentalDisease"
							|| checkbox.getName() == "disabledPerson"
							|| checkbox.getName() == "villagePoorPerson"
							|| checkbox.getName() == "fiveGuarantees"
							|| checkbox.getName() == "lowSecurity"
							|| checkbox.getName() == "specialCare"
							|| checkbox.getName() == "planningSpecialFamily"
							|| checkbox.getName() == "threeNo"
							) {
						this.formFields["notKeyPerson"].setValue(false);
					}
				} else {
					if (checkbox.getName() == "otherService") {
						...
						// } else if (checkbox.getName() ==
						// "slowDisease") {
						// this.disease = this.disease.replace("高血压",
						// "").replace(
						// "糖尿病", "").replace("脑卒中", "")
						// .replace("冠心病", "")
					}
			...
			onReady : function() {
				chis.application.fhr.script.FamilyContractServiceForm.superclass.onReady
						.call(this);
			...
			saveToServer : function(saveData) {
				...
				for (var i = 0; i < n; i++) {
					var it = items[i];
					if (saveData[it.id]) {
						if (it.group == "人群分类"||it.group == "重点人群分类"||it.group == "特殊人群分类") {
