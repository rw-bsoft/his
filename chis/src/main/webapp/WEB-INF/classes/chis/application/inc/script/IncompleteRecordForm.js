...
chis.application.inc.script.IncompleteRecordForm = function(cfg) {
	...
	cfg.labelWidth = 80;
	cfg.width = 715;
	this.queryServiceId = "chis.incompleteRecordService";
	chis.application.inc.script.IncompleteRecordForm.superclass.constructor
			.apply(this, [cfg])
...
Ext.extend(chis.application.inc.script.IncompleteRecordForm,
		chis.script.BizTableFormView, {
			...
			doCreate : function() {
				...
				this.empiId = "";
				this.phrId = "";
				chis.application.inc.script.IncompleteRecordForm.superclass.doCreate
						.call(this);
			...
			checkupPhrId : function() {
				...
				util.rmi.jsonRequest({
							...
							if (json.haveRecord) {
								...
								this.manaDoctorId = json.manaDoctorId;
			...
			onWinShow : function() {
				...
				var task = new Ext.util.DelayedTask(function() {
					if (this.initDataId) {
						var listEntryName = "chis.application.inc.schemas.INC_IncompleteRecord";
						...
						this.manaDoctorId = "";
						this.setButton(["save"], false);
						var clinicalReceptionDate = this.form.getForm()
								.findField("clinicalReceptionDate");
						clinicalReceptionDate.setValue(this.mainApp.serverDate);
					}
				}, this);
				task.delay(500);
			},
			...
			onBeforePrint : function(type, pages, ids_str) {
				...
				pages.value = "chis.prints.template.admit";
				ids_str.value = "&recordId=" + this.initDataId;
			},

			doGetYLData : function() {
				this.loadYLData();
			},
			...
			loadYLData : function() {
				util.rmi.jsonRequest({
							...
							if (json.body) {
								this.selectYLData(json.body);
							}
						}, this)
			},

			selectYLData : function(data) {
				var records = [];
				for (var i = 0; i < data.length; i++) {
					var r = data[i];
					var record = new Ext.data.Record(r);
				...
				var YLDataSelectView = this.midiModules["YLDataSelectView2"];
				if (!YLDataSelectView) {
					var YLDataSelectView = new chis.application.mpi.script.CombinationSelect(
							{
								entryName : "chis.application.inc.schemas.INC_IncompleteRecord_SelectList",
								autoLoadData : false,
								enableCnd : false,
								modal : true,
								title : "选择接诊记录",
					...
					this.midiModules["YLDataSelectView2"] = YLDataSelectView;
					YLDataSelectView.on("onSelect", function(r) {
								this.initFormData(r.data);
							}, this);
				}
				YLDataSelectView.getWin().show();
				var task = new Ext.util.DelayedTask(function() {
							YLDataSelectView.setRecords(records);
						}, this);
				task.delay(300);
			}
