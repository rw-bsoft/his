...
chis.application.mpm.script.MasterplateMaintainForm = function(cfg) {
	...
	chis.application.mpm.script.MasterplateMaintainForm.superclass.constructor.apply(this, [cfg]);
	this.saveServiceId="chis.templateService";
	this.saveAction="saveMasterplateMaintain";
}

Ext.extend(chis.application.mpm.script.MasterplateMaintainForm, chis.script.BizTableFormView, {
	doCancel : function() {
	...
	getWhmbValue : function(){
		var frm = this.form.getForm();
		var whmbFld = frm.findField("whmb");
		var whmb = "";
		if(whmbFld){
			whmb = whmbFld.getValue(); 
		}
		return whmb;
	}
});