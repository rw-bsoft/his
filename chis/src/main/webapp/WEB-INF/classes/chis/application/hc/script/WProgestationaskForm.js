...
chis.application.hc.script.WProgestationaskForm = function(cfg) {
	cfg.colCount = 3;
	cfg.labelWidth = 90;
	...
	chis.application.hc.script.WProgestationaskForm.superclass.constructor
			.apply(this, [cfg]);
	this.saveServiceId = "chis.healthCheckService";
	this.saveAction = "saveWProgestationaskrecord";
	this.entryName = "chis.application.hc.schemas.hc_w_progestationask";
	this.on("save", this.onSave, this);
};

Ext.extend(chis.application.hc.script.WProgestationaskForm,
		chis.script.BizTableFormView, {
			doNew : function() {
				this.initDataId = this.exContext.ids["hc_w_progestationask.phrId"];
				chis.application.hc.script.WProgestationaskForm.superclass.doNew
						.call(this);
		...
		initFormData : function(data) {
				this.exContext.control = data["hc_w_progestationask_actions"];
				if (this.op == "create") {
					this.initDataId = null;
				}
				chis.application.hc.script.WProgestationaskForm.superclass.initFormData
						.call(this, data);
				var phrIdField = this.form.getForm().findField("phrId");
				if (!phrIdField || !phrIdField.getValue()) {
					phrIdField.setValue(this.exContext.ids.phrId);
			...
			onReady : function() {
				chis.application.hc.script.WProgestationaskForm.superclass.onReady
						.call(this);
				this.nowDate = Date.parseDate(this.mainApp.serverDate, "Y-m-d");
				var form = this.form.getForm();
				var zfsfzh = form.findField("ZFSFZH");//丈夫身份证号
				zfsfzh.on("blur", this.onzfsfzhChange, this);
				
			},
			onzfsfzhChange:function(field){
				if (!field.validate()) {
					return;
				}
				...
				var zysfzh=this.checkIdcard(sfzh)
				if(sfzh!=zysfzh){
