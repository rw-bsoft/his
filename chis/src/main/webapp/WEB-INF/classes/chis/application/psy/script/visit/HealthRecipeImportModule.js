...
$import("chis.script.BizModule", "chis.script.util.widgets.MyMessageTip");

chis.application.psy.script.visit.HealthRecipeImportModule = function(cfg) {
	chis.application.psy.script.visit.HealthRecipeImportModule.superclass.constructor
			.apply(this, [cfg]);
	this.on("winShow", this.onWinShow, this);
	this.overIndex = 0;
}
Ext.extend(chis.application.psy.script.visit.HealthRecipeImportModule,
		chis.script.BizModule, {
			...
			getRecipeHasList : function() {
				this.RecipeHasList = this
						.createCombinedModule("refRecipeHasList" + this.fromId,
								this.refRecipeHasList);
			...
			getRecipeList : function() {
				this.RecipeList = this.createCombinedModule("RecipeList"
								+ this.fromId, this.refRecipeList);
			...
			onListLoadData : function(store) {
				...
				if (this.winShow) {
					this.JKCFRecordsCache = [];
					this.RecipeHasList.store.each(function(r) {
								this.JKCFRecordsCache.push(r);
							}, this);
				} else if (this.JKCFRecordsCache
						&& this.JKCFRecordsCache.length > 0) {
					for (var i = 0; i < this.JKCFRecordsCache.length; i++) {
						this.RecipeHasList.store.add(this.JKCFRecordsCache[i]);
					}
				}
				this.winShow = false;
			},
