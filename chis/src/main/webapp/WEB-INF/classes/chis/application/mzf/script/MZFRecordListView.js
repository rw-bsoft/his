...
chis.application.mzf.script.MZFRecordListView = function(cfg) {
	this.initCnd = cfg.cnds || ["eq", ["$", "a.status"], ["s", "0"]];
	//this.needOwnerBar=true;
	chis.application.mzf.script.MZFRecordListView.superclass.constructor
			.apply(this, [cfg]);
	this.businessType="2";
}

Ext.extend(chis.application.mzf.script.MZFRecordListView,
		chis.script.BizSimpleListView, {
			...
			ndsfradiocnd:function(status){
				...
				if(status==1){
					ndsfCnd= ['in', ['$', 'a.empiId'], 
				             ["select dia.empiId from MDC_MZFVisit dia where a.empiId=dia.empiId and to_char(dia.visitDate,'yyyy')='"+this.yearField.getValue()+"'"]];
				}else
				if(status==0){
			...
			doCreateByEmpi : function() {
				var advancedSearchView = this.midiModules["MZFRecordListView_EMPIInfoModule"];
				if (!advancedSearchView) {
					...
					advancedSearchView.on("onEmpiReturn", this.onEmpiSelected,
							this)
					this.midiModules["MZFRecordListView_EMPIInfoModule"] = advancedSearchView;
				}
			...
			showEhrViewWin : function() {
				...
				var cfg = {};
				cfg.closeNav = true;
				cfg.initModules = ['MZF_00',"MZF_01"]
				cfg.mainApp = this.mainApp;
				cfg.activeTab = this.activeTab;
				cfg.needInitFirstPanel = true
				var module = this.midiModules["MZFRecordListView_EHRView"];
				if (!module) {
					module = new chis.script.EHRView(cfg);
					this.midiModules["MZFRecordListView_EHRView"] = module;
...
//				chis.application.mzf.script.MZFRecordListView.superclass.onStoreLoadData
...
//			},
			...
			doWriteOff : function() {
				...
				cfg.record = r
				cfg.mainApp = this.mainApp
				var module = this.midiModules["MZFRecordLogoutFormView"];
				if (!module) {
					module = new chis.application.mzf.script.MZFRecordLogoutFormView(cfg);
					module.on("writeOff", this.onWriteOff, this);
					this.midiModules["MZFRecordLogoutFormView"] = module;
				} else {
					Ext.apply(module, cfg)
				}
				module.getWin().show();
			...
			doDelete : function() {
				...
				if (!deletemdc) {
					deletemdc = new chis.application.mzf.script.MZFRecordDelete(
							{
								record : record,
								mainApp : this.mainApp
							});
			...
			doLook : function(){
				Ext.Msg.alert("友情提醒：","慢阻肺分组已修改为分组评估生成随访模式，如没有随访计划请先评估，再去随访页面点击【本年度】刷新随访！");
			}
		});