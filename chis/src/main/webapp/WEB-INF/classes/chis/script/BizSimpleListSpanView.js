...
$styleSheet("chis.resources.ext.ux.grid.gridRowOrColspan")

chis.script.BizSimpleListSpanView = function(cfg){
	cfg.isFirstRowShowMergeCells = cfg.isFirstRowShowMergeCells || false;
	chis.script.BizSimpleListSpanView.superclass.constructor.apply(this, [ cfg ]);
}

Ext.extend(chis.script.BizSimpleListSpanView,chis.script.BizSimpleListView,{
	/*
	 * *合并单元格的函数，合并表格内所有连续的具有相同值的单元格。调用方法示例：
	 * this.on("loadData",this.onLoadData,this);
	 * onLoadData : function(store){
		this.gridSpan(this.grid,"row","[source_text],[createUnit_text]","source_text")
		},
	 * 参数：grid-需要合并的表格,rowOrCol-合并行还是列,cols-需要合并的列（行合并的时候有效）,
	 * sepCols以哪个列为分割(即此字段不合并的2行，其他字段也不许合并)，默认为空
	 */
	gridSpan : function(grid, rowOrCol, cols, sepCol) {
		...
		for (i = 0; i < count1; i++) {
			...
			for (j = 0; j < count2; j++) {
				...
				if (allRecs[index1].get(colName) == preValue
					...
					if (j == count2 - 1) {
						var index = 0;
						if(this.isFirstRowShowMergeCells){
							index = firstSameCell;
						}else{
							index = firstSameCell
