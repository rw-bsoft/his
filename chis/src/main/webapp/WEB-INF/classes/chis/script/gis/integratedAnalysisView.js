...
chis.script.gis.integratedAnalysisView = function(cfg) {
	var def = {
		width : 620,
		westWidth : 200,
	...
	Ext.apply(this, def)
	chis.script.gis.integratedAnalysisView.superclass.constructor
			.apply(this, [cfg])
}

Ext.extend(chis.script.gis.integratedAnalysisView, app.desktop.Module, {
	initPanel : function() {
		if (this.panel) {
			return this.panel
		}
	...
	createTabPanel : function() {
		...
		var tabitem = [{
			...
			title : "统计图",
			exCfg : {
				script : "app.modules.chart.ChartViewPortal"
			}
		}		// , {
		// viewType : "gis",
		// layout : "fit",
		...
		// listServiceId:"chis.simpleReport",
		// entryName : "MHC/year"
		// }
		// }
		// {viewType:"gis",layout:"fit",title:"人口金字塔图",exCfg:{script:"app.biz.demographicView",entryName:this.entryName}}
		]
	...
	onTreeItemClick : function(node, e) {
		...
		if (node.leaf) {
			...
			params.push({
						...
						title : encodeURIComponent(encodeURIComponent(node.attributes.title.replace('+','%2B'))),
