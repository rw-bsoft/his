...
chis.script.CardReader = {
	...
	init:function(){
		...
		if(!chis.script.CardReader.agent){
			try{
				//var agent =  new ActiveXObject("ICCINTERACTIVEXLib.ICCInterCtl")
//				alert("init")
				chis.script.CardReader.agent = Ext.getDom('ICCInterCtl')
				//chis.script.CardReader.agent = ICCInterCtl
			}
			catch(e){
				alert("读卡器控件初始化失败:" + e)
			}
	...
	wrapMessage:function(ret,str){
		var n = str.length
		var fields = chis.script.CardReader.fields
		var p = n;
		fields[7].end = p
		p = p - 6
		fields[7].start = p
...
if(Ext.isIE){
	chis.script.CardReader.readCardInfo = function(){
		var agent = chis.script.CardReader.agent
		var ret = {
			code:0
		}
		if(!agent){
...
chis.script.CardReader.statusText = {
	0:"正常",
	1:"卡片类型不正确",
	2:"卡尚未插入",
	3:"卡尚未取出",
