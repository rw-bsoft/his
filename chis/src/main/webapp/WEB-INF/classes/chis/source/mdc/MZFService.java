 ...
 */
public class MZFService extends MDCService {
	private static final Logger logger = LoggerFactory
			.getLogger(MZFService.class);
	 ...
	 * 糖尿病信息删除
	 * */
	public void doDeleteDiabetesRecord(Map<String, Object> req,
			Map<String, Object> res, BaseDAO dao, Context ctx)
			throws ServiceException {
		Map<String, Object> body = (Map<String, Object>) req.get("body");
		...
		try {
			...
			p.put("phrId", phrId);
			//删除糖尿病服药记录
			String deletemedicinesql="delete from MDC_DiabetesMedicine where phrId=:phrId ";
		    dao.doUpdate(deletemedicinesql, p);
		    //删除随访记录
		    String deletevisitsql="delete from MDC_DiabetesVisit where phrId=:phrId ";
		    dao.doUpdate(deletevisitsql, p);
		    //删除分组评估
		    String deletegroupsql="delete from MDC_DiabetesFixGroup where phrId=:phrId ";
		    dao.doUpdate(deletegroupsql, p);
		    //删除糖尿病档案
		    String deleterecordsql="delete from MDC_DiabetesRecord where phrId=:phrId ";
		    dao.doUpdate(deleterecordsql, p);
		    //删除随访计划
		    p.put("businessType", "2");
