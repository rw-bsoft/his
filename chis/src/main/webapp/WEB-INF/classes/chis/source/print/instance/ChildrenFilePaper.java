 ...
 */
public class ChildrenFilePaper extends BSCHISPrint implements IHandler {
	...
	private void doChange(Map<String, Object> response,int type) throws PrintException,
			ServiceException {
		for (String key : response.keySet()) {
			if(key.startsWith("weightDevelopment_text")||key.startsWith("heightDevelopment_text"))
			{
				if(response.get(key)!=null)
				{
					String value=(String)response.get(key);
					if(value.equals("上")){
						response.put(key,"上√ 中  下");
		...
		int ss[][]={{},{1,3,6,9},{12,18,24,30},{36,48,60,72}};		
		int s[]=ss[type];

		if(type==1)//一岁以内
		{
				if(response.get("face1")!=null&&response.get("face1").equals("9"))
					response.put("face1","3");
				if(response.get("face3")!=null&&response.get("face3").equals("9"))
					response.put("face3","3");
				
				if(response.get("face6")!=null&&response.get("face6").equals("9"))
					response.put("face6","2");
				if(response.get("face9")!=null&&response.get("face9").equals("9"))
					response.put("face9","2");
		...
		{
		if(response.get("weightDevelopment_text"+s[i])==null)
		{
			response.put("weightDevelopment_text"+s[i],"上  中  下");	
		}
		if(response.get("heightDevelopment_text"+s[i])==null)
		{
			response.put("heightDevelopment_text"+s[i],"上  中  下");	
		}
		if(response.get("face"+s[i])!=null&&response.get("face"+s[i]).equals("9"))
			response.put("face"+s[i],"3");
		...
		if(response.get("guide"+s[i])!=null)
		{
			...
			{
				if(guideXs[k].equals("5"))
				{
					response.put("guide"+s[i]+"X6","√");					
				}
				else if(guideXs[k].equals("6"))
				{
					response.put("guide"+s[i]+"X5","√");					
		...
		if(type==3)//3-6岁
		{
			if(response.get("illnessType"+s[i])!=null)
			{
				String illnessX=(String)response.get("illnessType"+s[i]);
				String illnessXs[]=illnessX.split(",");
				
				for(int k=0;k<illnessXs.length;k++)
				{
						response.put("illness"+s[i]+"X"+illnessXs[k],"√");	
				}
	...
	private Map<String, Object> getZd(String schema) throws PrintException,
			ServiceException {
		Map<String, Object> parameters = new HashMap<String, Object>();
		try {
			Class bs = BSCHISEntryNames.class;
			Field f = bs.getField(schema);
			String entity = (String) f.get(bs);
			Schema sc = SchemaController.instance().get(entity);
			List<SchemaItem> sl = sc.getItems();
			for (int i = 0; i < sl.size(); i++) {
				SchemaItem si = sl.get(i);
				if (si.isCodedValue()) {
	...
	private Map<String, Object> getThreeMap(String phrId,
		...
		this.getCheckupMapValue(exp, parameters,
				BSCHISEntryNames.CDH_CheckupThreeToSixPaper);
	...
	public static void main(String[] args) {
		Class bs = BSCHISEntryNames.class;
		try {
			Field f = bs.getField("CDH_CheckupInOne");
			System.out.println(f.get(bs));
		} catch (SecurityException e) {
			e.printStackTrace();
		} catch (NoSuchFieldException e) {
			e.printStackTrace();
