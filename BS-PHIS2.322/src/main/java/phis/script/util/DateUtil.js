...
// 得到几天后的时间
Date.getDateAfter = function(date, day) {
	var body = {
		"date" : date,
		"day" : day
	}
	var r = phis.script.rmi.miniJsonRequestSync({
				serviceId : "publicService",
				serviceAction : "getDateAfter",
				body : body
			});
	return r.json.date;
}
// 得到几天后的时间 带时分秒  zhaojian 2017-11-14
Date.getDateTimeAfter = function(day) {
	var body = {
		"day" : day
	}
	var r = phis.script.rmi.miniJsonRequestSync({
				serviceId : "publicService",
				serviceAction : "getDateTimeAfter",
				body : body
			});
	return r.json.date;
}
...
Date.getDatedif_days = function(sDate1, sDate2) {    //sDate1和sDate2是2006-12-18格式  
        var dateSpan,
            tempDate,
            iDays;
