...
(function() {
	...
	var isArray = objType('Array'), isObj = objType('Object'); // 判断元素是否数组、object
	window.ymPrompt = {
		version : '4.0',
		pubDate : '2009-03-02',
	...
	while (t = initFn.shift())
		ymPrompt[t] = eval('0,function(){_initFn.' + t + '?_initFn.' + t
				+ '.push(arguments):(_initFn.' + t + '=[arguments])}');
	/* 以下为公用函数及变量 */
	var isIE = !+'\v1'; // IE浏览器
	var isCompat = document.compatMode == 'CSS1Compat'; // 浏览器当前解释模式
	var IE6 = isIE && /MSIE (\d)\./.test(navigator.userAgent)
			&& parseInt(RegExp.$1) < 7; // IE6以下需要用iframe来遮罩
	...
	var addEvent = (function() {
		return new Function('env', 'fn', 'obj', 'obj=obj||document;'
						+ (window.attachEvent
								? "obj.attachEvent('on'+env,fn)"
								: 'obj.addEventListener(env,fn,false)')
						+ ';ymPrompt.eventList.push([env,fn,obj])')
	})(); // 事件绑定
	var detachEvent = (function() {
		return new Function('env', 'fn', 'obj', 'obj=obj||document;'
						+ (window.attachEvent
								? "obj.detachEvent('on'+env,fn)"
								: 'obj.removeEventListener(env,fn,false)'))
	})(); // 取消事件绑定
	...
	var mkBtn = function(txt, sign, autoClose, id) {
		...
		if (isArray(txt)) {
			...
			while (txt.length)
				(item = txt.shift())
						&& t[t.push(mkBtn.apply(null, dftBtn[item] || item))
								- 1] || t.pop();
			return t;
		}
		...
		return {
			...
			html : "<input type='button' id='"
					+ id
					+ "' onclick='ymPrompt.doHandler(\""
					+ sign
					+ "\","
					+ autoClose
					+ ")' style='cursor:pointer' class='btnStyle handler' value='"
					+ txt + "' />"
		};
	};
	/* 生成按钮组合的html */
	...
	(function() {
		...
		if (!rootEl || typeof rootEl != 'object')
			return addEvent('load', callee, window); // 等待页面加载完成
		/* 防止在IE下因document未就绪而报“IE无法打开INTERNET站点的错”的错 */
		if (isIE && document.readyState != 'complete')
			return addEvent('readystatechange', function() {
		...
		if (!isIE && frameset)
			return; // frameset页面且不是IE则直接返回，否则会出现错误。
		/* 获取scrollLeft和scrollTop，在fixed定位时返回0，0 */
		var getScrollPos = function() {
			return curCfg.fixPosition && useFixed ? [0, 0] : [
					rootEl.scrollLeft, rootEl.scrollTop];
		}
		...
		div.innerHTML = [
				/* 遮罩 */
				"<div id='maskLevel' style=\'" + maskStyle
						+ ';z-index:10000;\'></div>',
				IE6
						? ("<iframe id='maskIframe' src='javascript:false' style='"
								+ maskStyle + ";z-index:9999;filter:alpha(opacity=0);opacity:0'></iframe>")
						: '',
				/* 窗体 */
				...
				IE6
						...
						: '',
				"<div class='ym-tl' id='ym-tl'><div class='ym-tr'><div class='ym-tc' style='cursor:move;'><div class='ym-header-text'></div><div class='ym-header-tools'>",
				...
				"</div></div></div></div>",
				"<div class='ym-ml' id='ym-ml'><div class='ym-mr'><div class='ym-mc'><div class='ym-body' style='position:relative'></div></div></div></div>",
				...
				/* 阴影 */
				isIE
						? "<div id='ym-shadow' style='position:absolute;z-index:10000;background:#808080;filter:alpha(opacity=80) progid:DXImageTransform.Microsoft.Blur(pixelradius=2);display:none'></div>"
						: ''].join('');
		...
		var cal_cord = function() {
			var pos = getScrollPos();
			cur_cord = [parseInt(ym_win.style.left) - pos[0],
					parseInt(ym_win.style.top) - pos[1]]
		}; // 保存坐标(相对页面左上角坐标)
		/* 从常态到最大化 */
		var doMax = function() {
		...
		addEvent('dblclick', function(e) {
					...
					curCfg.maxBtn
							&& (e.srcElement || e.target).parentNode != ym_hTool
							&& max()
				}, ym_head);
		...
		var getWinSize = function() {
			return [Math.max(rootEl.scrollWidth, rootEl.clientWidth),
					Math.max(rootEl.scrollHeight, rootEl.clientHeight)]
		};
		...
		var mEvent = function(e) {
			...
			if (!curCfg.dragOut) { // 页面可见区域内拖动
				...
				sLeft = Math.min(Math.max(sLeft, sl), rootEl.clientWidth
								- ym_win.offsetWidth + sl);
				sTop = Math.min(Math.max(sTop, st), rootEl.clientHeight
								- ym_win.offsetHeight + st);
		...
		var uEvent = function() {
			...
			/* IE下窗口外部拖动 */
			bindEl
					&& (detachEvent("losecapture", uEvent, bindEl), bindEl
							.releaseCapture());
		};
		addEvent('mousedown', function(e) {
					if ((e.srcElement || e.target).parentNode == ym_hTool)
						return false; // 点击操作按钮不进行启用拖动处理
					...
					/* IE下窗口外部拖动 */
					bindEl
							&& (addEvent("losecapture", uEvent, bindEl), bindEl
									.setCapture());
				}, ym_head);
		/* 页面滚动弹出窗口滚动 */
		...
		var keydownEvent = function(e) {
			...
			if (btnCache) {
				...
				document.activeElement
						&& document.activeElement.id != btnCache[btnIndex].id
						&& (nofocus = true);
				if (keyCode == 9 || keyCode == 39)
					nofocus && (btnIndex = -1), $(btnCache[++btnIndex == l
							? (--btnIndex)
							: btnIndex].id).focus();
				if (keyCode == 37)
					nofocus && (btnIndex = l), $(btnCache[--btnIndex < 0
							? (++btnIndex)
							: btnIndex].id).focus();
				if (keyCode == 13)
					return true;
		...
		var keyEvent = function(e, d) {
			...
			if (!d
					&& /input|select|textarea/i
							.test((e.srcElement || e.target).tagName))
				return true;
			try {
				e.returnValue = false;
				e.keyCode = 0;
		...
		var resizeMask = function(noDelay) {
			...
			isIE
					? noDelay === true ? resize() : setTimeout(resize, 0)
					: resize();
			cur_state == 'min' ? doMin() : cur_state == 'max'
					? doMax()
					: setWinSize(); // 最大化最小化状态还原
		};
		/* 蒙版的显示隐藏,state:true显示,false隐藏，默认为true */
		var maskVisible = function(visible) {
			if (!curCfg.showMask)
				return; // 无遮罩
			(visible === false ? detachEvent : addEvent)("resize", resizeMask,
					window); // 页面大小改变及时调整遮罩大小
			if (visible === false)
				return setStyle(maskEl, 'display', 'none'); // 隐藏遮罩
			setStyle(maskLevel, {
		...
		var getPos = function(f) {
			...
			f = isArray(f) && f.length == 2
					? (f[0] + '+{2},{3}+' + f[1])
					: (posMap[f] || posMap['c']);
			var pos = [rootEl.clientWidth - ym_win.offsetWidth,
					rootEl.clientHeight - ym_win.offsetHeight]
					.concat(getScrollPos());
		...
		var setWinSize = function(w, h, pos) {
			...
			setStyle(ym_body, 'height', h - $height(ym_headbox)
							- $height(ym_btn) - $height(ym_bottom) + 'px'); // 设定内容区的高度
		...
		var cacheWin = []; // 队列中的窗口
		var cacheMsgHd = new Ext.util.MixedCollection();// hashCode相同的不重复提醒
		var winVisible = function(visible) {
			var fn = visible === false ? detachEvent : addEvent;
			fn('scroll', curCfg.fixPosition && !useFixed
							? scrollEvent
							: saveWinInfo, window);
			setStyle(ym_wins, 'position', curCfg.fixPosition && useFixed
							? 'fixed'
							: 'absolute');
			...
			if (visible === false) { // 关闭
				...
				var closeFn = function() {
					...
					cacheMsgHd.removeKey(curCfg.hashcode);
					delete curCfg.hashcode;
					if (cacheWin.length) {
						ymPrompt.show.apply(null, cacheWin[0].concat(true))
					}
				};
				/* 渐变方式关闭 */
			...
			for (var o = document.getElementsByTagName('object'), i = o.length
					- 1; i > -1; i--)
				o[i].style.visibility != 'hidden' && _obj.push(o[i])
						&& (o[i].style.visibility = 'hidden');
			setStyle([ym_hText, ym_hTool], 'display', (curCfg.titleBar
							? ''
							: 'none'));
			...
			ym_body.innerHTML = !curCfg.iframe
					? ('<div class="' + curCfg.msgCls + '">' + curCfg.message + '</div>')
					: "<iframe style='"
							+ ifmStyle
							...
							+ "'></iframe><div style='"
							+ ifmStyle
							+ ";background:#000;opacity:0.1;filter:alpha(opacity=10);display:none'></div>"; // 内容
			(function(el, obj) {
				for (var i in obj)
					try {
						el[i] = obj[i]
			...
			setStyle(ym_btn, 'display',
					((ym_btnContent.innerHTML = joinBtn(mkBtn(curCfg.btn)))
							? ''
							: 'none')); // 没有按钮则隐藏
			!curCfg.useSlide && curCfg.showShadow
					&& setStyle(ym_shadow, 'display', '');
		...
		ymPrompt.apply(ymPrompt, {
			...
			show : function(args, fargs, show) { // 如果有窗口未关闭则将本次传入的信息放到队列里
				...
				// MyMessageTip.msg("提示","o.hashcode:"+o.hashcode+"--"+"curCfg:"+curCfg.hashcode+"<br/>contains:"+cacheMsgHd.containsKey(o.hashcode));
				if (o.hashcode && o.hashcode == curCfg.hashcode) {// 提示信息和当前信息一致，不重复提示
					return;
				}
				if (o.hashcode && curCfg.hashcode
						&& cacheMsgHd.containsKey(o.hashcode)) {
					return;
				}

				if (!show && cacheWin.push([args, fargs])
						&& cacheWin.length > 1) {
					if (o.hashcode) {
						cacheMsgHd.add(o.hashcode, 1);
					}
					return;
				}
				ymPrompt.apply(curCfg, ymPrompt.apply({}, o, fargs), ymPrompt
								.setDefaultCfg()); // 先还原默认配置
				/* 修正curCfg中的无效值(null/undefined)改为默认值 */
				for (var i in curCfg)
					curCfg[i] = curCfg[i] != null ? curCfg[i] : ymPrompt.cfg[i];
				init();
			},
			doHandler : function(sign, autoClose, closeFirst) {
				...
				try {
					// (curCfg.handler)(sign)
					Ext.callback(curCfg.handler, curCfg.scope || this, [sign]);
				} catch (e) {
					alert(e.message)
				};
			},
			...
			setDefaultCfg : function(cfg) {
				return ymPrompt.cfg = ymPrompt.apply({}, cfg, ymPrompt.apply(
								{}, ymPrompt.cfg, dftCfg));
			},
			getButtons : function() {
				var btns = btnCache || [], btn, rBtn = [];
				while (btn = btns.shift())
					rBtn.push($(btn.id));
				return rBtn;
			}
		});
		...
		/* 执行用户初始化时的调用 */
		var t;
		for (var i in _initFn)
			while (t = _initFn[i].shift())
				ymPrompt[i].apply(null, t);
		/* 取消事件绑定 */
		addEvent('unload', function() {
					while (ymPrompt.eventList.length)
						detachEvent.apply(null, ymPrompt.eventList.shift());
				}, window);
	})();
})(); // 各消息框的相同操作
ymPrompt.apply(ymPrompt, {
