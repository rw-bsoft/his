...
public class FsbSummaryofincomeFile implements IHandler {
	...
	public void getParameters(Map<String, Object> request,
		...
		try {
			...
			li = dao.doQuery(
					"SELECT JC_JZHZ.XMBH as XMBH,JC_JZHZ.SQJC as SQJC,JC_JZHZ.QFJE as QFJE,JC_JZHZ.BQFS as BQFS,JC_JZHZ.BQJS as BQJS,JC_JZHZ.XJZP as XJZP,JC_JZHZ.CBJE as CBJE,JC_JZHZ.QTJE as QTJE,JC_JZHZ.BQYE as BQYE FROM JC_JZHZ JC_JZHZ WHERE JC_JZHZ.HZRQ>=:adt_hzrq_b and JC_JZHZ.HZRQ<=:adt_hzrq_e and JC_JZHZ.JGID=:al_jgid order by JC_JZHZ.XMBH asc,JC_JZHZ.HZRQ desc",
					parameters);
			
			for (int i = 0; i < li.size(); i++) {
//				String qtysFb="";
				if (Integer.parseInt(li.get(i).get("XMBH") + "") == 1) {
					li.get(i).put("XMBH", "家床病人结算");
					li.get(i).put("CBJE",
					...
					String ids_zyjs_hql = "select d.FKFS as FKFS,sum(d.FKJE) as FKJE,e.FKMC as FKMC from ("+
							...
							" union all "+
							"select a.JKFS as FKFS, a.JKJE as FKJE from JC_TBKK a, JC_JCJS b where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq_b AND b.HZRQ<=:adt_hzrq_e AND b.JGID=:al_jgid"+
							" union all "+
							"select a.JKFS as FKFS, (-1*a.JKJE) as FKJE from JC_TBKK a, JC_JKZF b,JC_JCJS c where b.JKXH = a.JKXH and c.JSLX<>4 and a.ZYH = c.ZYH and a.JSCS = c.JSCS and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid"+
...
//							" union all "+
							"select a.JKFS as FKFS, a.JKJE as FKJE from JC_TBKK a, JC_JCJS b where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq_b AND b.HZRQ<=:adt_hzrq_e AND b.JGID=:al_jgid "+//and (a.HZRQ<:adt_hzrq_b or a.HZRQ>:adt_hzrq_e or a.HZRQ is null)
							" union all "+
							"SELECT a.JKFS as FKFS,(-1*a.JKJE) as FKJE FROM JC_TBKK a ,JC_JKZF b,JC_JCJS c WHERE c.JSLX<>4 and a.ZYH = c.ZYH and a.JSCS = c.JSCS AND b.JKXH = a.JKXH and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid "+// and (b.HZRQ<:adt_hzrq_b or b.HZRQ>:adt_hzrq_e or b.HZRQ is null)
							") d left outer join GY_FKFS e on d.FKFS = e.FKFS group by d.FKFS,e.FKMC";
...
//							"select a.FKFS as FKFS, a.FKJE as FKJE from JC_FKXX a, JC_JSZF b,JC_JZXX c where a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid"+
//							" union all "+
//							"select a.FKFS as FKFS, a.FKJE as FKJE from JC_FKXX a, JC_JCJS b,JC_JZXX c where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid"+
//							" union all "+
//							"SELECT a.JKFS as FKFS,a.JKJE as FKJE FROM JC_TBKK a,JC_JZXX b WHERE a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ>=:adt_hzrq_b AND b.HZRQ<=:adt_hzrq_e AND b.JGID=:al_jgid"+
//							" union all "+
//							"SELECT (-1*a.JKJE) as FKJE ,a.JKFS as FKFS FROM JC_TBKK a ,JC_JKZF b,JC_JZXX c WHERE b.JKXH = a.JKXH AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid"+
//							") d left outer join GY_FKFS e on d.FKFS = e.FKFS group by d.FKFS,e.FKMC";
//					String ids_brxz_hql = "select sum(c.FYHJ) as FYHJ,sum(c.ZFHJ) as ZFHJ,c.BRXZ as BRXZ,d.XZMC as XZMC,d.DBPB as DBPB from ("+
//					"SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a,JC_JZXX b WHERE a.JSLX<>4 and a.FYHJ<>a.ZFHJ and a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ>=:adt_hzrq_b AND b.HZRQ<=:adt_hzrq_e AND b.JGID=:al_jgid"+
//					" union all "+
//					"SELECT (-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a ,JC_JSZF b,JC_JZXX c WHERE a.JSLX<>4 and a.ZYH = b.ZYH AND a.JSCS = b.JSCS and a.FYHJ<>a.ZFHJ and b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ>=:adt_hzrq_b AND c.HZRQ<=:adt_hzrq_e AND c.JGID=:al_jgid"+
//					") c left outer join GY_BRXZ d on c.BRXZ = d.BRXZ group by c.BRXZ,d.XZMC,d.DBPB";
//				}
