...
public class FamilySickBedChargesSummary extends AbstractActionService implements
		DAOSupportable {
	private static FamilySickBedChargesSummary cck = null;
	Date idt_hzrq = null;
	List<Map<String, Object>> li = new ArrayList<Map<String, Object>>();
	SimpleDateFormat sdfdate = new SimpleDateFormat("yyyy-MM-dd");
	SimpleDateFormat sdfdatetime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	private FamilySickBedChargesSummary() {
	}
...
//		String QTYSFB="";
		...
		try {
			...
			int hzsign = BSPHISUtil.fsb_wf_IsGather(idt_hzrq, idt_LastDate, dao,
					ctx);
			if (hzsign == 1) {
...
//				}
				String ids_fkfs_hql = "select d.FKFS as FKFS,sum(d.FKJE) as FKJE,e.FKMC as FKMC from ("+
						"select a.FKFS as FKFS, (-1*a.FKJE) as FKJE from JC_FKXX a, JC_JSZF b,JC_JZXX c where a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid and b.ZFGH = :czgh"+
						" union all "+
						"select a.FKFS as FKFS, a.FKJE as FKJE from JC_FKXX a, JC_JCJS b,JC_JZXX c where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid and b.CZGH = :czgh"+
						" union all "+
						"SELECT a.JKFS as FKFS,a.JKJE as FKJE FROM JC_TBKK a,JC_JZXX b WHERE a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ IS NULL AND b.JZRQ<=:adt_jzrq AND b.JGID=:al_jgid and a.CZGH = :czgh"+
						" union all "+
						"SELECT a.JKFS as FKFS,(-1*a.JKJE) as FKJE FROM JC_TBKK a ,JC_JKZF b,JC_JZXX c WHERE b.JKXH = a.JKXH AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid and b.ZFGH = :czgh"+
				...
				"SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a,JC_JZXX b WHERE a.JSLX<>4 and a.FYHJ<>a.ZFHJ and a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ IS NULL AND b.JZRQ<=:adt_jzrq AND b.JGID=:al_jgid and a.CZGH = :czgh"+
				" union all "+
				"SELECT (-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a ,JC_JSZF b,JC_JZXX c WHERE a.JSLX<>4 and a.ZYH = b.ZYH AND a.JSCS = b.JSCS and a.FYHJ<>a.ZFHJ and b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid and b.ZFGH = :czgh"+
...
//				QTYSFB=li.get(i).get("QTYSFB")+" ";
			...
			String ids_fkfs_hql = "select d.FKFS as FKFS,sum(d.FKJE) as FKJE,e.FKMC as FKMC from ("+
					"select a.FKFS as FKFS, (-1*a.FKJE) as FKJE from JC_FKXX a, JC_JSZF b,JC_JZXX c where a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid"+
					" union all "+
					"select a.FKFS as FKFS, a.FKJE as FKJE from JC_FKXX a, JC_JCJS b,JC_JZXX c where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid"+
					" union all "+
					"SELECT a.JKFS as FKFS,a.JKJE as FKJE FROM JC_TBKK a,JC_JZXX b WHERE a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ IS NULL AND b.JZRQ<=:adt_jzrq AND b.JGID=:al_jgid"+
					" union all "+
					"SELECT a.JKFS as FKFS,(-1*a.JKJE) as FKJE FROM JC_TBKK a ,JC_JKZF b,JC_JZXX c WHERE b.JKXH = a.JKXH AND b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid"+
					") d left outer join GY_FKFS e on d.FKFS = e.FKFS group by d.FKFS,e.FKMC";
			String ids_brxz_hql = "select sum(c.FYHJ) as FYHJ,sum(c.ZFHJ) as ZFHJ,c.BRXZ as BRXZ,d.XZMC as XZMC,d.DBPB as DBPB from ("+
			"SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a,JC_JZXX b WHERE a.JSLX<>4 and a.FYHJ<>a.ZFHJ and a.JZRQ = b.JZRQ and b.JGID = a.JGID and b.HZRQ IS NULL AND b.JZRQ<=:adt_jzrq AND b.JGID=:al_jgid"+
			" union all "+
			"SELECT (-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,a.BRXZ as BRXZ FROM JC_JCJS a ,JC_JSZF b,JC_JZXX c WHERE a.JSLX<>4 and a.ZYH = b.ZYH AND a.JSCS = b.JSCS and a.FYHJ<>a.ZFHJ and b.JZRQ = c.JZRQ and b.JGID = c.JGID and c.HZRQ IS NULL AND c.JZRQ<=:adt_jzrq AND c.JGID=:al_jgid"+
			") c left outer join GY_BRXZ d on c.BRXZ = d.BRXZ group by c.BRXZ,d.XZMC,d.DBPB";
...
//			}
			...
			zfzslist = dao
					.doQuery(
							"SELECT JC_ZFPJ.PJLB as PJLB,JC_ZFPJ.PJHM as PJHM FROM JC_ZFPJ JC_ZFPJ,JC_JZXX JC_JZXX WHERE ( JC_ZFPJ.JZRQ = JC_JZXX.JZRQ ) AND ( JC_ZFPJ.CZGH = JC_JZXX.CZGH ) AND JC_JZXX.HZRQ IS NULL AND JC_JZXX.JZRQ <=:adt_jzrq and JC_JZXX.JGID=:al_jgid ORDER BY JC_ZFPJ.PJLB,JC_ZFPJ.PJHM",
							parametershzbd);
	...
	public void doInquiry(Map<String, Object> req, Map<String, Object> res,
		...
		li.clear();
		try {
			BSPHISUtil.fsb_wf_Query(req, res, li, dao, ctx);
		} catch (ModelDataOperationException e) {
			e.printStackTrace();
		}
	}

	public static FamilySickBedChargesSummary getInstance() {
		if (cck == null) {
			cck = new FamilySickBedChargesSummary();
		}
		return cck;
	}
