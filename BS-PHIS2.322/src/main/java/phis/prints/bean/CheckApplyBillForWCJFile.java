...
public class CheckApplyBillForWCJFile implements IHandler {
...
//					+ "";
			...
			//申请单名称、门诊号码/住院号码
			if (yllb == 1) {
				response.put("sqdmc", "胃肠镜申请单(门诊)");
				response.put("hmtitle", "门诊号码：");
			} else {
				response.put("sqdmc", "胃肠镜申请单(住院)");
			...
			//体格检查
//			if(!ctxx.equals("T:℃    P:次/分   R:次/分    BP: / mmHg   ") && !ctxx.equals("")){
//				response.put("ctxxtitle", "体格检查：");
