...
public class FsbSummarycategoryHospitalFile implements IHandler {
	@Override
	public void getFields(Map<String, Object> request,
		...
		try {
			...
			List<Map<String, Object>> xmflList = dao
					.doQuery(
							"SELECT b.ZYGB as ZYGB,b.SFMC as SFMC,sum(a.ZJJE) as ZJJE FROM JC_SRHZ a,GY_SFXM b WHERE ( a.SFXM = b.SFXM ) AND ( a.HZRQ >= :adt_hzrq ) and ( a.HZRQ <= :adt_hzrq_end ) AND a.JGID = :al_jgid GROUP BY b.ZYGB,b.SFMC",
							parameters);
	...
	public void getParameters(Map<String, Object> request,
		...
		try {
			...
			Map<String, Object> xmflMap = dao
					.doLoad("SELECT nvl(sum(a.ZJJE),0) as ZJJE FROM JC_SRHZ a,GY_SFXM b WHERE ( a.SFXM = b.SFXM ) AND ( a.HZRQ >= :adt_hzrq ) and ( a.HZRQ <= :adt_hzrq_end ) AND a.JGID = :al_jgid",
							parameter);
			response.put("HJJE", String.format("%1$.2f", xmflMap.get("ZJJE")));//总计金额
			
//			Map<String, Object> xmflMap2 = dao
//			.doLoad("SELECT sum(a.GRXJZF) as XJJE,sum(a.TCZC) as TCZF,sum(a.BCZHZF) as ZHZF,sum(a.YJQT) as JZJE FROM JC_JZXX a WHERE ( HZRQ >= :adt_hzrq ) and ( HZRQ <= :adt_hzrq_end ) AND JGID = :al_jgid",
//					parameter);
