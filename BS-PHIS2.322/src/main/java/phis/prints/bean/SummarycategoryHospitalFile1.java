...
public class SummarycategoryHospitalFile1 implements IHandler {
	...
	public void getParameters(Map<String, Object> request,
		...
		try {
			...
			hql3.append("SELECT g.ZYGB as ZYGB,g.SFMC as SFMC,sum(a.ZJJE) as ZJJE FROM ZY_FYMX a,GY_SFXM g WHERE ( a.FYXM = g.SFXM ) and a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH||'_'||a.JSCS not in (select c.JLXH||'_'||c.jscs from ZY_ZYJS b, ZY_FYMX_js c where c.ZYH = b.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))");
			hql5.append("SELECT g.ZYGB as ZYGB, g.SFMC as SFMC, sum(a.ZJJE) as ZJJE FROM ZY_FYMX a, GY_SFXM g WHERE (a.FYXM = g.SFXM) and a.JSCS<>0 AND a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))");
			if (bq > 0) {// 某个病区
			...
			hzhql3.append("select sum(a.ZJJE) as ZJJE from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH||'_'||a.JSCS not in (select c.JLXH||'_'||c.jscs from ZY_ZYJS b, ZY_FYMX_JS c where c.ZYH = b.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))");
			hzhql5.append("SELECT sum(a.ZJJE) as ZJJE FROM ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.JSCS<>0 and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))");
			if (bq > 0) {// 某个病区
				hzparameters.put("FYBQ", bq);
				hzhql3.append(" and a.FYBQ=:FYBQ ");
				hzhql5.append(" and a.FYBQ=:FYBQ ");
			}
			String hzsql = hzType == 5 ? hzhql5.toString() : hzhql3.toString();
...
//					+ "select a.FKFS as FKFS, (-1*a.FKJE) as FKJE from ZY_FKXX a, ZY_JSZF b,(select distinct ZYH,JSCS from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))) c where a.ZYH = c.ZYH and a.JSCS = c.JSCS and a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"
//					+ " union all "
					+ "select a.FKFS as FKFS, a.FKJE as FKJE from ZY_FKXX a, ZY_ZYJS b,(select distinct ZYH,JSCS from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))) c where a.ZYH = c.ZYH and a.JSCS = c.JSCS and a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"
					+ " union all "
					+ "select a.JKFS as FKFS, a.JKJE as FKJE from ZY_TBKK a, ZY_ZYJS b,(select distinct ZYH,JSCS from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))) c where a.ZYH = c.ZYH and a.JSCS = c.JSCS and a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"
					+ " union all "
					+ "select a.JKFS as FKFS, (-1*a.JKJE) as FKJE from ZY_TBKK a, ZY_JKZF b,ZY_ZYJS c,(select distinct ZYH,JSCS from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))) d where a.ZYH = d.ZYH and a.JSCS = d.JSCS and b.JKXH = a.JKXH and a.ZYH = c.ZYH and a.JSCS = c.JSCS and c.HZRQ>=:adt_hzrq AND c.HZRQ<=:adt_hzrq_end AND c.JGID=:al_jgid"
			...
			String ids_brxz_hql = "select sum(c.FYHJ) as FYHJ,sum(c.ZFHJ) as ZFHJ,c.BRXZ as BRXZ,d.XZMC as XZMC,d.DBPB as DBPB from ("
					+ "SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM ZY_ZYJS a,(select distinct ZYH,JSCS from ZY_FYMX a where a.HZRQ >= :adt_hzrq and a.HZRQ <= :adt_hzrq_end and a.JGID = :al_jgid and (a.JLXH in (select c.JLXH from ZY_ZYJS b, ZY_FYMX_JS c where b.ZYH = c.ZYH and c.JSCS = b.JSCS and b.HZRQ >= :adt_hzrq and b.HZRQ <= :adt_hzrq_end and b.JGID = :al_jgid and c.JLXH||'_'||c.JSCS not in (select d.jlxh||'_'||d.jscs from ZY_FYMX_js d,zy_zyjs e,zy_jszf f where d.ZYH = e.ZYH and d.JSCS = e.JSCS and f.zyh = e.zyh and f.JSCS = e.JSCS and f.HZRQ >= :adt_hzrq and f.HZRQ <= :adt_hzrq_end and e.JGID = :al_jgid)))) b WHERE a.ZYH = b.ZYH and a.JSCS = b.JSCS and a.FYHJ<>a.ZFHJ and a.HZRQ>=:adt_hzrq AND a.HZRQ<=:adt_hzrq_end AND a.JGID=:al_jgid"
