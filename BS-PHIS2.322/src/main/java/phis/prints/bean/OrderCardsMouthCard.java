...
public class OrderCardsMouthCard implements IHandler {
	...
	public void getFields(Map<String, Object> request,
		...
		try {
			...
			String wardName = new String(request.get("wardName").toString()// 病区名称
			...
			while (iter.hasNext()) {
				...
				if (ZXSJ.length != 0) {
					...
					for (int i = 0; i < ZXSJ.length; i++) {
						...
						SimpleDateFormat sdfsfm = new SimpleDateFormat(
								"yyyy-MM-dd");
						c.setTime(sdfsfm.parse(dysj));
						nowYear = c.get(Calendar.YEAR);
						nowMouth = c.get(Calendar.MONTH) + 1;
						nowDay = c.get(Calendar.DATE);
						o.put("BRNL", age + "");// 病人年龄
						o.put("Year", nowYear + "");// 年
						o.put("Month", nowMouth + "");// 月
						o.put("Day", nowDay + "");// 日
						o.put("BQMC", wardName);// 病区名称
	...
	public void getParameters(Map<String, Object> request,
		...
		String realPath = OrderCardsMouthCard.class.getResource("/").getPath();
	...
	public List<Map<String, Object>> getList(Map<String, Object> request,
		...
		try {
			...
			if (arr_zyh.length() != 0) {// 如果没有传进用户医嘱人
				...
				int FHBZ = Integer.parseInt(ParameterUtil.getParameter(JGID,"FHYZHJF", ctx));
				//增加参数用来控制是否打印自备药品和st药品0不打1打
				int YZKPDYZB = Integer.parseInt(ParameterUtil.getParameter(JGID,"YZKPDYZB", ctx));
				int YZKPDYST = Integer.parseInt(ParameterUtil.getParameter(JGID,"YZKPDYST", ctx));
				String dyxzsql="";
				if(YZKPDYZB==0 ){
					dyxzsql+=" and b.ZFYP not in ('1') ";
				}
				if( YZKPDYST==0 ){
					dyxzsql+="  and b.SYPC not in ('ST','st') ";
				}
				//yx-2017-09-26-sql里去除了自备药品和st药品
				sql.append("SELECT b.ZFYP as ZFYP,b.YZMC as YZMC,b.YPLX as YPLX,b.YPXH as YPXH,b.KSSJ as KSSJ," +
						" b.TZSJ as TZSJ,b.YCSL as YCSL,b.YCJL as YCJL,b.YZZH as YZZH,b.SYPC as SYPC,d.BRCH as BRCH," +
						" d.BRXM as BRXM,d.CSNY as CSNY,b.MRCS as MRCS,b.SRCS as SRCS,a.PCMC as PCMC,c.XMMC as XMMC," +
						" b.BZXX as BZXX,b.JLXH as JLXH,e.JLDW as JLDW,d.ZYHM as ZYHM,b.SRKS as SRKS,b.YEPB as YEPB," +
						" b.YZZXSJ as YZZXSJ FROM GY_SYPC a,ZY_BQYZ b,ZY_YPYF c,ZY_BRRY d,YK_TYPK e" +
						" WHERE (b.YPXH=e.YPXH) and (a.PCBM=b.SYPC) and (b.ZYH=d.ZYH) and (c.YPYF=b.YPYF)" +
						" and (d.CYPB=0 ) AND (b.ZYH in ("+arr_zyh+")) AND (b.SRKS=:BQDM) AND (b.LSBZ=0)" +
						" and (b.YCSL>0) AND ((((:YZLB=2 OR :YZLB=3 OR :YZLB=4) AND b.YPLX>0" +
						" AND (b.XMLX=1 or b.XMLX=2 or b.XMLX=3)) OR :YZLB=5 OR :YZLB=9) AND c.XMLB=:YZLB) AND "+
						" (b.YSBZ=0 OR (b.YSBZ=1 AND b.YSTJ=1)) AND (b.YZPB=0) and b.JGID=:JGID" +
						" and (:FHBZ=0 or b.FHBZ=1) " +dyxzsql);
				...
				while (iter.hasNext()) {
					...
					if (zxbz != 1) {
						body.remove(rec);
					} else {// update by caijy at 2014.10.9 for 自备药打印需要备注
						if ("1".equals(rec.get("ZFYP") + "")) {
							rec.put("YZMC", "(自备)" + rec.get("YZMC"));
						}
					}
				}
			}
	...
	public int validOrders(String sypc, Date kssj, Date nowDate, Context ctx) {
		...
		try {
			...
			records = SchemaUtil.setDictionaryMassageForList(records,
					"phis.application.cfg.schemas.GY_SYPC");
			...
			if (records.size() != 0) {// PCBM 是主键,最多只能搜到一条数据
				...
				if (rlz == 1) {
					...
					if (rzxzq.length() == zqbz) {
						zxbz = Integer.parseInt(rzxzq.substring(zqbz - 1));
					} else {
						zxbz = Integer
								.parseInt(rzxzq.substring(zqbz - 1, zqbz));
					}
				} else {
					...
					if (rzlen <= days) {
						...
						zxbz = Integer
								.parseInt(rzxzq.substring(days, days + 1));// 0或1
					}
				}
				return zxbz;
