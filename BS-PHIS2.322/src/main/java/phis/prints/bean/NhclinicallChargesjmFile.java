...
public class NhclinicallChargesjmFile implements IHandler {
	@Override
...
//                         " from MS_MZXX A, NJJB_JSXX B,NJJB_KXX C,MS_BRDA D WHERE A.BRID=D.BRID AND A.BRXZ=D.BRXZ AND D.SFZH=C.SFZH AND D.SHBZKH=C.SHBZKH AND  A.JGID = B.JGID AND A.FPHM = B.FPHM AND A.MZXH = B.MZXH AND TO_CHAR(A.SFRQ, 'YYYY-MM-DD') = TO_CHAR(B.JSSJ, 'YYYY-MM-DD')"+
