 ...
 */
public class ChargeSummaryZYFile implements Service {
	@Override
	public void execute(Map<String, Object> req, Map<String, Object> res,
		...
		try {
			String sql = "SELECT b.BRXZ as BRXZ,c.XZMC as XZMC,to_char(b.JZRQ,'yyyy-MM-dd HH24:mm:ss') as JZRQ,"
					+ "a.FYXM as FYXM, d.SFMC as SFMC,sum(a.ZJJE) as ZJJE "
					+ "FROM ZY_FYMX  a,ZY_ZYJS   b, GY_BRXZ c,GY_SFXM d "
					+ "WHERE  ( a.ZYH = b.ZYH ) "
					+ "AND ( a.JGID = b.JGID ) AND ( b.BRXZ = c.BRXZ )  "
					+ "AND ( a.FYXM = d.SFXM ) "
					+ " AND ( c.DBPB ='2' )  "
					+ "AND  ( a.JGID = :manaUnitId )  "
					+ "AND ( to_char(a.JFRQ,'yyyy-MM-dd HH24:mm:ss')>=:beginDate ) "
					+ "AND (to_char(a.JFRQ ,'yyyy-MM-dd HH24:mm:ss') <=:endDate ) "
					+ "GROUP BY (b.BRXZ,a.FYXM,c.XZMC,d.SFMC,b.JZRQ) ";
			String sql_zf = "SELECT b.BRXZ as BRXZ,c.XZMC as XZMC,to_char(b.JZRQ,'yyyy-MM-dd HH24:mm:ss') as JZRQ,"
					+ "a.FYXM as FYXM, d.SFMC as SFMC,sum(a.ZJJE) as ZJJE "
					+ "FROM ZY_FYMX  a,ZY_ZYJS   b, GY_BRXZ c,GY_SFXM d,ZY_JSZF e "
					+ "WHERE  ( a.ZYH = b.ZYH ) "
					+ "AND (b.ZYH=e.ZYH AND b.JSCS=e.JSCS) "
					+ "AND ( a.JGID = b.JGID ) AND ( b.BRXZ = c.BRXZ )  "
					+ "AND ( a.FYXM = d.SFXM ) "
					+ " AND ( c.DBPB ='2' )  "
					+ "AND  ( a.JGID = :manaUnitId )  "
