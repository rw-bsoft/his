...
public class NhHosChargedetailsFile implements IHandler {
	@Override
	public void getFields(Map<String, Object> request,
		...
		String sql=" select A.ZYH as ZYH,A.ZYHM as ZYHM,B.SUM08 as SUM08,B.SUM12 as SUM12,B.ICKH as ICKH,"
          +" A.BRXM as BRXM,A.HKDZ as HKDZ, D.FYHJ as ZJJE,B.SUM02 as BCJE," +
          "(select c.jbmc from EMR_ZYZDJL c where A.ZYH=C.JZXH and c.ZDLB='51' and rownum =1)  as JBMC,B.SUM39 as DBJJ,"
          +" b.sum06 as SUM06"
          +" FROM ZY_BRRY A,NH_BSOFT_JSJL B,ZY_ZYJS D"
          +" WHERE A.JGID =:JGID AND D.ZYH = A.ZYH AND B.ZYH = D.ZYH AND"
          +" B.JSCS = D.JSCS AND D.JSRQ >= to_date(:beginDate,'yyyy-mm-dd  HH24:mi:ss') "
          +" AND D.JSRQ <=to_date(:endDate,'yyyy-mm-dd HH24:mi:ss')"
          +" UNION ALL"
          +" select A.ZYH as ZYH,A.ZYHM as ZYHM,-B.SUM08 as SUM08,- B.SUM12 as SUM12,B.ICKH as ICKH,"
          +" A.BRXM as BRXM,A.HKDZ as HKDZ,-b.zjje as ZJJE,-B.SUM02 as BCJE, " +
          "(select c.jbmc from EMR_ZYZDJL c where A.ZYH=C.JZXH and c.ZDLB='51' and rownum =1)  as JBMC,-B.SUM39 as DBJJ,"
          +" b.sum06 as SUM06 "
          +" FROM ZY_BRRY A,NH_BSOFT_JSJL B,ZY_JSZF D"
          +" WHERE A.JGID =:JGID AND D.ZYH = A.ZYH AND B.ZYH = D.ZYH AND B.JSCS = D.JSCS AND "
          +" D.ZFRQ >=  to_date(:beginDate,'yyyy-mm-dd  HH24:mi:ss') "
