...
public class SummarycategoryHospitalForFile implements IHandler {
	@Override
			...
			List<Map<String, Object>> xmflList = dao
					.doSqlQuery(
							"select s.ZYGB as ZYGB,s.SFMC as SFMC,sum(s.ZJJE) as ZJJE "
									+ "from (SELECT b.ZYGB as ZYGB,b.SFMC as SFMC,c.ZJJE as ZJJE FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c "
									...
									+ "union all "
									+ "select b.ZYGB as ZYGB,b.SFMC as SFMC,-(c.ZJJE) as ZJJE FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c,ZY_JSZF d "
									+ "WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) "
			...
			List<Map<String, Object>> xmflList2 = dao
					.doSqlQuery(
							"select s.ZYGB as ZYGB,(case when s.ZYGB = 2 then '西药基药' when s.ZYGB = 1 then '中成药基药' end) as SFMC,sum(s.ZJJE) as ZJJE "
									+ "from (SELECT b.ZYGB as ZYGB,b.SFMC as SFMC,c.ZJJE as ZJJE FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c ,YK_TYPK d "
									...
									+ "FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c,ZY_JSZF d,YK_TYPK f "
									+ "WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) "
	...
	public void getParameters(Map<String, Object> request,
		...
		try {
			...
			List<Map<String, Object>> xmflList = dao
					.doSqlQuery(
							"select sum(s.ZJJE) as ZJJE from (SELECT c.ZJJE as ZJJE FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) AND ( a.HZRQ >=:adt_hzrq ) and ( a.HZRQ <:adt_hzrq_end ) AND a.JGID =:al_jgid union all select -(c.ZJJE) as ZJJE FROM ZY_ZYJS a,GY_SFXM b,ZY_FYMX_js c,ZY_JSZF d WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) AND ( a.ZYH = d.ZYH ) AND ( a.JSCS = d.JSCS ) AND ( a.JGID = d.JGID ) AND ( d.hzrq >=:adt_hzrq )  and ( d.hzrq <:adt_hzrq_end ) AND a.JGID =:al_jgid) s",
							parameter);
			...
			Map<String, Object> xmflMap2 = dao
					.doLoad("SELECT sum(TCZC) as TCZF,sum(SRJE) AS SRJE, sum(BCZHZF) as ZHZF FROM ZY_JZXX WHERE ( HZRQ >= :adt_hzrq ) and ( HZRQ < :adt_hzrq_end ) AND JGID = :al_jgid",
							parameter);
			...
			Map<String, Object> xmflMap3 = dao
					.doLoad("SELECT COUNT(BRID) AS ZJJE FROM ZY_BRRY where (ryrq >=:adt_hzrq ) and (ryrq <:adt_hzrq_end ) AND JGID =:al_jgid ",
							parameter);
			// 出院人数
			Map<String, Object> xmflMap4 = dao
					.doLoad("SELECT COUNT(BRID) AS ZJJE FROM ZY_BRRY where CYPB=8 and (cyrq >=:adt_hzrq ) and (cyrq <:adt_hzrq_end ) AND JGID =:al_jgid",
							parameter);
			//其中
//			String qtysFb="";
//			List<Map<String, Object>> qtflList = dao
//					.doSqlQuery(
//							"select t.ZJJE,g.XZMC from (select sum(s.ZJJE) as ZJJE,BRXZ   from (  SELECT sum(c.ZJJE) as ZJJE,a.BRXZ as BRXZ FROM ZY_ZYJS a, GY_SFXM b, ZY_FYMX_js c,gy_brxz d WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) AND ( a.HZRQ >=:adt_hzrq ) and ( a.HZRQ <=:adt_hzrq_end ) AND a.JGID =:al_jgid  AND a.brxz=d.brxz  group by a.brxz " +
//							" union all   " +
//							"select sum(- (c.ZJJE)) as ZJJE,a.BRXZ as BRXZ   FROM ZY_ZYJS a, GY_SFXM b, ZY_FYMX_js c, ZY_JSZF d,gy_brxz f  WHERE ( c.FYXM = b.SFXM ) AND ( a.ZYH = c.ZYH ) AND ( a.JSCS = c.JSCS ) AND ( a.JGID = c.JGID ) AND ( a.ZYH = d.ZYH ) AND ( a.JSCS = d.JSCS ) AND ( a.JGID = d.JGID ) AND ( d.hzrq >=:adt_hzrq )  and ( d.hzrq <:adt_hzrq_end ) AND a.JGID =:al_jgid    AND a.brxz=f.brxz  group by a.brxz  ) s group by BRXZ ) t left join gy_brxz g on t.BRXZ=g.BRXZ ",
//							parameter);
...
//			response.put("qtysFb", qtysFb);// 其中:
			String ids_zyjs_hql = "select d.FKFS as FKFS,sum(d.FKJE) as FKJE,e.FKMC as FKMC from ("+
					"select a.FKFS as FKFS, (-1*a.FKJE) as FKJE from ZY_FKXX a, ZY_JSZF b where a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"+
					" union all "+
					"select a.FKFS as FKFS, a.FKJE as FKJE from ZY_FKXX a, ZY_ZYJS b where a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"+
					" union all "+
					"select a.JKFS as FKFS, a.JKJE as FKJE from ZY_TBKK a, ZY_ZYJS b where a.ZYH = b.ZYH and a.JSCS = b.JSCS and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"+
					" union all "+
					"select a.JKFS as FKFS, (-1*a.JKJE) as FKJE from ZY_TBKK a, ZY_JKZF b,ZY_ZYJS c where b.JKXH = a.JKXH and a.ZYH = c.ZYH and a.JSCS = c.JSCS and c.HZRQ>=:adt_hzrq AND c.HZRQ<=:adt_hzrq_end AND c.JGID=:al_jgid"+
			...
			String ids_brxz_hql = "select sum(c.FYHJ) as FYHJ,sum(c.ZFHJ) as ZFHJ,c.BRXZ as BRXZ,d.XZMC as XZMC,d.DBPB as DBPB from ("+
					"SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM ZY_ZYJS a WHERE a.FYHJ<>a.ZFHJ and a.HZRQ>=:adt_hzrq AND a.HZRQ<=:adt_hzrq_end AND a.JGID=:al_jgid"+
					" union all "+
					"SELECT (-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,a.BRXZ as BRXZ FROM ZY_ZYJS a ,ZY_JSZF b WHERE a.ZYH = b.ZYH AND a.JSCS = b.JSCS and a.FYHJ<>a.ZFHJ and b.HZRQ>=:adt_hzrq AND b.HZRQ<=:adt_hzrq_end AND b.JGID=:al_jgid"+
