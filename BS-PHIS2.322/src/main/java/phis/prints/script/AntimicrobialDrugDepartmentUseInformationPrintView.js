...
phis.prints.script.AntimicrobialDrugDepartmentUseInformationPrintView = function(cfg) {
	this.width = 800
	this.height = 500
	...
	this.conditions = []
	this.sfrbrq = "";
	phis.prints.script.AntimicrobialDrugDepartmentUseInformationPrintView.superclass.constructor.apply(this,
			[cfg])
	Ext.apply(this, phis.script.common);
}
Ext.extend(phis.prints.script.AntimicrobialDrugDepartmentUseInformationPrintView, app.desktop.Module, {
	initPanel : function() {
		this.frameId = "SimplePrint_frame_AntimicrobialDrugDepartmentUseInformation";
		this.conditionFormId = "SimplePrint_form_AntimicrobialDrugDepartmentUseInformation";
		this.mainFormId = "SimplePrint_mainform_AntimicrobialDrugDepartmentUseInformation";
		var panel = new Ext.Panel({
			...
			html : "<iframe id='"
					+ this.frameId
					+ "' width='100%' height='100%' onload='simplePrintMask(\"AntimicrobialDrugDepartmentUseInformation\")'></iframe>"
		})
		this.panel = panel
		this.panelq = panel
	...
	doPrint : function() {
		...
		var dateTo = this.panel.getTopToolbar().items.get(3).getValue()
				.format('Y-m-d');
//		var url = this.printurl + ".print?pages=AntimicrobialDrugDepartmentUseInformation";
		var pages="phis.prints.jrxml.AntimicrobialDrugDepartmentUseInformation";
...
//		var url = this.printurl + ".print?pages=AntimicrobialDrugDepartmentUseInformation";
	...
	doQueryDetil: function(ksdm){
		var module = this.createModule("AntimicrobialDrugDoctorUseInfo_ZY",'phis.application.war.WAR/WAR/WAR59');
		module.ksdm=ksdm;
	...
	simplePrintMask : function(iframe, printId) {
		...
		if (tables.length > 1) {
			...
			for (var i = headerRow; i < trs.length - footerRow; i++) {// 只监听表格内容列
				...
				tr.cells[20].style = "display:none;";//隐藏第13列（ksdm）
			}
