...
//按科室界面
phis.prints.script.BasicDrugUseStatistics_MZCF = function(cfg) {
	this.width = 800
	this.height = 500
	this.printurl = util.helper.Helper.getUrl();
	this.sfrbrq = "";
	phis.prints.script.BasicDrugUseStatistics_MZCF.superclass.constructor.apply(this,
			[cfg])
	Ext.apply(this, phis.script.common);
}
Ext.extend(phis.prints.script.BasicDrugUseStatistics_MZCF, app.desktop.Module, {
	initPanel : function() {
		this.frameId = "SimplePrint_frame_BasicDrugUseStatistics_MZCF";
		this.conditionFormId = "SimplePrint_form_BasicDrugUseStatistics_MZCF";
		this.mainFormId = "SimplePrint_mainform_BasicDrugUseStatistics_MZCF";
	...
	doPrint : function() {
		...
		this.type="1";		
		var pages="phis.prints.jrxml.BasicDrugUseStatistics_MZCF";
...
//		var url = this.printurl + ".print?pages=AntimicrobialDrugDoctorUseInformation";
	...
	// 	var module = this.createModule("MZCF_BasicDrugUseStatistics",'phis.application.war.WAR/WAR/WAR72');
	// 	module.ksdm=ksdm;
				...
				if (iframe.attachEvent) {
					iframe.attachEvent("onload", function() {
								_ctx .simplePrintMask(iframe,
												"BasicDrugUseStatistics_MZCF");
							});
				} else {
					iframe.onload = function() {
						_ctx.simplePrintMask(iframe, "BasicDrugUseStatistics_MZCF");
					};
				}
			},
			/**
			...
			simplePrintMask : function(iframe, printId) {
				...
				if (tables.length > 1) {
					...
					for (var i = headerRow; i < trs.length - footerRow; i++) {// 只监听表格内容列
						...
						//tr.cells[0].style = "display:none;";//隐藏第13列（ksdm）
					}
