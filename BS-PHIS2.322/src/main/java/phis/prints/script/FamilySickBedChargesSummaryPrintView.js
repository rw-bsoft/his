...
phis.prints.script.FamilySickBedChargesSummaryPrintView = function(cfg) {
	// this.width = 800
	// this.height = 500
	this.printurl = util.helper.Helper.getUrl();
	this.conditions = []
	phis.prints.script.FamilySickBedChargesSummaryPrintView.superclass.constructor
			.apply(this, [cfg])
	Ext.apply(this, phis.script.SimpleModule);
}
Ext.extend(phis.prints.script.FamilySickBedChargesSummaryPrintView,
		phis.script.SimpleModule, {
			initPanel : function() {
				this.frameId = "SimplePrint_frame_FamilySickBedChargesSummary";
				this.conditionFormId = "SimplePrint_form_FamilySickBedChargesSummary";
				this.mainFormId = "SimplePrint_mainform_FamilySickBedChargesSummary";
				this.frameId1 = "SimplePrint_frame_FsbSummaryofincome";
				this.frameId2 = "SimplePrint_frame_FsbSummarycategoryHospital";
				this.frameId3 = "SimplePrint_frame_FsbSummarycategoryHospital3";
				this.frameId4 = "SimplePrint_frame_FsbSummarycategoryHospital4";
				this.tab = new Ext.TabPanel({
					...
					items : [{
						...
						html : "<iframe id='"
								+ this.frameId
								+ "' width='100%' height='94%' scrolling='yes' onload='simplePrintMask(\"FamilySickBedChargesSummary\")'></iframe>"
					}, {
						id : 'hz2',
						title : '汇总日报(二)',
						html : "<iframe id='"
								+ this.frameId1
								+ "' width='100%' height='100%' onload='simplePrintMask(\"FamilySickBedChargesSummary\")'></iframe>"
					}, {
						id : 'hz3',
						title : '汇总日报(三)',
						html : "<iframe id='"
...
//				}
				...
				var pages="phis.prints.jrxml.FsbSummaryofincome";
				 var url="resources/"+pages+".print?preview=1";
				url += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate));
				if (document.getElementById(this.frameId1)) {
					document.getElementById(this.frameId1).src = url
				}
				var pages="phis.prints.jrxml.FsbSummarycategoryHospital";
				 var url1="resources/"+pages+".print?preview=1";
				 url1 += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate));
				if (document.getElementById(this.frameId2)) {
					document.getElementById(this.frameId2).src = url1
				}
				var pages="phis.prints.jrxml.FsbSummarycategoryHospitalFor";
				 var url2="resources/"+pages+".print?preview=1";
				url2 += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate));
				if (document.getElementById(this.frameId3)) {
					document.getElementById(this.frameId3).src = url2
				}
				
				var pages="phis.prints.jrxml.FsbSummarycategoryHospital1";
				 var url3="resources/"+pages+".print?preview=1";
				 url3 += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate))+ "&hz=5";
				if (document.getElementById(this.frameId4)) {
			...
			doHelp : function(){
				Ext.MessageBox.alert("家床管理日终汇总模块分页说明", 
"汇总日报(一)：统计某一次结帐汇总的结算和缴款信息，主要是财务对所有收费员的交账信息进行核对.<br>"+
"汇总日报(二)：统计某一次结帐汇总的医院所有收入，包括在床和撤床病人发生的所有费用.<br>"+
"汇总日报(三)：统计某一次结帐汇总的所有在床病人的所有费用，不包含撤床病人的费用，按大类统计.<br>"+
"汇总日报(四)：统计某一次结帐汇总的结算撤床病人的所有费用，不包含在床病人的费用，按大类和医保分统计.<br>"+
"汇总日报(五)：统计某一次结帐汇总的结算撤床病人的本次汇总时间内发生的费用，按大类统计.");
			},
			getExecJs : function() {
			...
			doCancelCommit : function(){
				...
				msg += '<br/><br/>取消汇总日报：<br/><br/>&nbsp&nbsp&nbsp&nbsp汇总日期:&nbsp&nbsp&nbsp&nbsp' + result.json.HZRQ + '<br/>';
				Ext.MessageBox.confirm('取消汇总', msg,
				function(btn, text) {
					if (btn == "yes") {
			...
			doPrint : function() {
				...
				if (this.tab.getItem("hz1").isVisible()) {
					var pages="phis.prints.jrxml.FamilySickBedChargesSummary";
					 var url="resources/"+pages+".print?landscape=1&silentPrint=1&execJs="
							+ this.getExecJs();
					url += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate));
					url += "&save=2";
					var LODOP=getLodop();
				...
				if (this.tab.getItem("hz5").isVisible()) {//(五)出院发生
					var pages="phis.prints.jrxml.FsbSummarycategoryHospital1";
					 var url="resources/"+pages+".print?silentPrint=1&execJs="
							+ this.getExecJs();
					url += "&beginDate=" + encodeURI(encodeURI(this.hzDate)) + "&endDate=" + encodeURI(encodeURI(this.hzDate)) + "&hz=5";
					var LODOP=getLodop();
					LODOP.PRINT_INIT("打印控件");
					LODOP.SET_PRINT_PAGESIZE("0","","","");
					LODOP.ADD_PRINT_HTM("0","0","100%","100%",util.rmi.loadXML({url:url,httpMethod:"get"}));
...
//						}
						...
						//		+ "*.print?pages=phis.prints.jrxml.FamilySickBedChargesSummary";
						var pages="phis.prints.jrxml.FamilySickBedChargesSummary";
...
//						}
						Ext.getCmp(this.mainFormId).el.mask(
								"正在生成报表...", "x-mask-loading")
						url += "&beginDate=" + encodeURI(encodeURI(r.json.idt_hzrq)) + "&endDate="
								+ encodeURI(encodeURI(r.json.idt_hzrq)) + "&save=2";
			...
			doInquiry : function() {
				...
				module.doSure = function() {
					...
					var pages="phis.prints.jrxml.FamilySickBedChargesSummary";
					 var url="resources/"+pages+".print?type=1&beginDate="+encodeURI(encodeURI(hzrq))+"&endDate="+encodeURI(encodeURI(hzrq))+"&save=2";
			...
			onQueryWinShow : function() {
				...
				module.requestData.serviceAction = "querySQLJCJSList";
				module.loadData();
			},
			doQuery : function() {
				var form = Ext.getCmp(this.conditionFormId).getForm()
			...
			doLoadReport : function() {
				...
				if (r.code == 600) {
					if (confirm("第一次汇总将把所有数据结帐到" + r.json.idt_hzrq
							+ "的日期，确定产生汇总日报吗?")) {
						//var url = this.printurl
						//		+ "*.print?pages=phis.prints.jrxml.FamilySickBedChargesSummary&preview=1";
						var pages="phis.prints.jrxml.FamilySickBedChargesSummary";
						 var url="resources/"+pages+".print?preview=1";
						for (var i = 0; i < items.getCount(); i++) {
							var f = items.get(i)
...
//							}
					...
					Ext.MessageBox.alert("提示", "最后一次汇总日期为"+r.json.idt_LastDate+",当前所选日期小于最后汇总日期，不能再次汇总!");
					itemstb.get(10).setDisabled(false);
					return;
				} else if (r.code == 602) {
					Ext.MessageBox.alert("提示", "最后一次汇总日期为"+r.json.idt_LastDate+",当前所选日期等于最后汇总日期，不能再次汇总!");
					itemstb.get(10).setDisabled(false);
					return;
				} else if (r.code == 603) {
...
//					}
					Ext.getCmp(this.mainFormId).el.mask(
							"正在生成报表...", "x-mask-loading")
					url += "&beginDate=" + encodeURI(encodeURI(r.json.idt_hzrq +" 00:00:00")) + "&endDate="
							+ encodeURI(encodeURI(r.json.idt_hzrq +" 00:00:00")) + "&save=2";
