...
$import("phis.script.SimpleModule", "util.widgets.MyRadioGroup", "util.DateUtil",
		"util.dictionary.TreeDicFactory", "util.helper.Helper","phis.script.common")
//明细界面
phis.prints.script.AntimicrobialDrugClinicDoctorUseDetailsPrintView = function(cfg) {
	this.width = 1000
	this.height = 500
	this.printurl = util.helper.Helper.getUrl();
	this.sfrbrq = "";
	phis.prints.script.AntimicrobialDrugClinicDoctorUseDetailsPrintView.superclass.constructor.apply(this,
			[cfg])
	Ext.apply(this, phis.script.common);
}
Ext.extend(phis.prints.script.AntimicrobialDrugClinicDoctorUseDetailsPrintView, phis.script.SimpleModule, {
	initPanel : function() {
		this.frameId = "SimplePrint_frame_AntimicrobialDrugClinicDoctorUseDetailsPrintView";
		this.conditionFormId = "SimplePrint_form_AntimicrobialDrugClinicDoctorUseDetailsPrintView";
		this.mainFormId = "SimplePrint_mainform_AntimicrobialDrugClinicDoctorUseDetailsPrintView";
	...
	doPrint : function() {
		...
		var pages="phis.prints.jrxml.AntimicrobialDrugClinicDoctorUseDetails";
		var url="resources/"+pages+".print?type="+type;
		url += "&dateFrom="+dateFrom+"&dateTo="+dateTo+"&flag=2&ysdm="+this.ysdm
			+"&ksdm="+this.ksdm+"&ksscfs="+this.ksscfs
		if(type=="3"){
				...
				if (iframe.attachEvent) {
					iframe.attachEvent("onload", function() {
								_ctx .simplePrintMask(iframe,
												"AntimicrobialDrugClinicDoctorUseDetailsPrintView");
							});
				} else {
					iframe.onload = function() {
						_ctx.simplePrintMask(iframe, "AntimicrobialDrugClinicDoctorUseDetailsPrintView");
					};
				}
			},
			/**
