...
phis.application.cfg.script.ConfigInventoryInEditorList = function(cfg) {
	...
	phis.application.cfg.script.ConfigInventoryInEditorList.superclass.constructor
			.apply(this, [cfg])
	this.on("afterCellEdit", this.afterGridEdit, this);
}
Ext.extend(phis.application.cfg.script.ConfigInventoryInEditorList,
		phis.script.EditorList, {
			initPanel : function(sc) {
				var grid = phis.application.cfg.script.ConfigInventoryInEditorList.superclass.initPanel
						.call(this, sc)
			...
			doCreate : function(item, e) {
				phis.application.cfg.script.ConfigInventoryInEditorList.superclass.doCreate
						.call(this);
				var store = this.grid.getStore();
				var rowItem = store.data.itemAt(0);
				if (store.data.length != 1) {
					rowItem = store.data.itemAt((store.data.length) - 1);
				}
				rowItem.set('WZJG', this.wzjg);
				rowItem.set('LSJG', this.lsjg);
			...
			afterGridEdit : function(it, record, field, v) {
				if (it.id == "WZSL" || it.id == "WZJG") {
					...
					var v = parseFloat(record.get("WZSL") * record.get("WZJG"))
							.toFixed(2);
					var v1 = parseFloat(record.get("WZSL") * record.get("LSJG"))
							.toFixed(2);
					record.set("WZJE", v);
					record.set("LSJE", v1);
				}
