...
phis.application.cfg.script.ConfigDepartmentsInModule = function(cfg) {
	cfg.width = 1024;
	cfg.height = 500;
	phis.application.cfg.script.ConfigDepartmentsInModule.superclass.constructor.apply(
			this, [cfg]);
}
Ext.extend(phis.application.cfg.script.ConfigDepartmentsInModule,
			...
			doSave : function() {
				...
				var whatsthetime = function() {
					...
					for (var i = 0; i < n; i++) {
						...
						if (_ctr.list.store.getAt(i).data["KSDM"] != undefined
								&& _ctr.list.store.getAt(i).data["KSDM"] != "") {
							if (_ctr.list.store.getAt(i).data["WZJE"] == undefined
									|| _ctr.list.store.getAt(i).data["WZJE"] == 0) {
								MyMessageTip.msg("提示", "第" + (i + 1)
												+ "行物资金额不能为空!", true);
					...
					var r = phis.script.rmi.miniJsonRequestSync({
								...
								serviceAction : "saveDepartmentsIn",
								body : formdate,
								listBody : cskc
							});
