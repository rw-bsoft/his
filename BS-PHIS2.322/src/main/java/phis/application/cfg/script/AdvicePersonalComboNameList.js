...
phis.application.cfg.script.AdvicePersonalComboNameList = function(cfg) {
	cfg.disablePagingTbr = false;
	cfg.showRowNumber = true;
	var sslbValue = cfg.cnds;
	if (sslbValue == 1) {
		cfg.cnds = [
				'and',
				[
	...
	phis.application.cfg.script.AdvicePersonalComboNameList.superclass.constructor
			.apply(this, [cfg])
	this.on("beforeRemove", this.onBeforeRemove, this);
	this.on("remove", this.onRemove, this);
}
Ext.extend(phis.application.cfg.script.AdvicePersonalComboNameList,
		phis.script.SimpleList, {
			openModule : function(cmd, r, xy) {
				phis.application.cfg.script.AdvicePersonalComboNameList.superclass.openModule
						.call(this, cmd, r, xy)
			...
			onReady : function() {
				phis.application.cfg.script.AdvicePersonalComboNameList.superclass.onReady
						.call(this);
			...
			doAdd : function() {
				if (!this.opener.lastKSDM) {
					MyMessageTip.msg("提示", "请选择科室！", true);
					return;
				}
				this.fireEvent("beforeclose", this);
				this.prescriptionSetModule = this.createModule(
						"prescriptionSetModule", this.addRef);
				this.prescriptionSetModule.on("save", this.onSave, this);
				this.prescriptionSetModule.ZTLB = this.ZTLB;// 下拉框值
				this.prescriptionSetModule.SSLB = this.SSLB;// 下拉框值
				this.prescriptionSetModule.KSDM = this.opener.lastKSDM;// 下拉框值
				this.prescriptionSetModule.ZTLBKEY = this.ZTLBKEY;// 下拉框键
				this.prescriptionSetModule.SSLBKEY = this.SSLBKEY;// 下拉框键
				this.prescriptionSetModule.initPanel();
				var win = this.prescriptionSetModule.getWin();
				win.add(this.prescriptionSetModule.initPanel());
				win.show();
				win.center();
				this.prescriptionSetModule.doNew();
			},
			// 单击时改变作废按钮
			onRowClick : function() {
				var r = this.getSelectedRecord();
