...
phis.application.sup.script.InventoryInEjEditorList = function(cfg) {
	cfg.selectOnFocus = true;
	cfg.autoLoadData = false;
	cfg.disablePagingTbr = true;
	phis.application.sup.script.InventoryInEjEditorList.superclass.constructor.apply(
			this, [cfg])
	this.on("afterCellEdit", this.onAfterCellEdit, this);
}
Ext.extend(phis.application.sup.script.InventoryInEjEditorList,
			...
			onAfterCellEdit : function(it, record, field, v) {
				...
				if (it.id == "PCSL") {
					...
					if (((v != null && v != "") || v == 0)) {
						...
						lsje = (v * record.get("WZJG")).toFixed(2);
					}
