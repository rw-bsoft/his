...
phis.application.sup.script.InventoryInModule = function(cfg) {
	this.width = 1024;
	this.height = 500;
	cfg.listIsUpdate = true;
	phis.application.sup.script.InventoryInModule.superclass.constructor.apply(
			this, [ cfg ]);
}
Ext
		.extend(
				phis.application.sup.script.InventoryInModule,
				phis.script.SimpleModule,
				{
					...
					doCallIn : function() {
						if (this.list.store.getCount() > 0) {
							var module = this.createModule("getSUP11010103",
									this.refMode);
							this.module = module;
							if (this.list) {
