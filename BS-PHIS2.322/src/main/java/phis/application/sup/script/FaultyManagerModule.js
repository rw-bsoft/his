...
phis.application.sup.script.FaultyManagerModule = function(cfg) {
	cfg.width = window.screen.width / 2;
	phis.application.sup.script.FaultyManagerModule.superclass.constructor
			.apply(this, [cfg]);
	this.on('doSave', this.doSave, this);
}
Ext.extend(phis.application.sup.script.FaultyManagerModule,
		phis.script.SimpleModule, {
			...
			initPanel : function(sc) {
				...
				var radioGroup = new Ext.form.RadioGroup({
					height : 20,
					width : 740,
					id : 'faultyStautsRadio',
					name : 'faultyStautsRadio', // 后台返回的JSON格式，直接赋值
										...
										[
												...
												[
														...
														[
																'i',
																this.mainApp['phis'].treasuryId]]],
										['eq', ['$', 'DJZT'], ['i', this.uCheckInList.statusRadio.getValue().inputValue]]];
			...
			getUList : function() {
				this.uCheckInList = this.createModule(
						"faultyUndeterminedCheckInList", this.refUList);
				...
				this.uCheckInList.parentRadioGroup=this.radioGroup;
				this.uCheckInList.requestData.pageNo = 1;
