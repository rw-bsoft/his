...
phis.application.sup.script.InventoryInKSModule = function(cfg) {
	this.width = 1024;
	this.height = 500;
	cfg.listIsUpdate = true;
	phis.application.sup.script.InventoryInKSModule.superclass.constructor.apply(this,
			[cfg]);
}
Ext.extend(phis.application.sup.script.InventoryInKSModule, phis.script.SimpleModule,
		{
			initPanel : function() {
				if (this.panel) {
					return this.panel;
				...
				if (this.form.getFormData()) {
					...
					var whatsthetime = function() {
						for (var i = 0; i < count; i++) {
							body["WL_LRMX"].push(_ctr.list.store.getAt(i).data);
						}
			...
			changeButtonState : function(state) {
				...
				if (state == "new") {
					this.setButtonsState(["create", "save", "callIn", "print",
									"close"], true);
				}
				if (state == "save") {
					this.setButtonsState(["create", "save", "callIn", "subit",
									"print", "close"], true);
				}
				if (state == "subt") {
					this.setButtonsState(["create", "close"], true);
			...
			doCallIn : function() {
				var module = this.createModule("getSUP11010203", "phis.application.sup.SUP/SUP/SUP11010203");
