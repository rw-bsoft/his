...
phis.application.sup.script.InventoryModule = function(cfg) {
	this.width = 1024;
	this.height = 500;
	cfg.listIsUpdate = true;
	phis.application.sup.script.InventoryModule.superclass.constructor.apply(
			this, [ cfg ]);
}
Ext
		.extend(
				phis.application.sup.script.InventoryModule,
				phis.script.SimpleModule,
				{
					...
					changeButtonState : function(state) {
						...
						if (state == "save") {
							this.setButtonsState([ "save", "verify", "print",
									"close" ], true);
						}
						if (state == "verified") {
						...
						if (state == "commited") {
							this.setButtonsState([ "print", "close" ], true);
							this.list.setButtonsState([ "create", "remove" ],
									false);
						}
		            ...
		            doPrint : function() {
		                var module = this.createModule("inventoryPrint",
