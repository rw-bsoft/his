...
phis.application.sup.script.InventoryInTZEditorList = function(cfg) {
	cfg.selectOnFocus = true;
    cfg.disablePagingTbr = true;
	cfg.autoLoadData = false;
	phis.application.sup.script.InventoryInTZEditorList.superclass.constructor.apply(
			this, [cfg])
}
Ext.extend(phis.application.sup.script.InventoryInTZEditorList,
				...
				if (this.op) {
					if (this.op == "update") {
						if (this.requestData.cnd) {
							...
							this.requestData.serviceAction = "getLRMXTZInfo";
						}
					}
				}
				if (this.store) {
