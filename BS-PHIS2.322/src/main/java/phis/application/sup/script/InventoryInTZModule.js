...
phis.application.sup.script.InventoryInTZModule = function(cfg) {
	this.width = 1024;
	this.height = 500;
	cfg.listIsUpdate = true;
	phis.application.sup.script.InventoryInTZModule.superclass.constructor
			.apply(this, [ cfg ]);
}
Ext
		.extend(
				phis.application.sup.script.InventoryInTZModule,
				phis.script.SimpleModule,
				{
						...
						if (this.form.getFormData()) {
							...
							var whatsthetime = function() {
								for ( var i = 0; i < count; i++) {
									if (_ctr.GLFS == 3) {
										if (_ctr.list.store.getAt(i).data.PDBZ == undefined) {
					...
					doCallIn : function() {
						var module = this.createModule("getSUP11010303",
								"phis.application.sup.SUP/SUP/SUP11010303");
