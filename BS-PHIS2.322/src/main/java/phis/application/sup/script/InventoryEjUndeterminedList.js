...
phis.application.sup.script.InventoryEjUndeterminedList = function(cfg) {
	cfg.width = 1024;
	cfg.height = 550;
	cfg.winState = "center";
	...
	phis.application.sup.script.InventoryEjUndeterminedList.superclass.constructor
			.apply(this, [cfg])
}
Ext.extend(phis.application.sup.script.InventoryEjUndeterminedList,
		phis.script.SimpleList, {
			onReady : function() {
				if (this.grid) {
					phis.application.sup.script.InventoryEjUndeterminedList.superclass.onReady
							.call(this);
					var startDate = "";// 开始时间
					var endDate = ""; // 结束时间
					if (this.dateFrom) {
			...
			getCndBar : function(items) {
				...
				this.statusRadio = new Ext.form.RadioGroup({
							height : 20,
							width : 100,
							id : 'inventoryEjUndeterminedListdjzt',
							name : 'inventoryEjUndeterminedListdjzt', // 后台返回的JSON格式，直接赋值
							value : "0",
							items : [{
										boxLabel : '制单',
										name : 'inventoryEjUndeterminedListdjzt',
										inputValue : 0
									}, {
										boxLabel : '审核',
										name : 'inventoryEjUndeterminedListdjzt',
				...
				this.dateFrom = new Ext.form.DateField({
							id : 'inventoryEjUndeterminedListdateFrom',
							name : 'inventoryEjUndeterminedListdateFrom',
							value : dateFromValue,
							width : 100,
							allowBlank : false,
							altFormats : 'Y-m-d',
				...
				this.dateTo = new Ext.form.DateField({
							id : 'inventoryEjUndeterminedListdateTo',
							name : 'inventoryEjUndeterminedListdateTo',
							value : new Date(),
							width : 100,
							allowBlank : false,
							altFormats : 'Y-m-d',
			...
			doAdd : function() {
				...
				this.inventoryEjModule = this.oper.createModule(
						"inventoryEjModule", this.addRefkc);
				this.inventoryEjModule.on("save", this.onSave, this);
				this.inventoryEjModule.initPanel();
				var win = this.inventoryEjModule.getWin();
				win.add(this.inventoryEjModule.initPanel());
				win.show()
				win.center()
				if (!win.hidden) {
					this.inventoryEjModule.op = "create";
					this.inventoryEjModule.type = "normal";
					this.inventoryEjModule.doNew();
				}
			},
			doUpd : function() {
				...
				var win = this.inventoryEjModule.getWin();
				win.add(this.inventoryEjModule.initPanel());
				this.inventoryEjModule.doUpd();
				var djzt = r.data.DJZT;
				if (djzt == 0) {
					this.inventoryEjModule.changeButtonState("save");
				} else if (djzt == 1) {
					this.inventoryEjModule.changeButtonState("verified");
				}
				win.show()
				win.center()
				if (!win.hidden) {
