...
phis.application.med.script.ZYListForm = function(cfg) {
	cfg.showButtonOnTop = false;
	cfg.colCount = 4;
	cfg.autoLoadData = false;
	cfg.loadServiceId = "medicalTechnicalSectionService";
	cfg.loadAction = "getZYForm";
	phis.application.med.script.ZYListForm.superclass.constructor.apply(this, [cfg])
	this.on("loadData",this.onLoadData,this);
}

Ext.extend(phis.application.med.script.ZYListForm, phis.script.TableForm, {
...
//		}
		...
		phis.script.rmi.jsonRequest(Ext.apply({
			...
			onReady : function(){
				phis.application.med.script.ZYListForm.superclass.onReady.call(this);
				var form = this.form.getForm();
				var mzhm = form.findField("ZYHM");
				mzhm.un("specialkey", this.onFieldSpecialkey, this)
				mzhm.on("specialkey", function(mzhm, e) {
							var key = e.getKey()
							if (key == e.ENTER) {
								this.doMzhmEnter(mzhm);
							}
						}, this)
