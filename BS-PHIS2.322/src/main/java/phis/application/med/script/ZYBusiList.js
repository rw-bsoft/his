...
phis.application.med.script.ZYBusiList = function(cfg) {
...
//	this.serviceId = "wardPatientManageService";
	phis.application.med.script.ZYBusiList.superclass.constructor.apply(this,[cfg])
	this.on("loadData",this.onLoadData,this);
}
var recordIds = new Array();
Ext.extend(phis.application.med.script.ZYBusiList, phis.script.EditorList,
		{
			...
			rowSelect : function(e, rowIndex, record) {
				...
				this.dList.loadDatas(recordIds);
			},
			...
			init: function(){
					...
					this.serverParams = {serviceAction:"getZYList",zxks:this.mainApp['phis'].MedicalId,jgid:this.mainApp['phisApp'].deptId}
					phis.application.med.script.ZYBusiList.superclass.init.call(this)
			},
			initPanel : function(sc) {
				var grid = phis.application.med.script.ZYBusiList.superclass.initPanel.call(this, sc);
...
//				this.dList.requestData["serviceAction"] = "getZyList_Proj";
//				this.dList.requestData["jgid"]=this.mainApp['phisApp'].deptId;
//				this.dList.requestData["yjxh"] = yjxh ;
//				this.dList.loadData();
				/**
				 * 2013-06-20 gejj start 修改bug1775医技管理--医技项目执行：已收费的记录，【修改】按钮需不可用。
				 * 修改描述:在医嘱序号不为空并且大于0，说明是病区提交，则不能修改("修改"按钮禁用)
				 * 		    在医嘱序号为空，说明是医技增加，则可以修改("修改"按钮启用)
				 * */
				var yzxh = r.get("YZXH");
				if(yzxh && yzxh > 0){
...
//					this.opener.opener.panel.getTopToolbar().items.items[3].setDisabled(true);
			...
			getDList : function(){
				var module = this.createModule("phis.application.med.script.MEDDList",
						'phis.application.med.MED/MED/MED010104');
				if (module) {
			...
			getCM : function(items) {
				var cm = phis.application.med.script.ZYBusiList.superclass.getCM.call(this, items);
				var sm = this.sm;
