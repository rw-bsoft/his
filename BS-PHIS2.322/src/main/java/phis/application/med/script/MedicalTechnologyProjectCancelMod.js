...
$import("phis.script.SimpleModule", "phis.script.widgets.MyMessageTip",
		...
		phis.application.med.script.MedicalTechnologyProjectCancelMod = function(
				cfg) {
			phis.application.med.script.MedicalTechnologyProjectCancelMod.superclass.constructor
					.apply(this, [ cfg ]);
		},

		Ext.extend(
				phis.application.med.script.MedicalTechnologyProjectCancelMod,
				phis.script.SimpleModule, {
					...
					onQueryProject : function() {
						...
						if (r == null) {
							this.dowlist.reLoadData(0);
							return
						}
						this.dowlist.reLoadData(r.get("YJXH"));
					},
					doCancel:function(){
						this.uplist.cancel();
					}
