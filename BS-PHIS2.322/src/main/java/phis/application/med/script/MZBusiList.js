...
phis.application.med.script.MZBusiList = function(cfg) {
...
//	this.serviceId = "wardPatientManageService";
	phis.application.med.script.MZBusiList.superclass.constructor.apply(this,[cfg])
	this.on("loadData",this.onLoadData,this);
}
var recordIds = new Array();
Ext.extend(phis.application.med.script.MZBusiList, phis.script.EditorList,
		{
			...
			init: function(){
					...
					this.serverParams = {serviceAction:"getMzList",zxks:this.mainApp['phis'].MedicalId,jgid:this.mainApp['phisApp'].deptId}
					phis.application.med.script.MZBusiList.superclass.init.call(this)
			},
			initPanel : function(sc) {
				var grid = phis.application.med.script.MZBusiList.superclass.initPanel.call(this, sc);
...
//				this.dList.loadData();
				 ...
				 * */
				var fphm = r.get("FPHM");
				if(fphm){//发票号码不为空，说明已经收费过，则不能修改("修改"按钮禁用)
...
//					this.opener.opener.panel.getTopToolbar().items.items[3].setDisabled(false);
			...
			getDList : function(){
				var module = this.createModule("phis.application.med.script.MEDDList",
						'phis.application.med.MED/MED/MED010103');
				if (module) {
...
//				var cm = phis.application.med.script.MZBusiList.superclass.getCM.call(this, items);
//				var sm = this.sm;
