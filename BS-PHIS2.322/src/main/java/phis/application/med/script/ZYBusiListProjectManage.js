...
phis.application.med.script.ZYBusiListProjectManage = function(cfg) {
	this.width = 960;
	this.height = 600;
	this.modal = true
	phis.application.med.script.ZYBusiListProjectManage.superclass.constructor.apply(
			this, [cfg]);
...
Ext.extend(phis.application.med.script.ZYBusiListProjectManage,
		phis.script.SimpleModule, {
			...
			getForm : function() {
				var formModule = this.createModule("ZYListForm",
						this.ZYListForm);
			...
			getList : function() {
				var listModule = this.createModule("ZYProjectEditorList",
						this.ZYProjectEditorList);
			...
			doSave : function() {
				...
				phis.script.rmi.jsonRequest({
							...
							serviceAction : "saveZYYJAndProject",
							body : {
			...
			doNew : function() {
				...
				btn3[0].enable();
				this.op = "create";
				if (this.midiModules["ZYListForm"]) {
					this.midiModules["ZYListForm"].doNew();
					this.midiModules["ZYListForm"].XZPB = 1
				}
				if (this.midiModules["ZYProjectEditorList"]) {
					this.midiModules["ZYProjectEditorList"].doNew();
				}
			},
			doDelete : function() {
			...
			loadData : function() {
				...
				if (this.midiModules["ZYListForm"]) {
					this.midiModules["ZYListForm"].exContext = this.exContext;
					this.midiModules["ZYListForm"].XZPB = 0;
					this.midiModules["ZYListForm"].loadData();
				}
				if (this.midiModules["ZYProjectEditorList"]) {
					Ext
							.apply(
									this.midiModules["ZYProjectEditorList"].requestData,
									this.exContext.args["listRequestData"]);
					this.midiModules["ZYProjectEditorList"].loadData();
				}
			},
			doAdd : function() {
				if (this.formModule.data.ZYHM || this.formModule.queryData) {
					this.listModule.doCreate();
				}
			},
			doClear : function() {
