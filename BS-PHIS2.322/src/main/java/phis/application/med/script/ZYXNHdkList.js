...
phis.application.hos.script.ZYXNHdkList = function(cfg) {
	this.autoLoadData=false;
	Ext.apply(this,phis.script.SMCardReader);
	phis.application.hos.script.ZYXNHdkList.superclass.constructor
			.apply(this, [cfg]);
};

Ext.extend(phis.application.hos.script.ZYXNHdkList,
	...
	for (var i = 0; i < actions.length; i++) {
		...
		return buttons;
			...
			doCheck:function(cardid){
				...
				if(cardid.length >6){
					this.getWin().hide();
					this.fireEvent("zfnhdkreturn", cardid);
				}
			}
		});