...
phis.application.ccl.script.CheckApplyFeeDetails_KD_CIC = function(cfg) {
	cfg.autoLoadData = false;
	cfg.disablePagingTbr = true;
	phis.application.ccl.script.CheckApplyFeeDetails_KD_CIC.superclass.constructor
			.apply(this, [cfg]);
}
Ext.extend(phis.application.ccl.script.CheckApplyFeeDetails_KD_CIC,
		phis.script.SimpleList, {
			...
			doCommit : function() {
				...
				if(Ext.getCmp("sslx").getValue().inputValue==1&&form.findField("XL").getValue()==""){
					MyMessageTip.msg("提示", "心电图检查单心率不能为空", true);
					return;
				}
				if(Ext.getCmp("sslx").getValue().inputValue==1&&form.findField("XLV").getValue()==""){
