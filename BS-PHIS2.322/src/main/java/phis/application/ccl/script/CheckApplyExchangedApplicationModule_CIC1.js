...
phis.application.ccl.script.CheckApplyExchangedApplicationModule_CIC1 = function(
		cfg) {
	cfg.printurl = util.helper.Helper.getUrl();
	phis.application.ccl.script.CheckApplyExchangedApplicationModule_CIC1.superclass.constructor
			.apply(this, [cfg])
}
Ext.extend(
		phis.application.ccl.script.CheckApplyExchangedApplicationModule_CIC1,
		phis.script.SimpleModule, {
			...
			getTbar : function() {
				...
				// });
				/*
				 * tbar.push({ xtype : "button", text : "检查结果查询", iconCls :
				 * "query", scope : this, handler : this.doResult });
				 */
				return tbar;
			},
			...
			/*
			 * doResult : function(){ var list =
			 * this.checkApplyExchangedApplicationList; var yjxh =
			 * list.getSelectedRecord().data.SQDH+"1"; var jgid =
			 * this.mainApp['phis'].phisApp.deptId; jgid =
			 * jgid.substring(jgid.length-2,jgid.length);
			 * //window.open("http://192.168.10.3/Response/HisRedirect.asp?LoginId=HIS&Password=123456&HisId="+yjxh+"&kind=3
			 * ");
			...
			doPrint : function() {
				...
				var mzhm = this.opener.opener.exContext.empiData.MZHM;
				 ...
				 * LODOP.ADD_PRINT_BARCODE(40,930,138,38,"128Auto",jgid+sqdh+"2");
				 */
				...
				LODOP.ADD_PRINT_BARCODE(80, "66.5%", 138, 38, "128Auto", mzhm);
				//
				// 直接打印
				LODOP.PRINT();
				// 区分针式打印机和激光打印机
				/*
				 * var fileList = []; var queryFile = {intOrient:"1",
				 * PageWidth:"210mm", PageHeight:"140mm"}
				 * $FSActiveXObjectUtils.initHtmlElement();//初始化 var fsoObj =
				 * $FSActiveXObjectUtils.getObject(); try { var reader =
				 * fsoObj.openTextFile("C:\\YSDY.txt", 1);
				 * while(!reader.AtEndofStream) { var content =
				 * reader.readline(); if(content.indexOf('#')>=0){ continue; }
				 * fileList.push(content.split("=")); } reader.close(); for(var
				 * i = 0 ; i < fileList.length ; i ++){ for(var key in
				 * queryFile){ if(fileList[i][0]==key){
				 * queryFile[key]=fileList[i][1] } } } var tmz = "70%" var
				 * lodopLeft = "0"; var PRINT_PAGE_PERCENT = "Full-Page";
				 * if(queryFile.intOrient==1){ lodopLeft = "0"; }
				 * if(queryFile.intOrient==3){ queryFile.intOrient=1; lodopLeft =
				 * "31mm" tmz = "60%"; PRINT_PAGE_PERCENT = "Full-Width"; } var
				 * marginTop = queryFile.marginTop ? queryFile.marginTop : "0";
				 * LODOP.SET_PRINT_PAGESIZE(queryFile.intOrient,queryFile.PageWidth,queryFile.PageHeight,"");
				 * LODOP.ADD_PRINT_HTM(marginTop,lodopLeft,queryFile.PageWidth,"100%",util.rmi.loadXML({url:url,httpMethod:"get"}));
				 * LODOP.SET_PRINT_MODE
				 * ("PRINT_PAGE_PERCENT",PRINT_PAGE_PERCENT);
				 * LODOP.ADD_PRINT_BARCODE(40,tmz,138,38,"128Auto",jgid+sqdh+"1");
				 * LODOP.SET_PRINTER_INDEX(-1); LODOP.PREVIEW(); }catch (e) {
				 * LODOP.SET_PRINT_PAGESIZE("2",0,0,"A4");
				 * LODOP.ADD_PRINT_HTM("0","50%","50%","100%",util.rmi.loadXML({url:url,httpMethod:"get"}));
				 * LODOP.SET_PRINT_MODE ("PRINT_PAGE_PERCENT","Full-Height");
				 * LODOP.ADD_PRINT_BARCODE(80,930,138,38,"128Auto",jgid+sqdh+"1");
				 * LODOP.SET_PRINTER_INDEX(-1); //预览 LODOP.PREVIEW(); }
				 */
			},
			getExecJs : function() {
