...
phis.application.pha.script.PharmacyCheckOutList = function(cfg) {
	cfg.width = 1024;
	cfg.height = 550;
	cfg.cnds = ['eq', ['$', 'CKPB'], ['i', 1]];
	cfg.modal = this.modal = true;
	cfg.autoLoadData = false;
	phis.application.pha.script.PharmacyCheckOutList.superclass.constructor.apply(this,
			[cfg])
}
Ext.extend(phis.application.pha.script.PharmacyCheckOutList, phis.script.SimpleList,
		{
			// 生成日期框
...
//				this.simple = simple;
			...
			doCndQuery : function(button, e, addNavCnd) {
						...
						if (r.code > 300) {
							...
							cnd = [
									...
									[
											'ge',
											['$',
													"str(CKRQ,'yyyy-mm-dd hh24:mi:ss')"],
											['s', dates[0]]],
									[
											'le',
											['$',
				...
				if (this.checkOutWayValue) {
					cnd = [
							'and',
							['eq', ['$', 'CKFS'], ['i', this.checkOutWayValue]],
							cnd];
				}
			...
			doPrint : function() {
				var module = this.createModule("yesstorehouselistprint",
						this.refYesPharmacyOutPrint)
				var r = this.getSelectedRecord()
				if (r == null) {
