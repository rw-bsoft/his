...
phis.application.pha.script.PharmacyCheckOutDetailForm = function(cfg) {
	cfg.showButtonOnTop = false;
	phis.application.pha.script.PharmacyCheckOutDetailForm.superclass.constructor.apply(
			this, [cfg])
			this.on("loadData",this.onLoadData,this);
}
Ext.extend(phis.application.pha.script.PharmacyCheckOutDetailForm,
		phis.script.TableForm, {
			onReady : function() {
				phis.application.pha.script.PharmacyCheckOutDetailForm.superclass.onReady
						.call(this);
			...
			// 新增页面打开出库方式选中带过来的值
			fillCkfs : function() {
				if (this.checkOutWayValue && this.op == "create") {
					var form = this.form.getForm();
					form.findField("CKFS").setValue(this.checkOutWayValue);
				}
			},
			// 由于出库方式要根据药房识别(动态)特殊处理,未在框架上找到合适的处理方法,故修改创建dic方法
			createDicField : function(dic) {
				if(dic.id=="drugStorage"){
				dic.filter="['eq',['$','item.properties.YFSB'],['l',"+parseInt(this.mainApp['phis'].pharmacyId)+"]]";
				}
			...
			onLoadData:function(){
				if(this.isRead){
			this.form.getForm().findField("CKBZ").setDisabled(true) ;}
			else{
			this.form.getForm().findField("CKBZ").setDisabled(false) ;
			}
			},
