...
phis.application.pha.script.PharmacyMySimpleRightList = function(cfg) {
	cfg.width = 1024;
	cfg.height = 550;
	...
	phis.application.pha.script.PharmacyMySimpleRightList.superclass.constructor.apply(this,
			[cfg])
	this.on("beforeclose", this.doCancel, this);
}
Ext.extend(phis.application.pha.script.PharmacyMySimpleRightList, phis.script.SimpleList, {// 生成日期框
	...
	doCndQuery : function(button, e, addNavCnd) {
		...
		if (simple) {
			...
			if (stroeDate != null && stroeDate != "") {
				...
				var prior_begin = this.getLastMonth(dateSplit[0], dateSplit[1]);// 界面财务月份前一个月的月初时间
				var body = {};
				body["begin"] = begin;
				body["end"] = end;
