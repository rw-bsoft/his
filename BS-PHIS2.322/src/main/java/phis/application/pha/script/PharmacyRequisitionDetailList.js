...
phis.application.pha.script.PharmacyRequisitionDetailList = function(cfg) {
	...
	phis.application.pha.script.PharmacyRequisitionDetailList.superclass.constructor
			.apply(this, [cfg])

}
Ext.extend(phis.application.pha.script.PharmacyRequisitionDetailList,
		phis.application.pha.script.PharmacyMySimpleDetailList, {
			setBackInfo : function(obj, record) {
				...
				var body = {
					"YPXH" : ypxh,
					"YPCD" : ypcd,
					"MBBZ" : record.get("YFBZ"),
					"YPMC" : record.get("YPMC")
				};
				...
				if (ret.code > 300) {
					 ...
					 * else { if(ret.json.body.code != 200) {
					 * MyMessageTip.msg("提示", ret.json.body.msg, true); return; }
					 * else {
					 *  } }
					 */
				_ctx = this;
				var kcsl = ret.json.body.KCSL;
				if (kcsl == null || kcsl == undefined || kcsl == "") {
					kcsl = 0;
				}
				var rowItem = griddata.itemAt(row);
				rowItem.set('KCSL', kcsl);
			...
			onAfterCellEdit : function(it, record, field, v) {
				...
				if (it.id == "YPSL") {
					...
					if (((v != null && v != "") || v == 0)
							...
							&& record.get("YPXH") != "") {
						var tkcsl = this.getTargetYPSL(record.get("YPXH"),
								record.get("YPCD"));
						if (v > tkcsl) {
			...
			getTargetYPSL : function(ypxh, ypcd) {
				var body = {
					"YPXH" : ypxh,
					"YPCD" : ypcd,
					"MBYF" : this.MBYF
				...
				var ret = phis.script.rmi.miniJsonRequestSync({
							serviceId : this.serviceId,
							serviceAction : this.queryTarHouseStore,
							body : body
						});
				if (ret.code > 300) {
					this.processReturnMsg(ret.code, ret.msg,
							this.queryTargetYPSL);
					return;
				}
				return ret.json.body.KCSL;
			},
			onStoreLoadData : function(store, records, ops) {
				...
				for (var i = 0; i < count; i++) {
					var body = {
						...
						"YPMC" : this.store.getAt(i).get("YPMC")
					};
					var ret = phis.script.rmi.miniJsonRequestSync({
								serviceId : this.serviceId,
								serviceAction : this.queryKcslActionId,
