...
public class FamilySickBedCostProcessingModel {
	protected BaseDAO dao;
	protected Logger logger = LoggerFactory
			.getLogger(TreatmentNumberModule.class);

	public FamilySickBedCostProcessingModel(BaseDAO dao) {
		this.dao = dao;
	}
	...
	public void doQueryItemInfo(Map<String, Object> req,
		...
		if ("ZYHM".equals(cndName)) {
			...
			sql = "select a.ZYH as ZYH, a.ZYHM as ZYHM,a.BRCH as BRCH,a.BRXM as BRXM,a.BRXB as BRXB,a.BRXZ as BRXZ,a.BRKS as BRKS,a.RYRQ as RYRQ,  a.CYRQ as CYRQ,a.ZSYS as ZSYS,a.BRBQ as BRBQ,a.JSCS as JSCS from JC_BRRY a where a.CYPB < 8 AND a.JGID = :manaUnitId AND a.ZYHM = :ZYHM ORDER BY a.ZYHM";
		}
	...
	public void doSaveCost(Map<String, Object> req, Map<String, Object> res,
		...
		try {
			...
			if (body.get("isZT") != "" && parseInt(body.get("isZT")) == 1) {
				...
				Map<String, Object> ZSYS = dao.doLoad(
						"SELECT ZSYS as ZSYS From JC_BRRY Where ZYH = :ZYH",
						parameters);
				String YSGH = "";
	...
	public void doQueryRefundInfo(Map<String, Object> req,
		...
		try {
			Map<String, Object> JSCS = dao
					.doLoad("SELECT Max(JSCS) as JSCS From JC_JCJS Where ZYH = :ZYH AND JGID = :JGID",
							jscsParameters);
		...
		StringBuffer hql = new StringBuffer();
		hql.append("select a.ZYH as ZYH,   a.FYXH as FYXH,  a.FYMC as FYMC,a.YPCD as YPCD,a.ZFBL as ZFBL,a.FYDJ as FYDJ,a.DZBL as DZBL,a.YZXH as YZXH,a.FYKS as FYKS,a.ZXKS as ZXKS,a.FYBQ as FYBQ,sum(a.FYSL) as FYSL,sum(a.ZFJE) as ZFJE,sum(a.ZJJE) as ZJJE,sum(a.ZLJE) as ZLJE,0.00 as TYSL from JC_FYMX  a");
	...
	public void doQueryRefundInfo1(Map<String, Object> req,
		...
		// try {
		// Map<String, Object> JSCS = dao
		// .doLoad("SELECT Max(JSCS) as JSCS From JC_JCJS Where  ZFPB=0 and JGID = :JGID  and ZYH = :ZYH",
		// JSCSparameter);
		// if (JSCS.get("JSCS") != null) {
