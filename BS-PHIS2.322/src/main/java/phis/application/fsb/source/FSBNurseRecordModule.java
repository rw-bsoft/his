...
public class FSBNurseRecordModule {
	protected Logger logger = LoggerFactory
			.getLogger(FSBNurseRecordModule.class);
	private BaseDAO dao;

	public FSBNurseRecordModule(BaseDAO dao) {
		this.dao = dao;
	}
	...
	public void doSaveHLJH(Map<String, Object> req, Map<String, Object> res,
		...
		parMap.put("HLHS", user.getUserId());
		...
		try {
			...
			if (parMap.containsKey("JLBH") && parMap.get("JLBH") != null
					&& parMap.get("JLBH") != "") {
				dao.doSave("update", BSPHISEntryNames.JC_HLJH, parMap, false);
			} else {
				dao.doSave("create", BSPHISEntryNames.JC_HLJH, parMap, false);
			}
		} catch (Exception e) {
			logger.error("护理记录模块，更新护理明细表单失败", e);
			throw new ModelDataOperationException(
	...
	public void doRemoveHLJH(Map<String, Object> req, Map<String, Object> res,
		...
		try {
			if (parMap.containsKey("JLBH") && parMap.get("JLBH") != null
				...
				dao.doRemove(jlbh, BSPHISEntryNames.JC_HLJH);
			}
		} catch (Exception e) {
	...
	public void doSaveHLJL(Map<String, Object> req, Map<String, Object> res,
		...
		parMap.put("JGID", jgid);
		parMap.put("HLHS", user.getUserId());
		Date rhsj = BSHISUtil.toDate((String) parMap.get("RHSJ"));
		Date chsj = BSHISUtil.toDate((String) parMap.get("CHSJ"));
		if (rhsj != null && chsj != null && rhsj.compareTo(chsj) == 1) {
			throw new ModelDataOperationException(
					ServiceCode.CODE_DATABASE_ERROR, "入户时间不允许早于出户时间");
		}
		try {
			// parMap.put("KSRQ", ksrq);
			// parMap.put("TZRQ", tzrq);
			...
			if (parMap.containsKey("JLBH") && parMap.get("JLBH") != null
					&& parMap.get("JLBH") != "") {
				dao.doSave("update", BSPHISEntryNames.JC_HLJL, parMap, false);
			} else {
				dao.doSave("create", BSPHISEntryNames.JC_HLJL, parMap, false);
			}
		} catch (Exception e) {
			logger.error("护理记录模块，更新护理明细表单失败", e);
			throw new ModelDataOperationException(
	...
	public void doRemoveHLJL(Map<String, Object> req, Map<String, Object> res,
		...
		try {
			if (parMap.containsKey("JLBH") && parMap.get("JLBH") != null
					&& parMap.get("JLBH") != "") {
				Long jlbh = Long.parseLong(parMap.get("JLBH") + "");
				dao.doRemove(jlbh, BSPHISEntryNames.JC_HLJL);
			}
		} catch (Exception e) {
			logger.error("删除护理记录失败", e);
			throw new ModelDataOperationException(
					ServiceCode.CODE_DATABASE_ERROR, "删除护理记录失败");
		}
