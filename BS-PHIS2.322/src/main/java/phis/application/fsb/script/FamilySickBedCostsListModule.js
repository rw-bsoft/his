...
phis.application.fsb.script.FamilySickBedCostsListModule = function(cfg) {
	cfg.width = 960;
	cfg.height = 500;
	cfg.modal = true;
	Ext.apply(this, app.modules.common)
	phis.application.fsb.script.FamilySickBedCostsListModule.superclass.constructor
			.apply(this, [ cfg ])
}
var radioValue = 1;
Ext
		.extend(
				phis.application.fsb.script.FamilySickBedCostsListModule,
				phis.script.SimpleModule,
				{
					...
					getList : function() {
						var module = this.createModule("refCostsListList",
								this.refList);
					...
					getForm : function() {
						var module = this.createModule("refCostsListForm",
								this.refForm);
						this.form = module;
					...
					createButton : function() {
						...
						for ( var i = 0; i < actions.length; i++) {
							...
							if (action.id == 'print' || action.id == 'cancel'
									|| action.id == 'refresh') {
								var btn = {};
								btn.accessKey = f1 + i;
								btn.cmd = action.id;
					...
					doWhole : function() {
						...
						var fyrq = this.fyrqCheckBox.getValue().length > 0 ? true
								: false;
						if (this.data.JSLX == 10) {
							this.list.requestData.cnd = [
									'and',
						...
						if (fyrq) {
							var startDate = (new Date(this.fyrqStart.getValue()))
									.format('Y-m-d')
							var endDate = (new Date(this.fyrqEnd.getValue())
									.add(Date.DAY, 1)).format('Y-m-d')
					...
					doPrint : function() {
						...
						var pages = 'phis.prints.jrxml.FamilySickBedCostMxgs';
						if (printStyle == '汇总格式') {
							pages = 'phis.prints.jrxml.FamilySickBedCostHzgs';
						}
						if (printStyle == '医嘱格式') {
							pages = 'phis.prints.jrxml.FamilySickBedCostYzgs';
						}

						// console.log(this.data);
						var JSCS = this.data.JSCS;
						var BRXM = this.data.BRXM;
						var ZYH = this.data.ZYH;
						var BRXZ_text = this.form.form.getForm().findField(
								"BRXZ").getRawValue();
//						var BRKS_text = this.form.form.getForm().findField(
//								"BRKS").getRawValue();
						var ZYHM = this.data.ZYHM;
						var RYRQ = this.data.KSRQ;
						var CYRQ = this.data.ZZRQ;
//						var CYRQ = this.form.form.getForm().findField("ZZRQ")
//								.getRawValue();
//						if (CYRQ) {
//							CYRQ = CYRQ.substring(0, 10) + ' 23:59:59';
//						}
...
//						var BRCH = this.data.BRCH;
						...
						var nowdatestr = nowdate.format('Y-m-d H:i:s');
						var endDate = nowdatestr;

						if (this.fyrqStart.getValue())
							startDate = (new Date(this.fyrqStart.getValue()))
									.format('Y-m-d H:i:s');
						if (this.fyrqEnd.getValue())
							endDate = (new Date(this.fyrqEnd.getValue()).add(
									Date.DAY, 1)).format('Y-m-d H:i:s');
...
//								BRCH : BRCH
						...
						LODOP
								.SET_PRINT_MODE("PRINT_PAGE_PERCENT",
										"Full-Width");
						// 预览LODOP.PREVIEW();
						// LODOP.PRINT_DESIGN();
						...
						// module.ZYH = ZYH;
						// module.BRXM =
						// encodeURIComponent(encodeURIComponent(BRXM));
						// module.BRXZ =
						// encodeURIComponent(encodeURIComponent(BRXZ));
						// module.DAYS = DAYS;
						// module.RYRQ = RYRQ;
						// module.ZFHJ = ZFHJ;
						// module.ZYHM = ZYHM;
						// module.BRKS =
						// encodeURIComponent(encodeURIComponent(BRKS));
						// module.BRCH = BRCH;
						// module.CYRQ = CYRQ;
						// module.FYHJ = FYHJ;
						// module.YPLX = YPLX;
					...
					doZfhj : function() {
						var res = util.rmi.miniJsonRequestSync({
							serviceId : "phis.familySickBedCostProcessingService",
					...
					doRefresh : function() {
						...
						this.form.form.getForm().findField("FYHJ").setValue(
								this.FYHJ);
						this.form.form.getForm().findField("ZFHJ").setValue(
								this.ZFHJ);
					}
				});