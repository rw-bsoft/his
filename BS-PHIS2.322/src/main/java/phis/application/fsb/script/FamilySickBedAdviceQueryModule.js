...
phis.application.fsb.script.FamilySickBedAdviceQueryModule = function(cfg) {
	cfg.width = "1024";
	cfg.modal = true;
	cfg.listServiceId = "familySickBedManageService";
	this.serviceId = "familySickBedManageService";
	phis.application.fsb.script.FamilySickBedAdviceQueryModule.superclass.constructor
			.apply(this, [cfg]);

	this.on("winShow", this.onWinShow, this);
}

Ext.extend(phis.application.fsb.script.FamilySickBedAdviceQueryModule,
		phis.script.SimpleModule, {
			...
			doAssignedEmpty : function() {
				...
				if (r) {
					...
					if (!r.get("TZSJ") || r.get("LSYZ")) {// 不存在单停时间
						MyMessageTip.msg("提示", "只有停嘱的长期医嘱才能取消停嘱!", true);
						return;
					}
			...
			getForm : function() {
				var module = this.createModule("fsbDoctorAdviceForm",
						this.refFamilySickBedAdviceForm);
				if (module) {
					this.form = module;
			...
			getList : function() {
				var module = this.createModule("fsbDoctorAdviceList",
						this.refFamilySickBedAdviceList);
				if (module) {
