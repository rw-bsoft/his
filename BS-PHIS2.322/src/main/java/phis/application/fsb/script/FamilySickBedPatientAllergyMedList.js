...
phis.application.fsb.script.FamilySickBedPatientAllergyMedList = function(cfg) {
	...
	this.removeRecords = [];
	phis.application.fsb.script.FamilySickBedPatientAllergyMedList.superclass.constructor
			.apply(this, [cfg])

	this.on("loadData", this.afterLoadData, this);
	this.on("beforeCellEdit", this.beforeGridEdit, this);
}
Ext.extend(phis.application.fsb.script.FamilySickBedPatientAllergyMedList,
		phis.script.EditorList, {
			initPanel : function(sc) {
				var grid = phis.application.fsb.script.FamilySickBedPatientAllergyMedList.superclass.initPanel
						.call(this, sc)
			...
			getSaveData : function() {
				...
				for (var i = 0; i < n; i++) {
					...
					r.data.BRID = this.opener.exContext.record.get("BRID")
					data.push(r.data)
				}
