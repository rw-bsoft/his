...
phis.application.fsb.script.FsbCostProcessingModule = function(cfg) {
	phis.application.fsb.script.FsbCostProcessingModule.superclass.constructor
			.apply(this, [cfg])
	this.on("choose", this.choose, this);
}

Ext.extend(phis.application.fsb.script.FsbCostProcessingModule,
		phis.script.SimpleModule, {
			...
			doSpecialkey : function(field, e) {
				...
				if (e.getKey() == Ext.EventObject.ENTER) { // 触发了listener后，如果按回车，执行相应的方法
					...
					if (name == "ZYHM" && !value) {
						form.findField("ZYHM").focus(false, 200);
					}
					if ((name == "ZYHM") && value) {
						this.getItemInfo(field);
					}
				...
				if (r.code > 300) {
					...
					if (!r.json.body) {
						if (field.getName() == "ZYHM") {
							Ext.Msg.alert("提示", "该家床号不存在!", function() {
					...
					form.findField("KSRQ").setValue(datas.KSRQ);
					if (datas.JSRQ) {
						form.findField("JSRQ").setValue(datas.JSRQ);
					} else {
						form.findField("JSRQ").setValue("");
					}
					if (datas.ZSYS) {
...
//					form.findField("ZXKS").setValue(datas.BRKS);
			...
			doVerification : function() {
				...
				var ls_ryrq = form.findField("KSRQ").getValue(); // string 类型
				var cyrq = form.findField("JSRQ").getValue();
				...
				if (fyrq < ryrq) {
					Ext.Msg.alert("提示", "费用日期不能小于开始日期！", function() {
								form.findField("KSRQ").focus(false, 100);
							});
					return;
				}
