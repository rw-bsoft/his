...
phis.application.fsb.script.FamilySickBedMedicalBackApplicationRightTopList = function(cfg) {
	cfg.gridDDGroup = "secondGridDDGroup";
	cfg.autoLoadData = true;
	phis.application.fsb.script.FamilySickBedMedicalBackApplicationRightTopList.superclass.constructor
			.apply(this, [cfg])
	this.on("loadData", this.onLoadData, this);
	this.on("afterCellEdit", this.onAfterCellEdit, this);
}

Ext.extend(phis.application.fsb.script.FamilySickBedMedicalBackApplicationRightTopList,
		phis.script.EditorList, {
			...
			loadData : function(zyh) {
				...
				this.requestData.cnd = [
						...
						['and', ['eq', ['$', 'a.ZYH'], ['d', zyh]],
								['eq', ['$', 'a.TJBZ'], ['i', 0]]]];
				phis.application.fsb.script.FamilySickBedMedicalBackApplicationRightTopList.superclass.loadData
						.call(this);
			},
			// 数据加载后改变按钮状态
