...
phis.application.fsb.script.FamilySickBedPatientPlanList = function(cfg) {
	cfg.disablePagingTbr = true;
	cfg.selectOnFocus = true;
	cfg.autoLoadData = false;
	cfg.showRowNumber = true;
	cfg.menuDisabled = true;
	...
	phis.application.fsb.script.FamilySickBedPatientPlanList.superclass.constructor
			.apply(this, [cfg])
	this.on("beforeCellEdit", this.beforeGridEdit, this);
	this.on("afterCellEdit", this.afterGridEdit, this)
}
Ext.extend(phis.application.fsb.script.FamilySickBedPatientPlanList,
		phis.script.EditorList, {
			initPanel : function(sc) {
				var grid = phis.application.fsb.script.FamilySickBedPatientPlanList.superclass.initPanel
						.call(this, sc);
				...
				sm.onEditorKey = function(field, e) {
					...
					if (k == e.ENTER) {
						...
						if (e.shiftKey) {
							...
							var xmlx = g.getStore().getAt(ed.row).get("XMLX");
			...
			doInsert : function(item, e, autoNewGroup) {// 当前记录后插入一条记录
				...
				if (selectdRecord) {
					...
					if ((selectdRecord.get("XMBH") == null
							...
							.get("XMBH") == 0)
							&& !(selectdRecord.get("XMLX") == 3 && selectdRecord
									.get("XMMC"))) {
						selectRow = selectRow;
					} else {
			...
			doInsertAfter : function(item, e, newGroup) {// 当前记录后插入一条记录
				...
				if (maxIndex == 0 || newGroup) {// 处方的第一条记录或者自动新组
					...
					rowItem.set("GYTJ_text", upRowItem.get("GYTJ_text"));
					rowItem.set("XMLX", upRowItem.get("XMLX"));
					rowItem.set("ZYH", upRowItem.get("ZYH"));
					rowItem.set("JGID", upRowItem.get("JGID"));
					rowItem.set("KSSJ", upRowItem.get("KSSJ"));
					rowItem.set("JSSJ", upRowItem.get("JSSJ"));
				}
			...
			getRemoteTpl : function() {
				return '<tpl if="YPLX !== 0"><td width="18px" style="background-color:#deecfd">{numKey}.</td><td width="100px">{YPMC}</td><td width="70px">{YFGG}</td><td width="20px">{YPDW}</td><td width="80px">{CDMC}</td><td width="50px">{LSJG}</td><td width="50px">{KCSL}</td></tpl>'
			...
			doSave : function(item, e) {
				...
				for (var i = 0; i < n; i++) {
					...
					r.set("ZYH", this.opener.initDataId);
					r.set("JGID", this.mainApp.deptId);
					r.set("YSDM", this.mainApp.uid);
					var items = this.schema.items;
					var msg = "保存失败";
					if(r.get("XMLX")==2||r.get("XMLX")==3){
						r.set("YCJL",null);
						r.set("JLDW",null);
					}
					if (r.get("JLBH") == 0)
					...
					if (r.get("XMBH") == null) {
						...
						if (r.get("KSSJ") > r.get("JSSJ")) {
							MyMessageTip.msg("提示", "开始日期不能大于结束日期！错误行 "
											+ (i + 1) + " 。", true);
							return;
						}
						if (r.get("JLBH") == null) {
				...
				phis.script.rmi.jsonRequest({
							...
							MyMessageTip.msg("提示", "诊疗计划保存成功!", true);
							this.loadData();
						}, this)
			...
			doRemove : function() {
				...
				Ext.Msg.confirm("请确认", "确认删除名称为【" + r.get("XMMC") + "】的计划明细吗？",
						function(btn) {
							if (btn == 'yes') {
								this.processRemove();
							}
			...
			beforeGridEdit : function(it, record, field, value) {
				...
				if (it.id == "YCJL" || it.id == "SYPC" || it.id == "GYTJ") {
					...
					if (record.get("XMLX") == 2 && it.id != 'SYPC') {
						return false;
					}
					if (!record.get("XMBH")) {
						return false;
			...
			doPrint:function(){
				...
				var pages = "phis.prints.jrxml.FamilySickBedPatientPlan";
				var url = "resources/" + pages + ".print?silentPrint=1"
				url += "&temp="+ new Date().getTime()+ "&ZYH="+ this.opener.initDataId+ "&flag=true";
				var LODOP = getLodop();
