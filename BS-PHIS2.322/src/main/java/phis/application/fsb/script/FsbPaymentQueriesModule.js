...
phis.application.fsb.script.FsbPaymentQueriesModule = function(cfg) {
	this.exContext = {};
	phis.application.fsb.script.FsbPaymentQueriesModule.superclass.constructor.apply(this,
			[cfg]);
}

Ext.extend(phis.application.fsb.script.FsbPaymentQueriesModule, phis.script.SimpleModule, {
	initPanel : function() {
		if (this.panel) {
			return this.panel;
		}
	...
	createForm : function() {
		...
		var FKFScombox = util.dictionary.SimpleDicFactory.createDic(paymentdic)
		...
		if (JKRQFrom) {
			if (JKRQFrom.length != 10) {
				Ext.MessageBox.alert("提示", "缴款开始日期格式不对", function() {
							from.findField("JKRQFrom").focus(false, 100);
						}, this);
				return;
			}
...
//		var BRKSCnd = ['eq', ['$', "b.BRKS"], ['d', BRKS]];
		...
		var cnd = ['eq',['$', 'a.JGID'],['$','%user.manageUnit.id']];
...
//		}
		...
		this.listModule.requestData.pageNo=1;
	...
	doNew : function() {
		...
		var date = new Date().getDate() >= 10 ? (new Date().getDate()) : "0"
				+ (new Date().getDate())
