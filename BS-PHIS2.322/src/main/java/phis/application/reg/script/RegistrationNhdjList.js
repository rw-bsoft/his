...
phis.application.reg.script.RegistrationNhdjList = function(cfg) {
	this.autoLoadData=false;
	Ext.apply(this,phis.script.SMCardReader);
	phis.application.reg.script.RegistrationNhdjList.superclass.constructor
			.apply(this, [cfg]);
};

Ext.extend(phis.application.reg.script.RegistrationNhdjList,
...
//						MyMessageTip.msg("提示", "农合接口返回："+ret.json.RET, true);
					}
				}
//				this.unmask();
		},
...
//				MyMessageTip.msg("提示", "请选择一条记录！！！", true);
			...
			var ret = phis.script.rmi.miniJsonRequestSync({
				...
				if (ret.code > 300) {
					...
					this.refresh()
					this.getWin().hide();
					this.fireEvent("nhdkreturn", ret.json.body);
				}
		},
		//处理下数据
