...
public class HospitalizationCheckoutModel {
	protected BaseDAO dao;
	protected Logger logger = LoggerFactory
			.getLogger(HospitalizationCheckoutModel.class);

	public HospitalizationCheckoutModel(BaseDAO dao) {
		this.dao = dao;
	}
	...
	public Map<String, Object> doCreate_jzrb(Map<String, Object> body,
		...
		// 住院结算表单
		String ids_jsfp_hql = " SELECT a.ZYH as ZYH,a.JSCS as JSCS,a.JSLX as JSLX,a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.JKHJ as JKHJ,a.FPHM as FPHM,a.ZFPB as ZFPB,a.TPHM as TPHM FROM ZY_ZYJS a,ZY_BRRY b  WHERE a.JSLX<>4 and a.ZYH=b.ZYH and  a.CZGH =:czgh AND a.JGID = :jgid AND  a.JZRQ IS NULL AND a.JSRQ <  :adt_jzrq";
		// 住院缴款表单
		String ids_jksj_hql = "SELECT a.JKXH as JKXH,a.JKJE as JKJE,a.JKFS as JKFS,a.SJHM as SJHM,a.ZFPB as ZFPB,a.CZGH as CZGH  FROM ZY_TBKK a WHERE a.CZGH = :czgh AND a.JGID = :jgid AND a.JZRQ IS NULL AND JSXZ IS NOT NULL AND a.JKRQ < :adt_jzrq";
		// 住院结算作废表单
		String ids_jszf_hql = " SELECT distinct a.ZYH as ZYH,a.JSCS as JSCS,a.JSLX as JSLX,(-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,(-1*a.JKHJ) as JKHJ,a.FPHM as FPHM,a.ZFPB as ZFPB,a.TPHM as TPHM FROM ZY_ZYJS a ,ZY_JSZF  b,ZY_BRRY d WHERE a.JSLX<>4 and (a.ZYH=d.ZYH ) and ( a.ZYH  = b.ZYH  ) AND ( a.JSCS = b.JSCS ) AND (a.FPHM = b.FPHM) AND b.ZFGH = :czgh AND b.JGID =:jgid AND  b.JZRQ IS NULL AND b.ZFRQ <  :adt_jzrq";
		// 住院缴款作废表单
		String ids_jkzf_hql = " SELECT a.JKXH as JKXH, (-1*a.JKJE) as JKJE ,a.JKFS as JKFS,a.SJHM as SJHM,a.ZFPB as ZFPB,a.CZGH as CZGH FROM ZY_TBKK a ,ZY_JKZF b WHERE ( b.JKXH = a.JKXH )   AND   b.ZFGH = :czgh AND b.JGID = :jgid AND  b.JZRQ IS NULL AND b.ZFRQ <  :adt_jzrq";
		// 住院付款表单
		String ids_jsfp_fk_hql = "select a.ZYH as ZYH, a.JSCS as JSCS, a.FKFS as FKFS, a.FKJE as FKJE, a.FKHM as FKHM from ZY_FKXX a, ZY_ZYJS b where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS AND a.FPHM = b.FPHM AND b.CZGH = :czgh  AND b.JGID = :jgid AND b.JZRQ IS NULL AND b.JSRQ < :adt_jzrq";
		// 住院付款作废表单
		String ids_jszf_fk_hql = "select a.ZYH as ZYH, a.JSCS as JSCS, a.FKFS as FKFS, a.FKJE as FKJE, a.FKHM as FKHM from ZY_FKXX a, ZY_JSZF b where a.ZYH = b.ZYH and a.JSCS = b.JSCS AND a.FPHM = b.FPHM AND b.ZFGH = :czgh  AND b.JGID = :jgid AND b.JZRQ IS NULL AND b.ZFRQ < :adt_jzrq";
//		// 保险判别为“0”时的 收费统计
//		String ids_dbpb_hql = "select sum (FKJE) as FKJE from(" +
//				" select sum(-1*a.JKJE) as FKJE  from ZY_TBKK a left join ZY_JKZF b on b.JKXH = a.JKXH where b.ZFGH = :czgh AND b.JGID = :jgid  AND b.JZRQ IS NULL  AND b.ZFRQ < :adt_jzrq " +
//				" UNION all " +
//				" select  sum(a.JKJE) as JKJE  from ZY_TBKK a  where a.CZGH = :czgh  AND a.JGID = :jgid AND a.JZRQ IS NULL AND a.JKRQ < :adt_jzrq " +
//				" UNION all" +
//				"  select -sum( a.FKJE ) as FKJE from ZY_FKXX a  left join ZY_JSZF b  on a.ZYH = b.ZYH  and a.JSCS = b.JSCS  where b.ZFGH= :czgh  AND b.JGID = :jgid  AND b.JZRQ IS NULL  AND b.ZFRQ< :adt_jzrq " +
//				") ";
//		// 保险判别为“0”时的 付款方式统计
//		String ids_fkfs_hql="  select g.FKJE,g.FKFS,f.FKMC from (select sum(b.FKJE) as FKJE,b.fkfs as FKFS  from ZY_ZYJS a   left join ZY_FKXX b  on a.ZYH = b.ZYH   and a.JSCS = b.JSCS left join GY_BRXZ c  on a.BRXZ = c.BRXZ  left join  GY_FKFS d  on b.FKFS=d.FKFS where a.JSLX <> 4   AND a.CZGH= :czgh AND a.JGID = :jgid AND a.JZRQ IS NULL  AND a.JSRQ< :adt_jzrq AND c.DBPB='0' group by b.fkfs  ) g left join GY_FKFS f on g.FKFS=f.FKFS ";
//		// 保险判别为不“0”时的 收费统计
//		String ids_brxz_hql = " select f.BRXZ as BRXZ,f.FKJE as FKJE ,g.XZMC as XZMC  from ( select a.BRXZ BRXZ,sum(b.FKJE) as FKJE   from ZY_ZYJS a left join ZY_FKXX b  on a.ZYH=b.ZYH   and a.JSCS = b.JSCS   left join GY_BRXZ c on a.BRXZ=c.BRXZ where a.JSLX <> 4   AND a.CZGH= :czgh AND a.JGID = :jgid AND a.JZRQ IS NULL  AND a.JSRQ< :adt_jzrq AND c.DBPB !='0' group by a.BRXZ )f left join GY_BRXZ g on f.BRXZ=g.BRXZ";
		String ids_fkfs_hql = "select c.FKFS as FKFS,sum(c.FKJE) as FKJE,d.FKMC as FKMC from ("+
		"select a.FKFS as FKFS, (-1*a.FKJE) as FKJE from ZY_FKXX a, ZY_JSZF b where a.ZYH = b.ZYH and a.JSCS = b.JSCS AND a.FPHM = b.FPHM AND b.ZFGH = :czgh  AND b.JGID = :jgid AND b.JZRQ IS NULL AND b.ZFRQ < :adt_jzrq "+
		" union all "+
		"select a.FKFS as FKFS, a.FKJE as FKJE from ZY_FKXX a, ZY_ZYJS b where b.JSLX<>4 and a.ZYH = b.ZYH and a.JSCS = b.JSCS AND a.FPHM = b.FPHM AND b.CZGH = :czgh  AND b.JGID = :jgid AND b.JZRQ IS NULL AND b.JSRQ < :adt_jzrq"+
		" union all "+
		"SELECT a.JKFS as FKFS,a.JKJE as FKJE FROM ZY_TBKK a WHERE a.CZGH = :czgh AND a.JGID = :jgid AND a.JZRQ IS NULL AND JSXZ IS NOT NULL AND a.JKRQ < :adt_jzrq"+
		" union all "+
		"SELECT a.JKFS as FKFS,(-1*a.JKJE) as FKJE FROM ZY_TBKK a ,ZY_JKZF b WHERE ( b.JKXH = a.JKXH ) AND b.ZFGH = :czgh AND b.JGID = :jgid AND  b.JZRQ IS NULL AND b.ZFRQ <  :adt_jzrq "+
		...
		"SELECT a.FYHJ as FYHJ,a.ZFHJ as ZFHJ,a.BRXZ as BRXZ FROM ZY_ZYJS a WHERE a.JSLX<>4 and a.CZGH =:czgh AND a.JGID = :jgid AND  a.JZRQ IS NULL AND a.JSRQ <  :adt_jzrq and a.FYHJ<>a.ZFHJ"+
		" union all "+
		"SELECT (-1*a.FYHJ) as FYHJ,(-1*a.ZFHJ) as ZFHJ,a.BRXZ as BRXZ FROM ZY_ZYJS a ,ZY_JSZF b WHERE a.JSLX<>4 and ( a.ZYH  = b.ZYH  ) AND ( a.JSCS = b.JSCS ) AND a.FPHM=b.FPHM AND b.ZFGH = :czgh AND b.JGID =:jgid AND  b.JZRQ IS NULL AND b.ZFRQ <  :adt_jzrq and a.FYHJ<>a.ZFHJ"+
...
//				}
			...
			String ll_tjks_hql = "SELECT COUNT(*) as TJZS  FROM ZY_TBKK a,ZY_ZYJS b WHERE b.JSLX<>4 and (a.ZYH = b.ZYH)AND(a.JSCS = b.JSCS)AND(a.ZFPB = 0)AND(b.ZFPB = 0)AND(b.CZGH = :czgh)AND(b.JZRQ IS NULL)AND(b.JSRQ < :ldt_End)AND (a.JGID = b.JGID) AND (a.JGID = :jgid)";
...
//					 }else{
						 qtysFb = qtysFb +ids_fkfs.get(i).get("FKMC")+ ":"
								...
								+ " ";
//					 }
					 if("52".equals(ids_fkfs.get(i).get("FKFS")+"")){
						 wxhj = BSHISUtil.doublesum(wxhj,BSPHISUtil.getDouble(ids_fkfs.get(i).get("FKJE"),2));
					 }else if("53".equals(ids_fkfs.get(i).get("FKFS")+"")){
						 zfbhj = BSHISUtil.doublesum(zfbhj,BSPHISUtil.getDouble(ids_fkfs.get(i).get("FKJE"),2));
					 }
				 }
			}
//			//保险判别不为“0”
	...
	public boolean doWf_Check(Map<String, Object> body, Context ctx)
		...
		if (ls_idt_jzrq.split(" ")[0].equals(sdf.format(new Date()))) {
			try {
				...
				ll_Count = dao.doCount("ZY_TBKK", hql, map);
				if (ll_Count > 0) {
					return false;
				}
				hql = "ZFGH = :is_UserId AND JZRQ IS NULL AND ZFRQ > :idt_jzrq and JGID = :gl_jgid ";
				ll_Count = dao.doCount("ZY_JKZF", hql, map);
				if (ll_Count > 0) {
					return false;
				}
				hql = "CZGH = :is_UserId AND JZRQ IS NULL AND JSRQ > :idt_jzrq and JGID = :gl_jgid";
				ll_Count = dao.doCount("ZY_ZYJS", hql, map);
				if (ll_Count > 0) {
					return false;
				}
				hql = "ZFGH = :is_UserId AND JZRQ IS NULL AND ZFRQ > :idt_jzrq and JGID = :gl_jgid ";
				ll_Count = dao.doCount("ZY_JSZF", hql, map);
				if (ll_Count > 0) {
					return false;
				}
			} catch (PersistentDataOperationException e) {
...
//			Date jzrq = BSHISUtil.toDate((jzxx.get("JZRQ") + ""));
			...
			dao.doSave("create", BSPHISEntryNames.ZY_JZXX, jzxx, false);
			// 将作废发票1、作废收据2及退票号码3写入ZY_ZFPJ
			for (int i = 1; i < 4; i++) {
				int ll_pjlb = i;
				...
				for (int j = 1; j <= pjhms.size(); j++) {
					...
					// hql =
					// "INSERT INTO ZY_ZFPJ(JZRQ, CZGH , PJLB ,PJHM,JGID) values( :idt_jzrq, :userid, :ll_pjlb, :pjhm, :gl_jgid)";
					dao.doSave("create", BSPHISEntryNames.ZY_ZFPJ, zfpj, false);
				}
			}
...
//			String ls_idt_End = body.get("idt_End") + "";
			...
			ll_Count = dao.doCount("ZY_TBKK", hqlWhere,
					parameters);
			String hql_update = "";
			if (ll_Count > 0) {
				parameters.put("idt_jzrq", idt_jzrq);
				hql_update = "UPDATE ZY_TBKK SET JZRQ = :idt_jzrq WHERE CZGH =:userid AND JZRQ IS NULL AND JKRQ < :ldt_End  and JGID = :gl_jgid ";
			...
			ll_Count = dao.doCount("ZY_JKZF", hqlWhere,
					parameters);
			if (ll_Count > 0) {
				parameters.put("idt_jzrq", idt_jzrq);
				hql_update = "UPDATE ZY_JKZF SET JZRQ = :idt_jzrq WHERE ZFGH = :userid AND JZRQ IS NULL AND ZFRQ < :ldt_End and JGID = :gl_jgid";
				dao.doUpdate(hql_update, parameters);
			}
			...
			ll_Count = dao.doCount("ZY_ZYJS", hqlWhere,
					parameters);
			if (ll_Count > 0) {
				parameters.put("idt_jzrq", idt_jzrq);
				hql_update = "UPDATE ZY_ZYJS SET JZRQ = :idt_jzrq WHERE CZGH = :userid AND JZRQ IS NULL AND JSRQ < :ldt_End and JGID = :gl_jgid";
				dao.doUpdate(hql_update, parameters);
			}
			...
			ll_Count = 0;
			hqlWhere = "ZFGH = :userid AND JZRQ IS NULL AND ZFRQ < :ldt_End  and JGID = :gl_jgid";
			ll_Count = dao.doCount("ZY_JSZF", hqlWhere,
					parameters);
			if (ll_Count > 0) {
				parameters.put("idt_jzrq", idt_jzrq);
				hql_update = "UPDATE ZY_JSZF SET JZRQ = :idt_jzrq WHERE ZFGH = :userid AND JZRQ IS NULL AND ZFRQ < :ldt_End  and JGID = :gl_jgid";
				dao.doUpdate(hql_update, parameters);
			}
		} catch (PersistentDataOperationException e) {
			logger.error("Has done Checkout query failed", e);
...
//					para.put("adt_jzrq_e", adt_clrq_e);
					String ids_jsfp_hql = " SELECT a.FPHM as FPHM FROM ZY_ZYJS a,ZY_BRRY b  WHERE a.ZYH=b.ZYH and  a.CZGH =:as_czgh AND a.JGID = :al_jgid AND  a.JZRQ IS not NULL AND to_char(a.JZRQ,'yyyy-mm-dd hh24:mi:ss') =:adt_jzrq_s and a.FPHM is not null order by a.FPHM asc";
					...
					String ids_jksj_hql = " SELECT a.SJHM as SJHM from ZY_TBKK a WHERE a.CZGH = :as_czgh AND a.JGID = :al_jgid AND a.JZRQ IS not NULL AND to_char(a.JZRQ,'yyyy-mm-dd hh24:mi:ss')=:adt_jzrq_s and a.SJHM is not null order by a.SJHM asc";
			...
			long iws_pjxx1_pjzs = 0;
			long iws_pjxx2_pjzs = 0;
			hql = "SELECT a.PJLB as PJLB,a.PJHM as PJHM FROM ZY_ZFPJ a WHERE  a.CZGH = :as_czgh AND a.JGID = :al_jgid AND  to_char(a.JZRQ,'yyyy-mm-dd hh24:mi:ss') = :adt_jzrq_s";
			if (body.get("jzrq") != null) {
				hql = "SELECT a.PJLB as PJLB,a.PJHM as PJHM FROM ZY_ZFPJ a WHERE  a.CZGH = :as_czgh AND a.JGID = :al_jgid AND  to_char(a.JZRQ,'yyyy-mm-dd hh24:mi:ss') = :jzrq";
			}
	...
	// 取消汇总查询
		public void doQueryCancelCommit(Map<String, Object> req,
			...
			try {
				List<Map<String, Object>> list = dao
						.doSqlQuery(
								"select distinct to_char(JZRQ,'yyyy-mm-dd hh24:mi:ss') as JZRQ,CZGH as CZGH from ZY_JZXX where CZGH=:czgh and JGID=:jgid and HZRQ is null order by JZRQ desc",
								parameters);
				if (list.size() > 0) {
					res.put("body", list);
				}
		...
		public void doCancelCommit(Map<String, Object> req,
			...
			try {
				List<?> list = dao.doSqlQuery("SELECT JZRQ as JZRQ FROM ZY_JZXX WHERE CZGH =:czgh and JGID =:jgid and JZRQ=:jzrq and HZRQ is null", parameters);
				if (list.size() ==0){
					throw new ModelDataOperationException(
				...
				List<Map<String, Object>> list1 = dao
						.doSqlQuery(
								"select  to_char(max(JZRQ),'yyyy-mm-dd hh24:mi:ss') as JZRQ from ZY_JZXX where CZGH=:czgh and JGID=:jgid and HZRQ is null",
								jzmaxparameters);
				if (list1.size() >0){
				...
				dao.doUpdate("delete from ZY_ZFPJ where JGID=:jgid and CZGH=:czgh and JZRQ=:jzrq", parameters);
				dao.doUpdate("delete from ZY_JZXX where JGID=:jgid and CZGH=:czgh and JZRQ=:jzrq", parameters);
				dao.doUpdate("update ZY_TBKK set JZRQ = null where CZGH =:czgh  and JZRQ=:jzrq and JGID = :jgid", parameters);
				dao.doUpdate("update ZY_JKZF set JZRQ = null where ZFGH = :czgh  and JZRQ=:jzrq and JGID = :jgid", parameters);
				dao.doUpdate("update ZY_ZYJS set JZRQ = null where CZGH = :czgh  and JZRQ=:jzrq and JGID = :jgid", parameters);
				dao.doUpdate("update ZY_JSZF SET JZRQ = null where ZFGH = :czgh  and JZRQ=:jzrq and JGID = :jgid", parameters);
			} catch (PersistentDataOperationException e) {
				throw new ModelDataOperationException(
						ServiceCode.CODE_DATABASE_ERROR, "取消日报信息出错!");
			}
		...
		public void doQuerySQL(Map<String, Object> req, Map<String, Object> res,
			...
			try {
				if (req.containsKey("cnd")) {
					...
					hql1.append(" SELECT JZRQ as JZRQ FROM ZY_JZXX ")
		...
		public void doGetLastHZRQ(Map<String, Object> req, Map<String, Object> res,
			...
			try {
				...
				parameters.put("jgid", jgid);
				parameters.put("czgh", uid);
				String sql = "select to_char(max(jzrq),'yyyy-mm-dd hh24:mi:ss') as jzrq from ZY_JZXX where JGID=:jgid and CZGH = :czgh";
...
//				cck.doGetParameters(req, res, ctx);
//				if(res.get("amount")==null||(res.get("amount")+"").length()==0){
//					throw new ModelDataOperationException(
