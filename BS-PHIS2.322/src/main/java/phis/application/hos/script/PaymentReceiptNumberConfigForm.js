...
phis.application.hos.script.PaymentReceiptNumberConfigForm = function(cfg) {
	cfg.width = 750;
	phis.application.hos.script.PaymentReceiptNumberConfigForm.superclass.constructor
			.apply(this, [cfg])
	this.on("beforeSave", this.onBeforeSave, this);
}
Ext.extend(phis.application.hos.script.PaymentReceiptNumberConfigForm,
		phis.script.TableForm, {
			onReady : function() {
				phis.application.hos.script.PaymentReceiptNumberConfigForm.superclass.onReady
						.call(this)
			...
			onBeforeSave : function(entryName, op, saveData) {
				var sfqyjkgycs = phis.script.rmi.miniJsonRequestSync({
					serviceId : "paymentReceiptNumberConfigService",
					serviceAction : "queryJksjCount",
					schemaDetailsList : "ZY_YGPJ",
					op : this.op
				});
				if (sfqyjkgycs.code == 607) {
					MyMessageTip.msg("提示", sfqyjkgycs.msg, true);
					return false;
				}
				var s = /^[0-9]+$/i;
				...
				var r = phis.script.rmi.miniJsonRequestSync({
							serviceId : "paymentReceiptNumberConfigService",
							serviceAction : "paymentReceiptNumberConfigVerification",
							schemaDetailsList : "ZY_YGPJ",
							op : this.op,
							body : dataRepeat
						});
				...
				if (parseInt(saveData.QSHM, 10) > parseInt(saveData.DQHM, 10)
						&& saveData.QSHM != '' && saveData.DQHM != '') {
					this.DQHM.setValue(this.DQHMVLAUE);
					MyMessageTip.msg("提示", "起始号码不能大于使用号码", true);
					return false;
				}
				if (parseInt(saveData.ZZHM, 10) < parseInt(saveData.DQHM, 10)
						&& saveData.ZZHM != '' && saveData.DQHM != '') {
					this.DQHM.setValue(this.DQHMVLAUE);
				...
				var r = phis.script.rmi.miniJsonRequestSync({
							serviceId : "paymentReceiptNumberConfigService",
							serviceAction : "paymentReceiptNumberConflictVerification",
							schemaDetailsList : "ZY_YGPJ",
							op : this.op,
							body : dataConflict
						});
				...
				if (parseInt(this.DQHMVLAUE, 10) > parseInt(saveData.DQHM, 10)) {
					this.DQHM.setValue(this.DQHMVLAUE);
