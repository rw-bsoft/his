...
phis.application.hos.script.HospitalAccountingQueryModule = function(cfg) {
	this.exContext = {};
	phis.application.hos.script.HospitalAccountingQueryModule.superclass.constructor
			.apply(this, [cfg]);
}

Ext.extend(phis.application.hos.script.HospitalAccountingQueryModule,
		phis.script.SimpleModule, {
			...
			doQuery : function() {
				...
				var FYKS = form.findField("FYKS").getValue();
				...
				if (ZYHM) {
					...
					var r = phis.script.rmi.miniJsonRequestSync({// 补住院号
						serviceId : "hospitalCompensationNumberService",
						serviceAction : "getCompensationNumber",
						body : data
					});
					form.findField("ZYHM").setValue(r.json.ZYHM);
				ZYHM = r.json.ZYHM;
				}
			...
			doPrint : function() {
				...
				//url += "&ksrq=" + this.opener.opener.tbar[0].getValue().format('Ymd'); 
				 var fyksValue=FYKS.getValue();
				 if(fyksValue==null || fyksValue=="" || fyksValue=="null"){
					 fyksValue="null";
				 }
				 ...
				 url += "&" + FYKS.getName() + "=" + encodeURI(fyksValue);
