...
phis.application.twr.script.DRNewHospitalizationApplicationForm = function(cfg) {
	cfg.showButtonOnTop = true;
	this.colCount = cfg.colCount = 4;
	phis.application.twr.script.DRNewHospitalizationApplicationForm.superclass.constructor
			.apply(this, [cfg]);
	this.brid = '';
	this.mzhm = '';
	this.csny = '';
...
Ext.extend(phis.application.twr.script.DRNewHospitalizationApplicationForm,
		phis.script.TableForm, {
			loadData : function() {
				...
				var brinfo = phis.script.rmi.miniJsonRequestSync({
							...
							"EMPIID" : this.empiId,
							"departmentId" : this.mainApp.departmentId
						});
				...
				if (form) {
					...
					if (br.ZSXX) {
						form.findField("BQMS").setValue(br.ZSXX);
					}
					if(br.YSDH){
						form.findField("YSDH").setValue(br.YSDH);
					}
					form.findField("ZZYY").setValue("");
					form.findField("ZLJG").setValue("");
					form.findField("ZYSX").setValue("");
				}
			},
			doSubmit : function() {
