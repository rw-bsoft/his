...
phis.application.twr.script.DRNewHospitalizationApplicationFormLook = function(cfg) {
	this.colCount = cfg.colCount = 4;
	phis.application.twr.script.DRNewHospitalizationApplicationFormLook.superclass.constructor
			.apply(this, [ cfg ]);
}
Ext.extend(phis.application.twr.script.DRNewHospitalizationApplicationFormLook,
		phis.script.TableForm, {
			loadData : function() {
				var form = this.form.getForm();
				var brinfo = phis.script.rmi.miniJsonRequestSync({
					serviceId : "referralService",
					serviceAction : "queryZyScInfo",
				...
				if (form) {
					...
					if (br.JBMC) {
						form.findField("MSZD").setValue(br.JBMC);
					}
					if (br.ZHUANZHENYY) {
					...
					if (br.ZHUANZHENZYSX) {
						...
						form.findField("ZYSX").setValue(br.ZHUANZHENZYSX);
					}
					if (br.SHENQINGJGMC) {
