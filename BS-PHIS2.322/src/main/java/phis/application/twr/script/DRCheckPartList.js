...
phis.application.twr.script.DRCheckPartList = function(cfg) {
	cfg.autoLoadData = false;
	cfg.selectOnFocus = true;
	cfg.showRowNumber = false;
	phis.application.twr.script.DRCheckPartList.superclass.constructor.apply(this,
			[cfg])
}
 ...
 */
Ext.extend(phis.application.twr.script.DRCheckPartList, phis.script.SelectList,
		{
			onReady : function() {
				this.data = {}
			},
								...
								if (body) {
									...
									for (var i = 0; i < json.body.length; i++) {
										var rec = {
											...
											"partCode" : json.body[i].partCode,// 检查部位代码
											"partName" : json.body[i].partName,
										};
