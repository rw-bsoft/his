...
phis.application.emr.script.EMRmodeBlmbList = function(cfg) {
	cfg.autoLoadData = false;
	phis.application.emr.script.EMRmodeBlmbList.superclass.constructor.apply(
			this, [cfg])
	this.on("loadData", this.afterLoadData, this);
}

Ext.extend(phis.application.emr.script.EMRmodeBlmbList, phis.script.SimpleList,
		{
			expansion : function(cfg) {
				this.requestData.serviceId = "phis.emrManageService";
				this.requestData.serviceAction = "loadBlmb";
				if (this.openBy == "FSB") {
					this.requestData.openBy = "FSB";
				}
				var radioGroup = [{
							xtype : "radio",
							boxLabel : '病历模版',
			...
			depComboLoad : function(store) {
				// alert(Ext.encode(this.opener.opener.exContext.ids));
				// alert(Ext.encode(this.opener.opener.exContext.empiData.BRKS));
				this.depCombo
						.setValue(this.opener.opener.exContext.empiData.BRKS);
			...
			doRefresh : function() {
				...
				if (sslb == 1 && this.openBy != 'FSB') {
					if (this.depCombo.getValue()
							&& this.depCombo.getValue() != "0") {
				...
				if (sslb == 1) {
					if (this.openBy == 'FSB') {
						this.requestData.schema = "phis.application.emr.schemas.V_EMR_BLMB_FSB";
					} else {
						this.requestData.schema = "phis.application.emr.schemas.V_EMR_BLMB";
					}
				} else {
					this.requestData.KSDM = this.exContext.empiData.BRKS
					this.requestData.schema = "phis.application.emr.schemas.V_EMR_BLMB_PRI";
				}
				if (this.node.BLLX == 1) {
					this.requestData.BLLX = this.node.BLLX;
				}
				this.doCndQuery();
			},
