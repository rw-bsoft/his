...
phis.application.xnh.script.NhDiseasesManageList = function(cfg) {
	cfg.winState = "center";// cfg.winState=[100,50]两个写法都可以
	this.showButtonOnTop = true;
	phis.application.xnh.script.NhDiseasesManageList.superclass.constructor
			.apply(this, [cfg])
}
Ext.extend(phis.application.xnh.script.NhDiseasesManageList,
		phis.script.SimpleList, {
			doDownloads:function(){
				this.mask("正在努力下载中，可能需要很长时间，你可以去刷下微信，呼哈！");
				var dicName = {
				...
				var ret = phis.script.rmi.miniJsonRequestSync({
							...
							serviceAction : "savedownloadjbml",
							body:body
						});
				if (ret.code > 300) {
