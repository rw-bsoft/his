...
Ext.extend(com.bsoft.phis.pub.DepartmentManageModule,
		com.bsoft.phis.SimpleList, {
			...
			doTurnBranch : function() {
				var r = this.getSelectedRecord()
				if (r == null) {
					return
				}
				var module = this.createModule("refTurnBranch",
						this.refTurnBranch);
				var win = module.getWin();
				win.add(module.initPanel());
				module.on("doSubmit", this.doSubmit, this);
				module.requestData.cnd = ['eq', ['$', 'JGID'],
						['s', r.data.JGID]]
				module.refresh();
				win.show();
			},
			doSubmit : function(newksdm) {
				var r = this.getSelectedRecord()
				if (r == null) {
					return
				}
				var data = {
					YGDM : r.get("YGDM"),
					NEWKSDM : newksdm.KSDM
				}
				var ret = phis.script.rmi.miniJsonRequestSync({
							serviceId : "departmentManageService",
							serviceAction : "updateKSDM",
							body : data
						});
				if (ret.code > 300) {
					this.processReturnMsg(ret.code, ret.msg, this.onDblClick,
							this);
					MyMessageTip.msg("提示", "员工转科失败!", true);
					return;
				} else {
					MyMessageTip.msg("提示", "员工转科成功!", true);
					this.refresh();
					return;
				}
			}
