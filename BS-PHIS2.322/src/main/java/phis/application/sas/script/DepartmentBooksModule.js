...
$import("phis.script.SimpleModule", "util.helper.Helper");
		phis.application.sas.script.DepartmentBooksModule = function(cfg) {
			phis.application.sas.script.DepartmentBooksModule.superclass.constructor
					.apply(this, [ cfg ]);
		},
		Ext
				.extend(
						phis.application.sas.script.DepartmentBooksModule,
						phis.script.SimpleModule,
						{
							initPanel : function() {
								...
								var radioGroup = new Ext.form.RadioGroup(
										{
											...
											listeners : {
												change : function(group,
														newValue, oldValue) {
													if (this.tabModule.midiModules["departmentBooksSearch"]) {
														this.tabModule.midiModules["departmentBooksSearch"]
																.loadData(newValue.inputValue);
													} else {
														this.getMode();
														this.tabModule.ZBLB = newValue.inputValue;
													}
													if (this.tabModule.midiModules["departmentBooksDetails"]) {
														this.tabModule.midiModules["departmentBooksDetails"]
																.loadData(newValue.inputValue);
													} else {
														this.getMode();
														this.tabModule.ZBLB = newValue.inputValue;
								...
								radioGroup
										.setValue(zblbItems[0].initialConfig.inputValue);
							...
							getMode : function() {
								this.tabModule = this.createModule("getMode",
										this.refMode);
