...
phis.application.hph.script.HospitalPharmacyBackMedicineUnderTabModule = function(cfg) {
	this.records=[];
	phis.application.hph.script.HospitalPharmacyBackMedicineUnderTabModule.superclass.constructor.apply(
			this, [cfg])
}

Ext.extend(phis.application.hph.script.HospitalPharmacyBackMedicineUnderTabModule,
				...
				if (op == "loadData") {
					...
					arr.push("");
						...
						var cnd = ['and', module.cnds,
								['in', ['$', 'a.ZYH'], arr]];
						if(record.length>0){
						cnd=['and',cnd,['eq',['$','a.TYBQ'],['d',record[0].json.TYBQ]],['eq',['$','a.YFSB'],['l',this.mainApp['phis'].pharmacyId]]]
						}
