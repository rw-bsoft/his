...
phis.application.hph.script.HospitalPharmacyAccountingList = function(cfg) {
	...
	phis.application.hph.script.HospitalPharmacyAccountingList.superclass.constructor
			.apply(this, [cfg])
	this.on("afterCellEdit", this.onAfterCellEdit, this);
}

Ext.extend(phis.application.hph.script.HospitalPharmacyAccountingList,
...
//				var factory = util.dictionary.DictionaryLoader
				...
				data.LSJE = (parseFloat(data.YPSL)*parseFloat(data.LSJG)).toFixed(2);
...
//						obj.hasFocus = false;// add by yangl 2013.02.18
					...
					record.data.pharmacyId = this.opener.mainApp.pharmacyId;
					record.data.BRXZ = this.opener.form.form.getForm().findField("BRXZ").getValue();
//					console.debug(record.data);
					var ret = phis.script.rmi.miniJsonRequestSync({
						serviceId : "hospitalPharmacyService",
					...
					if (ret.code > 300) {
						...
						if (ret.json.body) {
							...
							this.doRemove();
							this.setBackSet(ret.json.body,0);
//							for(var i = 0 ; i < ret.json.body.length ; i ++){
//								this.setBackSet(ret.json.body[i].TYPE,ret.json.body[i]);
//							}
//							this.doCreate();
						}
					}
...
//				}
			...
			doJshj : function() {
				...
				if(this.datas&&this.i<this.datas.length){
					this.setBackSet(this.datas, this.i);
				}
			},
			setYPSL : function(record,ypsl){
				ypsl=parseFloat(ypsl);
				if(ypsl<0){
				...
				if (r.code > 300) {
					MyMessageTip.msg("提示", "【"+record.data.YPMC+"】"+r.msg, true);
					record.set("YPSL",0);
					record.set("LSJE",0);
					this.doJshj();
					return;
					...
					if(retRecord.TYPE==1){//后台查询返回只有一条记录的时候(KCSB不为0或者相同的记账记录只有一条)
						var srsl=parseFloat(record.get("YPSL"));
						if(retRecord.YPSL){
							if(parseFloat(retRecord.YPSL)<srsl){
								MyMessageTip.msg("提示","输入数量大于库存数量,库存数量为:"+parseFloat(retRecord.YPSL),true);
								srsl=parseFloat(retRecord.YPSL);
								record.set("YPSL",srsl);
							}
						}
						record.set("LSJG",retRecord.LSJG);
						record.set("JHJG",retRecord.JHJG);
						record.set("KCSB",retRecord.KCSB);
						record.set("LSJE",(srsl*parseFloat(record.get("LSJG"))).toFixed(2));
						this.doJshj();
						return;
					}
					...
					var cnd;
					if(ypsl>0){
						this.list = this.createModule("kclist", this.refKcList);
						cnd=['and',['eq',['$','a.YPXH'],['l',record.get("YPXH")]],['eq',['$','a.YPCD'],['l',record.get("YPCD")]],['eq',['$','a.YFSB'],['l',this.mainApp['phis'].pharmacyId]]];
						this.list.YPSL=ypsl;
					}else{
						this.list = this.createModule("jzlist", this.refList);
						cnd=['and',['in',['$','KCSB'],retRecord.KCSB],['eq',['$','ZYH'],['l',ZYH]]];
					}
...
//					}
			...
			doCreate : function(item, e) {
				...
				phis.application.hph.script.HospitalPharmacyAccountingList.superclass.doCreate
						.call(this);
				var store = this.grid.getStore();
				var n = store.getCount() - 1
				store.getAt(n).set("LB",this.getLb());
			...
			onCheckData : function(sbxh,jhjg,lsjg,ypsl) {
				var count=this.store.getCount();
				for(var i=0;i<count;i++){
				if(this.store.getAt(i).get("KCSB")==sbxh&&this.store.getAt(i)!=this.checkRecord){
				...
				var dpYpsl=this.checkRecord.get("YPSL");
				if(ypsl){
				dpYpsl=ypsl;
				}
				this.checkRecord.set("YPSL", parseFloat(dpYpsl));
				this.checkRecord.set("KCSB", sbxh);
				this.checkRecord.set("JHJG", jhjg);
				this.checkRecord.set("LSJG", lsjg);
				this.checkRecord.set("LSJE",(parseFloat(dpYpsl)*parseFloat(this.checkRecord.get("LSJG"))).toFixed(2));
				this.onClose();
				this.doJshj();
			},
			onClose : function() {
				if (this.checkRecord.get("KCSB") == null
					...
					MyMessageTip.msg("提示", "未选择对应记录!", true);
