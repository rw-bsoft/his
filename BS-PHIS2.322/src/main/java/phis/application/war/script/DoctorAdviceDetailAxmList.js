...
phis.application.war.script.DoctorAdviceDetailAxmList = function(cfg) {
	cfg.disablePagingTbr = true; // 分页暂时不要
	cfg.autoLoadData =false;
	phis.application.war.script.DoctorAdviceDetailAxmList.superclass.constructor
			.apply(this, [cfg]);
}
Ext.extend(phis.application.war.script.DoctorAdviceDetailAxmList,
		phis.script.SelectList, {
			loadData : function(al_zyh) {
				this.requestData.serviceId = "phis.doctorAdviceExecuteService";
				this.requestData.serviceAction = "xmQuery";
				phis.application.war.script.DoctorAdviceDetailAxmList.superclass.loadData.call(this);
			},
