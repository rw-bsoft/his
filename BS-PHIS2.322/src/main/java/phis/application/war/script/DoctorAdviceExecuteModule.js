...
phis.application.war.script.DoctorAdviceExecuteModule = function(cfg) {
	this.exContext = {};
	cfg.mutiSelect = true;
	cfg.mutiSelect = this.mutiSelect = true;
	phis.application.war.script.DoctorAdviceExecuteModule.superclass.constructor.apply(
			this, [cfg]);
	this.on("winShow", this.onWinShow, this);
	this.on("close", this.onWinClose, this);// add by yangl 关闭时抛出事件
}
Ext.extend(phis.application.war.script.DoctorAdviceExecuteModule,
...
//					}
			...
			onListRowClick : function(patientGrid, rowIndex, e) {
				...
				this.detailsList.ZYH = r.data.ZYH;
				this.detailsList.loadData(r.data.ZYH);
			},
