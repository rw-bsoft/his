...
phis.application.war.script.WardDoctorPrintManagementRightModule = function(
		cfg) {
	phis.application.war.script.WardDoctorPrintManagementRightModule.superclass.constructor
			.apply(this, [cfg]);
	this.on("tabchange", this.onChange, this)
}
Ext.extend(
		phis.application.war.script.WardDoctorPrintManagementRightModule,
		phis.script.TabModule, {
			...
			doJxdy:function(){
				...
				url+="&ZYH="+this.zyh+"&DYLX=jxdy&yzzt="+this.yzzt+"&YZLX="+yzqx;
...
//				};		
			...
			if(this.yzzt==2){
				...
				win.center()
				if (!win.hidden) {
					this.zdhList.requestData.cnd=['and',['eq',['$','b.YZQX'],["i",yzqx]],['eq',['$','b.ZYH'],["l",this.zyh]]]
				...
				win.center()
				if (!win.hidden) {
					this.zdhList_ls.requestData.cnd=['and',['eq',['$','b.YZQX'],["i",yzqx]],['eq',['$','b.ZYH'],["l",this.zyh]]]
