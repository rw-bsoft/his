...
phis.application.war.script.EMRmodeBllbTree = function(cfg) {
	this.checkModel = 'cascade';// 'multiple':多选; 'single':单选; 'cascade':级联多选
	this.checkTree = false;// 是否构建可以选择的树
	phis.application.war.script.EMRmodeBllbTree.superclass.constructor.apply(
			this, [cfg])
}
Ext.extend(phis.application.war.script.EMRmodeBllbTree,
...
//										}
			...
			beforeLoad : function(node) {
				if (node.id != "root") {
					...
					this.tree.loader.baseParams.entryName = 'phis.application.emr.schemas.EMRTYPE';
