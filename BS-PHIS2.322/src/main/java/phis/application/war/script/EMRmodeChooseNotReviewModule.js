...
phis.application.war.script.EMRmodeChooseNotReviewModule = function(cfg) {
	cfg.exContext = {};
	phis.application.war.script.EMRmodeChooseNotReviewModule.superclass.constructor.apply(this,
			[cfg]);
}

Ext.extend(phis.application.war.script.EMRmodeChooseNotReviewModule, phis.script.SimpleModule,
		{
			...
			getBllbTree : function() {
				var module = this.createModule("NotReviewlbTree_"+this.BLLB,
						this.NotReviewlbTree);
			...
			getBlmbAndGrmbList : function() {
				var module = this.createModule("NotReviewmbList_"+this.BLLB,
						this.NotReviewmbList);
				this.list = module;
			...
			doLoadEMR : function() {
				...
				r.data.mblb = this.list.radioGroup.getValue().inputValue;
				// 根据mbbh获取模版内容
				var data = r.data;
				var BLLX;
				if (data.BLLB == "294") {
