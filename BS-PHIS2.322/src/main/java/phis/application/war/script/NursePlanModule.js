...
phis.application.war.script.NursePlanModule = function(cfg) {
	// this.exContext = {};
	// this.listServiceId = "wardPatientManageService";
	// this.modeName = "dataview";
	cfg.width = 1024;
	cfg.height = 900;
	phis.application.war.script.NursePlanModule.superclass.constructor.apply(
			this, [cfg]);
}

Ext.extend(phis.application.war.script.NursePlanModule,
		phis.script.SimpleModule, {
			...
			doAction : function(item, e) {
				if (item.id == "createPlan") {// 新增
					this.midiModules[this.NPDataView].addJl02();
				} else if (item.id == "removePlan") {// 删除
			...
			doPrint : function() {
				...
				var module = this.createModule("nurseRecordDataShowprint",
						"phis.application.war.WAR/WAR/WAR6604")
				module.ZYH = this.zyh;
				module.initPanel();
				module.doPrint();
			}
