...
phis.application.war.script.InhospLisMedicalReportModule = function(cfg) {
	this.isModify = false;
	phis.application.war.script.InhospLisMedicalReportModule.superclass.constructor.apply(
			this, [cfg]);
	this.data = {};
}
 ...
 * LIS报告页面内嵌HIS
 */
Ext.extend(phis.application.war.script.InhospLisMedicalReportModule,
		phis.script.SimpleModule, {
			initPanel : function() {
				...
				/**
				 * yjlb:调用类别  (1:门诊/2:住院)
				 * jgdm:机构代码  this.data.jgdm
				 * brsy:病人ID    this.data.zyh
				 * ysgh:医生工号  this.data.ysgh
				 */
				var zdmc = "";//诊断名称
				var zdxh = 0;//诊断序号
				var zyhm = "";
				if(this.exContext.empiData){
					zyhm = this.exContext.empiData.ZYHM
				}else{
					zyhm = this.data.zyhm;
				}
				var url = "module=BGCX&yjlb=2&jgdm="+this.mainApp['phisApp'].deptId+"&patientid="+zyhm+"&ysgh="+this.mainApp.uid
				...
				if(this.mainApp['phisApp'].deptId=="320124005016"){
					url= "module=BGCX&yjlb=2&jgdm=320124005&patientid="+zyhm+"&ysgh="+this.mainApp.uid
				}
				...
				//开申请单时机构编码修改为320124004，科室代码修改为1109（群力分院）
					...
					url= "module=BGCX&yjlb=2&jgdm=320124004&patientid="+zyhm+"&ysgh="+this.mainApp.uid
				}
				//alert(url);
				if (this.panel)
					return this.panel;
