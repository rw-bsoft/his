...
phis.application.war.script.WardPatientAllergyMedList = function(cfg) {
	...
	this.removeRecords = [];
	phis.application.war.script.WardPatientAllergyMedList.superclass.constructor
			.apply(this, [cfg])

	this.on("loadData", this.afterLoadData, this);
	this.on("beforeCellEdit", this.beforeGridEdit, this);
}
Ext.extend(phis.application.war.script.WardPatientAllergyMedList,
		phis.script.EditorList, {
			initPanel : function(sc) {
				var grid = phis.application.war.script.WardPatientAllergyMedList.superclass.initPanel
						.call(this, sc)
			...
			doCreate : function(item, e) {
				...
				if (this.opener.exContext.record) {
					data.BRID = this.opener.exContext.record.get("BRID");
				} else {
					data.BRID = this.opener.exContext.ids.brid;
				}
			...
			setBackInfo : function(obj, record) {
				...
				// 添加过敏药物类别
				rowItem.set('GMYWLB', record.get("GMYWLB"));
				var factory = util.dictionary.DictionaryLoader;
				var gmywlbDic = factory.load({
							id : 'phis.dictionary.gmywlb'
						});
				var gmywlb = gmywlbDic.wraper[record.get("GMYWLB")];
				var gmywlb_text = '';
				if (gmywlb) {
					gmywlb_text = gmywlb.text;
				}
				rowItem.set('GMYWLB_text', gmywlb_text);
				//
