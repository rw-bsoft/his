...
public class WardOutPharmacySetService extends AbstractActionService implements
		DAOSupportable {
	protected BaseDAO dao;
	protected Logger logger = LoggerFactory
			.getLogger(WardOutPharmacySetService.class);
		...
		try {
			...
			for (int i = 0; i < body.size(); i++) {
				Map<String, Object> qxkz = (Map<String, Object>) body.get(i);
				   if(body.get(i).get("YFSB")== null || body.get(i).get("YFSB") == ""){
					   throw new RuntimeException("药房不能为空！");
				   }
				   if(body.get(i).get("BQDM")== null || body.get(i).get("BQDM") == ""){
					   throw new RuntimeException("病区不能为空！");
				   }
				   if(body.get(i).get("ZXPB")== null || body.get(i).get("ZXPB") == ""){
					   throw new RuntimeException("注销不能为空！");
				   }
				   if(body.get(i).get("TYPE")== null || body.get(i).get("TYPE") == ""){
					   throw new RuntimeException("医嘱类型不能为空！");
				   }
				   if(body.get(i).get("DMSB")== null || body.get(i).get("DMSB") == ""){
				    ...
				    long BQDM = Long.parseLong(body.get(i).get("BQDM")+"");
				    ...
				    if(body.get(i).get("JLXH") == null || body.get(i).get("JLXH")== ""){
				    	 qxkz.put("GNFL", 4);
						 qxkz.put("JGID", JGID);
						 qxkz.put("ZXPB",ZXPB);
						 qxkz.put("YFSB",YFSB);
						 ...
						 qxkz.put("DMSB",DMSB);
						 qxkz.put("BQDM",BQDM);
						 dao.doSave("create",BSPHISEntryNames.BQ_FYYF, qxkz, false);
						 }else{
					    	 long JLXH = Long.parseLong(body.get(i).get("JLXH")+"");
					    	 dao.doUpdate("update BQ_FYYF set GNFL=" + GNFL + " ,TYPE=" + TYPE
										+ " , YFSB=" + YFSB + " , DMSB='"+DMSB
	...
	public void doUpdateStage(Map<String, Object> req, Map<String, Object> res, 
		...
		try{
			dao.doUpdate("update BQ_FYYF set ZXPB="+ ZXPB+" where JLXH="+JLXH+" and JGID='"+JGID+"'", null);
