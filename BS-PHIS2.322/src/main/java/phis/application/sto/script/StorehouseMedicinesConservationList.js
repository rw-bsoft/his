...
phis.application.sto.script.StorehouseMedicinesConservationList = function(cfg) {
	cfg.modal = true;
	cfg.width=1020
	phis.application.sto.script.StorehouseMedicinesConservationList.superclass.constructor
			.apply(this, [cfg]);
}

Ext.extend(phis.application.sto.script.StorehouseMedicinesConservationList,
		phis.script.SimpleList, {
			...
			initPanel : function(sc) {
				...
				return phis.application.sto.script.StorehouseMedicinesConservationList.superclass.initPanel
						.call(this, sc);
			},
			// 加载数据
			loadData : function() {
				...
				if (simple) {
					...
					cnd = ['and', cnd,
							['eq', ['$', "str(YHRQ,'yyyy')"], ['s', stroeDate]]]
				}
				this.requestData.cnd = cnd;
				phis.application.sto.script.StorehouseMedicinesConservationList.superclass.loadData
						.call(this);
			},
			// 生成时间框
			...
			if(r.get("YSGH")!=null&&r.get("YSGH")!=""&&r.get("YSGH")!=undefined){
				return "已确认";
			}
			return "未确认";
			},
			...
			if(r==null){
			return;}
			var body={"YHDH":r.get("YHDH"),"XTSB":r.get("XTSB")}
			this.module = this.createModule("module", this.addRef);
			this.module.on("save", this.onSave, this);
			this.module.on("close", this.onClose, this);
				var win = this.module.getWin();
