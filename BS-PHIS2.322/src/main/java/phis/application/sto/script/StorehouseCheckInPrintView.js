...
phis.application.sto.script.StorehouseCheckInPrintView = function(cfg) {
	this.width = 800
	this.height = 500
	...
	this.conditions = []
	phis.application.sto.script.StorehouseCheckInPrintView.superclass.constructor.apply(
			this, [cfg])
}
Ext.extend(phis.application.sto.script.StorehouseCheckInPrintView, app.desktop.Module, {
	initPanel : function() {
		this.frameId = "SimplePrint_frame_storehouseIn";
		this.conditionFormId = "SimplePrint_form_storehouseIn";
		this.mainFormId = "SimplePrint_mainform_storehouseIn";
		var panel = new Ext.Panel({
			...
			html : "<iframe id='"
					+ this.frameId
					+ "' width='100%' height='100%' onload='simplePrintMask(\"storehouseIn\")'></iframe>"
		})
		this.panel = panel
		return panel
	},
	...
	doPrint : function() {
		var param=phis.script.common.loadSystemParams({"privates" : ['YKRKDDYZ']}).YKRKDDYZ;
		...
		var pages="phis.prints.jrxml.StorehouseIn2_"+param;
		 var url="resources/"+pages+".print?silentPrint=1";
		for (var i = 0; i < items.getCount(); i++) {
			var f = items.get(i)
			url += "&" + f.getName() + "=" + f.getValue()
		}
		url += "&temp=" + new Date().getTime() + "&xtsb=" + this.xtsb
				+ "&rkfs=" + this.rkfs + "&rkdh=" + this.rkdh+ "&fdjs=" + this.fdjs
		
		//zhaojian 2017-09-06 去除打印时的边框
