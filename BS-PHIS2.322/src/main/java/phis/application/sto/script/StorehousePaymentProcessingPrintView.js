...
phis.application.sto.script.StorehousePaymentProcessingPrintView = function(cfg) {
	this.printurl = util.helper.Helper.getUrl();
	phis.application.sto.script.StorehousePaymentProcessingPrintView.superclass.constructor.apply(
			this, [cfg])
}
Ext.extend(phis.application.sto.script.StorehousePaymentProcessingPrintView, app.desktop.Module, {
	initPanel : function() {
	...
	printPreview : function(btn) {
		...
		if (page == 'single') {
			var pages="phis.prints.jrxml.PaymentProcessing";
			 var url="resources/"+pages+".print?silentPrint=1&type="
					+ type + "&ypxh=" + this.requestData + "&ysdh=" + this.ysdh
					+ "&rkfs=" + this.rkfs; 
			 if(type == 1){
			 var LODOP=getLodop();
			...
			var pages="phis.prints.jrxml.PaymentProcessing";
			 var url="resources/"+pages+".print?silentPrint=1&type="
					+ type + "&ypxh=0" + "&ysdh=" + this.ysdh + "&rkfs="
					+ this.rkfs;
			 if(type == 1){
			 var LODOP=getLodop();
				LODOP.PRINT_INIT("打印控件");
