...
phis.application.sto.script.StorehousePriceBooksInventoryPrintView = function(cfg) {
	this.printurl = util.helper.Helper.getUrl();
	phis.application.sto.script.StorehousePriceBooksInventoryPrintView.superclass.constructor
			.apply(this, [cfg])
}
Ext.extend(phis.application.sto.script.StorehousePriceBooksInventoryPrintView,
		app.desktop.Module, {
			initPanel : function() {
			...
			printPreview : function(btn) {
				...
				if (page == 'single') {
					var pages="phis.prints.jrxml.MedicinesPriceBooksInventory";
					 var url="resources/"+pages+".print?silentPrint=1&type="
							+ type + "&ypxh=" + this.requestData + "&showZero="
							+ this.showZero;
					 if(type == 1){
						var LODOP=getLodop();
					...
					var pages="phis.prints.jrxml.MedicinesPriceBooksInventory";
					 var url="resources/"+pages+".print?silentPrint=1&type="
							+ type + "&ypxh=0" + "&showZero=" + this.showZero;
					 if(type == 1){
							var LODOP=getLodop();
							LODOP.PRINT_INIT("打印控件");
							LODOP.SET_PRINT_PAGESIZE("0","","","");
