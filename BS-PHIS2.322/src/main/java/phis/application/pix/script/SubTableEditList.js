...
$import("app.modules.list.EditorListView", "phis.application.pix.script.SubTableForm",
		"util.dictionary.DictionaryLoader");

phis.application.pix.script.SubTableEditList = function(cfg) {
	cfg.modified = false;
	this.disablePagingTbr = true ;
	phis.application.pix.script.SubTableEditList.superclass.constructor.apply(this, [cfg])
	this.on("afterCellEdit", this.onAfterCellEdit, this)
	this.on("beforeCellEdit",this.onBeforeCellEdit,this)
}

Ext.extend(phis.application.pix.script.SubTableEditList, app.modules.list.EditorListView, {
	...
	onReady : function() {
		// 新建记录时不允许刷新。
		phis.application.pix.script.SubTableEditList.superclass.onReady.call(this);
		this.fillStore();
	},
	...
	fillStore : function() {
		...
		if (!this.dic) {
			this.dic = util.dictionary.DictionaryLoader.load({
						id : "phis.dictionary.mpiExtension"
					});
		}
