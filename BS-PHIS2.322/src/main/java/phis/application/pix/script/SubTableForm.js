...
phis.application.pix.script.SubTableForm = function(cfg) {
	cfg.actions = [{
				id : "save",
				name : "确定"
			}, {
	...
	phis.application.pix.script.SubTableForm.superclass.constructor.apply(this, [cfg])
	this.saveServiceId = "empiService";
	...
	this.typeCodes = {
		"phis.application.pix.schemas.MPI_Address" : "addressTypeCode",
		"phis.application.pix.schemas.MPI_Phone" : "phoneTypeCode",
		"phis.application.pix.schemas.MPI_Card" : "cardTypeCode",
		"phis.application.pix.schemas.MPI_Certificate" : "certificateTypeCode"
	}
...
Ext.extend(phis.application.pix.script.SubTableForm, app.modules.form.MCFormView, {
	onReady : function() {
		if (this.entryName == "MPI_Certificate") {
	...
	doSave : function() {
		...
		if (!this.checkAdd(r)) {
			...
			if (this.entryName == "phis.application.pix.schemas.MPI_Card") {
	...
	saveCard:function(r){
		//查询是否有重复的证件或者卡.
		if(this.entryName =="MPI_Card" || this.entryName =="MPI_Certificate"){
			var queryData = {};
			if(this.entryName =="MPI_Certificate"){
				queryData = {
					certificates:[{
