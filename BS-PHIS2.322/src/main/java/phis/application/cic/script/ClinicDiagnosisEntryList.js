...
$import("phis.script.EditorList", "phis.script.common","phis.application.cic.script.frmz.ClinicFeverPatientList")
...
phis.application.cic.script.ClinicDiagnosisEntryList = function(cfg) {
	...
	this.refModule = "phis.application.cic.CIC/CIC/CIC5601";
}
var temp = 1;
Ext.extend(phis.application.cic.script.ClinicDiagnosisEntryList,
	phis.script.EditorList, {
					...
					switch (it.type) {
						...
						case 'date' :
							var cfg = {
								...
								emptyText : "请选择日期",
								format : 'Y-m-d',
								maxValue: new Date()		//新增发病日期控件的最大值为今天-- add by dinghaif 2019.05.08
							}
							if (this.selectOnFocus) {
								cfg.selectOnFocus = true;
							}
		...
		saveXGBDinfo : function(saveData){
			if (this.checkXGBD(saveData)) {
				var SBXH = 0;
				var result = util.rmi.miniJsonRequestSync({
					serviceId: "phis.clinicManageService",
					serviceAction : "querySBXH",
					jzxh : this.exContext.ids.clinicId
				});
				if (result.code==200){
					if(result.json.SBXH){
						SBXH = result.json.SBXH;
					}
				}
				if (SBXH == 0){//新增
					Ext.Msg.confirm("确认", "是否保存发热病人信息", function(btn) {
						if (btn == 'yes') {
							if(!this.module){
								this.module = this.createModule("refModule", this.refModule);
								this.module.opener = this;
								this.module.openerType = 2;//门诊调入
								this.module.brid = this.exContext.ids.brid;
								this.module.empiId = this.exContext.ids.empiId;
								this.module.jzxh = this.exContext.ids.clinicId;
							...
							this.module.empiId = this.exContext.ids.empiId;
							this.module.jzxh = this.exContext.ids.clinicId;
							this.module.northForm.initFormData(this.module.northForm.formData);
							win.show();
							win.center();
							var notrhForm = this.module.northForm.form.getForm();
							notrhForm.findField("LYBS").setValue(this.exContext.ids.clinicId);//门诊保存为JZXH
						} else 	{
							return;
						}
					}, this);
				}else{
					Ext.Msg.confirm("确认", "是否修改发热病人信息", function(btn) {
						if (btn == 'yes') {
							// 获取数据
							var result = util.rmi.miniJsonRequestSync({
		...
		checkXGBD : function(saveData) {
			for (var i = 0; i < saveData.length; i++) {
				// 如果疾病判别是发热门诊疾病
				if (saveData[i].JBPB == '21') {
					return true;
				}
			}
			return false;
		...
		beforeGridEdit : function(it, record, field, value) {
			if(it.id=='ZDSJ')
				field.setMaxValue(Date.parseDate(Date.getServerDate(),'Y-m-d').add(Date.DAY,1));
			if (it.id == "ZXLB") {
				if (record.get("ZDXH")) {
					return false;
				} else {
