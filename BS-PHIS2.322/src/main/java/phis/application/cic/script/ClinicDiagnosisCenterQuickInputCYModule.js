...
phis.application.cic.script.ClinicDiagnosisCenterQuickInputCYModule = function(cfg) {
	this.serviceId = cfg.serviceId;
	this.queryServiceAction = cfg.queryServiceAction;
	phis.application.cic.script.ClinicDiagnosisCenterQuickInputCYModule.superclass.constructor.apply(this, [cfg]);
	
}

Ext.extend(phis.application.cic.script.ClinicDiagnosisCenterQuickInputCYModule,phis.script.SimpleModule, {
			...
			getJBList : function() {
				...
				this.jblist.grid.on("rowdblclick", function(grid, index, e) {
							this.fireEvent("quickInput", "zdmcCY", grid.getStore().getAt(index));
						}, this)
			...
			getZHList : function() {
				...
				this.zhlist.grid.on("rowdblclick", function(grid, index, e) {
							this.fireEvent("quickInput", "zhCY", grid.getStore()
											.getAt(index));
						}, this)
