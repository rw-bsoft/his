...
public class ClinicDisposalEntryModule implements BSPHISEntryNames {
	...
	public void doSaveDisposalEntry(Map<String, Object> req,
			...
			long brid, String brxm, int djly, long ghgl)
		...
		int wpjfbz = Integer.parseInt("".equals(ParameterUtil.getParameter(
				...
				: ParameterUtil.getParameter(jgid,BSPHISSystemArgument.MZWPJFBZ, ctx));
		int wzsfxmjg = Integer.parseInt("".equals(ParameterUtil.getParameter(
				jgid, BSPHISSystemArgument.WZSFXMJG, ctx)) ? "0"
				: ParameterUtil.getParameter(jgid,BSPHISSystemArgument.WZSFXMJG, ctx));
		Long yjxh = (long) 0;
		...
		try {
			...
			String sql="select a.BRXZ as BRXZ from MS_GHMX a where a.SBXH=:SBXH";
			Map<String, Object> pa=new HashMap<String, Object>();
			pa.put("SBXH", ghgl);
			Map<String, Object> ma=dao.doLoad(sql, pa);
			String brxz=ma==null?"":ma.get("BRXZ")+"";
			String njjbylmsg="";
			for (int i = 0; i < body.size(); i++) {
				...
				if(brxz.equals("2000")){
					...
					String ylsql="select a.YYZBM as YYZBM from GY_YLMX a where a.JGID=:JGID and a.FYXH=:FYXH ";
					Map<String, Object> p=new HashMap<String, Object>();
					p.put("JGID",zbmjgid);
					p.put("FYXH",Long.parseLong(map_.get("YLXH")+""));
					Map<String, Object> ylmap=dao.doLoad(ylsql, p);
					if(ylmap==null || ylmap.get("YYZBM")==null || (ylmap.get("YYZBM")+"").length()==0){
						if(njjbylmsg.length()==0){
							njjbylmsg+="处置中"+map_.get("FYMC");
						}else{
							njjbylmsg+="、"+map_.get("FYMC");
						}
					}
				}
				if ((!"0".equals(map_.get("YJXH") + "")
					...
					// add by yangl 2014.01.14保存前增加是否已收费判断，已收费则不修改
					Map<String, Object> params = new HashMap<String, Object>();
					params.put("YJXH", Long.parseLong(map_.get("YJXH") + ""));
					Long l = dao.doCount("MS_YJ01","YJXH=:YJXH and MZXH is not null and MZXH>0",params);
					if (l > 0) {
						continue;
					}
					// ......end
					...
					if (map_.get("ZXKS") != null && map_.get("ZXKS") != "") {
						parameters.put("ZXKS",Long.parseLong(map_.get("ZXKS")+""));
					} else {
						parameters.put("ZXKS",0L);
					}
					if (map_.get("KSDM") != null && map_.get("KSDM") != "") {
						parameters.put("KSDM",Long.parseLong(map_.get("KSDM")+""));
					} else {
						parameters.put("KSDM",0L);
					}
					...
					parameters.put("YJXH",Long.parseLong(map_.get("YJXH") + ""));
					dao.doSave("update", BSPHISEntryNames.MS_YJ02_CIC, map_,false);
					if (wpjfbz == 1 && wzsfxmjg == 1) {
						BSPHISUtil.setSupplies(dao, ctx,Long.parseLong(map_.get("SBXH") + ""), ghgl);
					}
					StringBuffer hql = new StringBuffer();
					hql.append("update MS_YJ01 set ZXKS=:ZXKS,YSDM=:YSDM,KSDM=:KSDM where YJXH=:YJXH");
					...
					if (("0".equals(map_.get("YJXH") + "")) || map_.get("YJXH") == null
						|| map_.get("YJXH") == "") {// 如果当前条没有YJXH
						// 就新增一张表
						Map<String, Object> yj01Map = new HashMap<String, Object>();
						Long deptId = user.getProperty("biz_departmentId",Long.class);
						if (map_.get("KSDM") != null && map_.get("KSDM") != "") {
							yj01Map.put("KSDM",Long.parseLong(map_.get("KSDM") + ""));
						} else {
							yj01Map.put("KSDM",deptId);
						}
						if (map_.get("YSDM") == null || map_.get("YSDM").toString().trim().length()==0) {
						...
						if (map_.get("ZXKS") != null && map_.get("ZXKS") != "") {
							yj01Map.put("ZXKS",Long.parseLong(map_.get("ZXKS") + ""));
						} else {
							yj01Map.put("ZXKS", 0);
						}
						...
						list_2=	dao.doSqlQuery(sq, parameters);
						for(Map<String,Object> o: list_2){
							sqid=((BigDecimal)o.get("SQXH")).longValue();
							yj01Map.put("SQID", sqid);
							System.out.println("sqid=="+sqid);
						}
						Map<String, Object> yj01Req = dao.doSave("create",BSPHISEntryNames.MS_YJ01_CIC, yj01Map, false);
						...
						// add by yangl 2014.01.14保存前增加是否已收费判断
						Map<String, Object> params = new HashMap<String, Object>();
						params.put("YJXH",Long.parseLong(map_.get("YJXH") + ""));
						Long l = dao.doCount("MS_YJ01","YJXH=:YJXH and MZXH is not null and MZXH>0",params);
						if (l > 0) {
							throw new ModelDataOperationException("当前处置已经收费，不允许编辑!");
						}
					}
					// 接着保存明细
					String op = (String) body.get(i).get("_opStatus");
					Map<String, Object> sbxhMap = dao.doSave(op,BSPHISEntryNames.MS_YJ02_CIC, map_, false);
					if (wpjfbz == 1 && wzsfxmjg == 1) {
						BSPHISUtil.setSupplies(dao, ctx,Long.parseLong(sbxhMap.get("SBXH") + ""), ghgl);
					}
				}
			}
/*			if(njjbylmsg.length() >0){
				res.put("njjbylmsg",njjbylmsg+"未对照南京金保自编码，将导致收费处不能收费！");
			}*/
			res.put("body", rebody);
		...
		try {
			...
			if (mzxh.get("MZXH") != null) {
				...
				if ("1".equals(body.get("SIGN") + "")) {// 删除医技主项
					...
					dao.doRemove(pkey1, "phis.application.cic.schemas."
							+ schemaDetailsList);// 删一条明细
					Map<String, Object> parameters = new HashMap<String, Object>();
	...
	public List<Map<String, Object>> doLoadPersonalSet(
		...
		try {
			...
			params.put("zbmjgid", zbmjgid);
			StringBuffer hql = new StringBuffer(
					"select distinct a.JLBH,b.FYXH as FYXH,a.XMMC as XMMC,a.XMSL as XMSL,a.YPZH as YPZH,c.FYDJ as FYDJ,b.FYGB as FYGB,b.FYDW as FYDW,b.XMLX as XMLX,c.FYKS as FYKS,b.MZSY as MZSY,decode(d.YYZBM,'',' ','医保可报销') as YYZBM");
			hql.append(" from YS_MZ_ZT02 a,GY_YLSF b,GY_YLMX c,GY_YLMX d ");
			hql.append(" where  a.XMBH = b.FYXH AND  b.FYXH = c.FYXH and a.ZTBH = :ZTBH and c.JGID=:JGID and d.FYXH=a.XMBH and d.JGID=:zbmjgid ORDER BY a.JLBH");
	...
	public Map<String, Object> doLoadCostInfo(Map<String, Object> body,
		...
		try {
			...
			params.put("zbmjgid", zbmjgid);
			StringBuffer hql = new StringBuffer(
					"select distinct b.FYXH as FYXH,a.XMMC as XMMC,a.XMSL as XMSL,c.FYDJ as FYDJ,b.FYGB as FYGB,b.FYDW as FYDW,b.XMLX as XMLX,c.FYKS as FYKS,decode(d.YYZBM,'',' ','医保可报销') as YYZBM");
			hql.append(" from YS_MZ_ZT02 a,GY_YLSF b,GY_YLMX c,GY_YLMX d ");
			hql.append(" where  a.XMBH = b.FYXH AND  b.FYXH = c.FYXH and a.JLBH = :JLBH and c.JGID=:JGID and d.FYXH=a.XMBH and d.JGID=:zbmjgid ORDER BY a.ZTBH");
			Map<String, Object> meds = dao.doSqlLoad(hql.toString(), params);
	...
	public Map<String, Object> doLoadclinicAll(Map<String, Object> body,
			Context ctx) throws ModelDataOperationException, ControllerException {
		UserRoleToken user = UserRoleToken.getCurrent();
		String jgid = user.getManageUnit().getId();
		try {
			String zbmjgid="";
			...
			params.put("zbmjgid", zbmjgid);
			StringBuffer hql = new StringBuffer(
					"select distinct a.FYXH as FYXH,a.FYMC as XMMC,a.XMLX as XMLX,1 as XMSL,b.FYDJ as FYDJ,a.FYGB as FYGB ,a.FYDW as FYDW,decode(c.YYZBM,'',' ','医保可报销') as YYZBM");
			hql.append(" from GY_YLSF a,GY_YLMX b,GY_YLMX c ");
			hql.append(" where a.FYXH = b.FYXH and a.FYXH = :FYXH and b.JGID=:JGID and c.FYXH = a.FYXH and c.JGID=:zbmjgid and a.MZSY=1 ORDER BY a.FYXH");
			Map<String, Object> meds = dao.doSqlLoad(hql.toString(), params);
			return meds;
		} catch (PersistentDataOperationException e) {
			logger.error(
		...
		try {
			...
			if (ghgl != 0 && HSQL == null) {
				...
				hqlksdm.append(
						...
						.append(" MS_GHMX a,")
						.append(" SYS_Office b,")
						.append(" MS_GHKS c where a.KSDM=c.KSDM and c.MZKS=b.ID and a.SBXH=:sbxh");
			...
			if (HSQL == null) {
				...
				Map<String, Object> ksdmMap = dao
						.doLoad("select a.BRBQ as BRBQ,b.OFFICENAME as KSMC from ZY_BRRY a,SYS_Office b where a.BRBQ=b.ID and a.ZYH=:ZYH and a.JGID=:JGID",
								parametersksdm);
				if (ksdmMap != null) {
				...
				if (map != null) {
					...
					for (int i = 0; i < bodys.size(); i++) {
						...
						long FYXH = Long.parseLong(bodys.get(i).get("YLXH")
								+ "");
						int YLSL = Integer.parseInt(Math.round(Double
								.parseDouble(bodys.get(i).get("YLSL") + ""))
								+ "");// 数量
	...
	public void doGetKsxx(Map<String, Object> req, Map<String, Object> res,
		...
		String sql = "select b.ID as ksdm,b.OFFICENAME as ksmc  from SYS_Personnel a left join SYS_Office b on a.OFFICECODE = b.ID where a.PERSONID=:ygdm";
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("ygdm", req.get("ygdm") + "");
			List<Map<String, Object>> list = dao.doSqlQuery(sql, map);
	...
	public void doGetJcbwdm(Map<String, Object> req, Map<String, Object> res,
			BaseDAO dao2, Context ctx) throws ModelDataOperationException {
		String sql = "select b.JCBWDM as JCBWDM,b.JCBWMC as ksmc  from GY_JCBW b where b.YXBZ=:YXBZ and b.JCLB=:JCLB ";
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("YXBZ",  "Y");
			map.put("JCLB",  req.get("JCLB")+"");
			List<Map<String, Object>> list = dao.doSqlQuery(sql, map);
			res.put("jcbwdm", list.get(0).get("JCBWDM"));
			res.put("jcbwmc", list.get(0).get("JCBWMC"));
		} catch (PersistentDataOperationException e) {
			e.printStackTrace();
			throw new ModelDataOperationException("科室信息查询失败！", e);
		}
