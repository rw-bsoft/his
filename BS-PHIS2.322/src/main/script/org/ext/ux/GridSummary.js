$package("org.ext.ux.grid")
$styleSheet("phis.resources.css.ext.ux.grid.GridSummary")

org.ext.ux.grid.GridSummary = function(config) {
    Ext.apply(this, config);
};

Ext.extend(org.ext.ux.grid.GridSummary, Ext.util.Observable, {
  init : function(grid) {
    this.grid = grid;
    ...
    for (var i = 0, len = cs.length; i < len; i++) {
      ...
      for (var j = 0, jlen = rs.length; j < jlen; j++) {
        ...
        if (cf && cf.summaryType) {
          data[c.name] = org.ext.ux.grid.GridSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data);
        }
      }
    }
...
org.ext.ux.grid.GridSummary.Calculations = {
